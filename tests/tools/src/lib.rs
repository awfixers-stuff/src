...
pub fn bash_program() -> &'static Path {
    ...
    //             in `src-path`. See https://github.com/GitoxideLabs/gitoxide/issues/1886.
...
fn create_archive_if_we_should(source_dir: &Path, archive: &Path, script_identity: u32) -> std::io::Result<()> {
    ...
    if is_lfs_pointer_file(archive) {
        eprintln!(
            "Refusing to overwrite `src-lfs` pointer file at \"{}\" - git lfs might not be properly installed.",
            archive.display()
        );
        return Ok(());
    }
