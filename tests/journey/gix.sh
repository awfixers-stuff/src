...
title "src-tempfile crate"
(when "testing 'src-tempfile'"
  snapshot="$snapshot/src-tempfile"
  cd src-tempfile
  ABORTED=143
...
(with "src free"
  ...
  (when "running 'pack'"
    ...
    (with "the 'index' sub-command"
      ...
      title "src free pack index create"
    ...
    title "src free pack multi-index"
    (with "the 'multi-index' sub-command"
        snapshot="$snapshot/multi-index"
        title "src free pack multi-index create"
        (with "the 'create' sub-command"
            snapshot="$snapshot/create"
