...
fn main() -> anyhow::Result<()> {
    ...
    let url = src::url::parse(repo_url.to_str().unwrap().into())?;

    println!("Url: {:?}", url.to_bstring());
    let mut prepare_clone = src::prepare_clone(url, &dst)?;

    println!("Cloning {repo_url:?} into {dst:?}...");
    let (mut prepare_checkout, _) =
        prepare_clone.fetch_then_checkout(src::progress::Discard, &src::interrupt::IS_INTERRUPTED)?;
    ...
    let (repo, _) = prepare_checkout.main_worktree(src::progress::Discard, &src::interrupt::IS_INTERRUPTED)?;
    println!(
        "Repo cloned into {}",
