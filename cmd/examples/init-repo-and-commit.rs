...
use anyhow::Context;
use src::{config::tree::Author, objs::tree};
...
fn main() -> anyhow::Result<()> {
    ...
    let mut repo = src::init_bare(git_dir)?;

    println!("Repo (bare): {}", repo.git_dir().display());

    let mut tree = src::objs::Tree::empty();
    ...
    config.set_raw_value(&Author::EMAIL, "maria@example.com")?;
    {
        ...
        let initial_commit_id = repo.commit("HEAD", "initial commit", empty_tree_id, src::commit::NO_PARENT_IDS)?;
