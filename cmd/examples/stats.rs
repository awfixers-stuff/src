...
use src::{prelude::ObjectIdExt, Reference};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut repo = src::discover(".")?;
...
mod visit {
    use src_hash::oid;
    use src_object::{bstr::BStr, tree::EntryRef};
    use src_traverse::tree::visit::Action;
    ...
    pub(crate) struct Tree {
        ...
        pub num_bytes: u64,
        pub repo: src::Repository,
    }

    impl Tree {
        pub fn new(repo: src::Repository) -> Self {
            Tree {
                num_trees: 0,
                num_links: 0,
                num_blobs: 0,
    ...
    impl src_traverse::tree::Visit for Tree {
