...
pub enum Error {
    ...
    #[error(transparent)]
    TreeIndexDiff(#[from] src_diff::index::Error),
}
...
pub struct Outcome {
    ...
    pub rewrite: Option<src_diff::rewrites::Outcome>,
    ...
    pub tree_index: src_index::State,
}
...
impl Repository {
    ...
    pub fn tree_index_status<'repo, E>(
        &'repo self,
        tree_id: &src_hash::oid,
        worktree_index: &src_index::State,
        pathspec: Option<&mut crate::Pathspec<'repo>>,
        renames: TrackRenames,
        mut cb: impl FnMut(
            src_diff::index::ChangeRef<'_, '_>,
            &src_index::State,
            &src_index::State,
        ) -> Result<src_diff::index::Action, E>,
    ) -> Result<Outcome, Error>
    where
        E: Into<Box<dyn std::error::Error + Send + Sync>>,
    {
        let _span = src_trace::coarse!("src::tree_index_status");
        let tree_index: src_index::State = self.index_from_tree(tree_id)?.into();
        ...
        if pathspec.is_none() {
            pathspec_storage = self
                .pathspec(
                    ...
                    None::<&str>,
                    false,
                    &src_index::State::new(self.object_hash()),
        ...
        let rewrite = src_diff::index(
            &tree_index,
            worktree_index,
            |change| cb(change, &tree_index, worktree_index),
            rewrites
                .zip(resource_cache.as_mut())
                .map(|(rewrites, resource_cache)| src_diff::index::RewriteOptions {
                    resource_cache,
                    find: self,
                    rewrites,
                }),
