...
use src_status::index_as_worktree::traits::{CompareBlobs, SubmoduleStatus};
...
pub enum Error {
    ...
    #[error(transparent)]
    Prefix(#[from] src_path::realpath::Error),
    #[error(transparent)]
    FilesystemOptions(#[from] config::boolean::Error),
    #[error(transparent)]
    IndexAsWorktreeWithRenames(#[from] src_status::index_as_worktree_with_renames::Error),
    #[error(transparent)]
...
pub struct Options {
    ...
    pub sorting: Option<src_status::index_as_worktree_with_renames::Sorting>,
    ...
    pub rewrites: Option<src_diff::Rewrites>,
...
impl Repository {
    ...
    pub fn index_worktree_status<'index, T, U, E>(
        &self,
        index: &'index src_index::State,
        patterns: impl IntoIterator<Item = impl AsRef<BStr>>,
        delegate: &mut impl src_status::index_as_worktree_with_renames::VisitEntry<
            'index,
            ContentChange = T,
            SubmoduleStatus = U,
        >,
        ...
        progress: &mut dyn src_features::progress::Progress,
        should_interrupt: &AtomicBool,
        options: Options,
    ) -> Result<src_status::index_as_worktree_with_renames::Outcome, Error>
    where
        T: Send + Clone,
        U: Send + Clone,
        E: std::error::Error + Send + Sync + 'static,
    {
        let _span = src_trace::coarse!("src::index_worktree_status");
        ...
        let resource_cache = crate::diff::resource_cache(
            self,
            src_diff::blob::pipeline::Mode::ToGit,
            attrs_and_excludes.inner,
            src_diff::blob::pipeline::WorktreeRoots {
                old_root: None,
                new_root: Some(workdir.to_owned()),
            },
        )?;

        let out = src_status::index_as_worktree_with_renames(
            index,
            workdir,
            delegate,
            compare,
            ...
            progress,
            src_status::index_as_worktree_with_renames::Context {
                pathspec: pathspec.search,
                resource_cache,
                should_interrupt,
                dirwalk: src_status::index_as_worktree_with_renames::DirwalkContext {
                    git_dir_realpath: git_dir_realpath.as_path(),
                    current_dir: cwd,
                    ignore_case_index_lookup: accelerate_lookup.as_ref(),
                },
            },
            src_status::index_as_worktree_with_renames::Options {
                sorting: options.sorting,
                object_hash: self.object_hash(),
                tracked_file_modifications: src_status::index_as_worktree::Options {
                    fs: fs_caps,
...
mod submodule_status {
    ...
    impl src_status::index_as_worktree::traits::SubmoduleStatus for BuiltinSubmoduleStatus {
        type Output = crate::submodule::Status;
        type Error = Error;

        fn status(&mut self, _entry: &src_index::Entry, rela_path: &BStr) -> Result<Option<Self::Output>, Self::Error> {
            use bstr::ByteSlice;
            if self
                .submodule_paths
...
pub enum Item {
    ...
    Modification {
        ...
        entry: src_index::Entry,
        /// The index of the `entry` for lookup in [`src_index::State::entries()`] - useful to look at neighbors.
        entry_index: usize,
        /// The repository-relative path of the entry.
        rela_path: BString,
        /// The computed status of the entry.
        status: src_status::index_as_worktree::EntryStatus<(), crate::submodule::Status>,
    },
    ...
    DirectoryContents {
        ...
        /// `dir_status` that wasn't the same as the one of `entry` and if [src_dir::walk::Options::emit_collapsed] was
        /// [CollapsedEntriesEmissionMode::OnStatusMismatch](src_dir::walk::CollapsedEntriesEmissionMode::OnStatusMismatch).
        /// It will also be `Some(dir_status)` if that option was [CollapsedEntriesEmissionMode::All](src_dir::walk::CollapsedEntriesEmissionMode::All).
        collapsed_directory_status: Option<src_dir::entry::Status>,
    },
    ...
    Rewrite {
        ...
        ///
        /// Note that its [`rela_path`](src_dir::EntryRef::rela_path) is the destination of the rewrite, and the current
        /// location of the entry.
        dirwalk_entry: src_dir::Entry,
        /// `collapsed_directory_status` is `Some(dir_status)` if this `dirwalk_entry` was part of a directory with the given
        /// `dir_status` that wasn't the same as the one of `dirwalk_entry` and if [src_dir::walk::Options::emit_collapsed] was
        /// [CollapsedEntriesEmissionMode::OnStatusMismatch](src_dir::walk::CollapsedEntriesEmissionMode::OnStatusMismatch).
        /// It will also be `Some(dir_status)` if that option was [CollapsedEntriesEmissionMode::All](src_dir::walk::CollapsedEntriesEmissionMode::All).
        dirwalk_entry_collapsed_directory_status: Option<src_dir::entry::Status>,
        ...
        /// Note that it doesn't (necessarily) exist in the object database, and may be [null](src_hash::ObjectId::null) if no hashing
        /// was performed.
        dirwalk_entry_id: src_hash::ObjectId,
        ...
        diff: Option<src_diff::blob::DiffLineStats>,
...
pub enum RewriteSource {
    ...
    RewriteFromIndex {
        ...
        /// equivalent to [Change::Removed](src_status::index_as_worktree::Change::Removed).
        ///
        /// Note that the [entry-id](src_index::Entry::id) is the content-id of the source of the rewrite.
        source_entry: src_index::Entry,
        /// The index of the `source_entry` for lookup in [`src_index::State::entries()`] - useful to look at neighbors.
        ...
        source_status: src_status::index_as_worktree::EntryStatus<(), crate::submodule::Status>,
    },
    ...
    CopyFromDirectoryEntry {
        ...
        ///
        /// Note that its [`rela_path`](src_dir::EntryRef::rela_path) is the source of the rewrite.
        source_dirwalk_entry: src_dir::Entry,
        ...
        /// if [src_dir::walk::Options::emit_collapsed] was [CollapsedEntriesEmissionMode::OnStatusMismatch](src_dir::walk::CollapsedEntriesEmissionMode::OnStatusMismatch).
        /// It will also be `Some(dir_status)` if that option was [CollapsedEntriesEmissionMode::All](src_dir::walk::CollapsedEntriesEmissionMode::All).
        source_dirwalk_entry_collapsed_directory_status: Option<src_dir::entry::Status>,
        ...
        source_dirwalk_entry_id: src_hash::ObjectId,
    },
}

///
pub mod iter {
    use src_status::index_as_worktree::{Change, EntryStatus};
    pub use src_status::index_as_worktree_with_renames::Summary;
    ...
    impl<'index> From<src_status::index_as_worktree_with_renames::RewriteSource<'index, (), SubmoduleStatus>>
        for RewriteSource
    {
        fn from(value: src_status::index_as_worktree_with_renames::RewriteSource<'index, (), SubmoduleStatus>) -> Self {
            match value {
                src_status::index_as_worktree_with_renames::RewriteSource::RewriteFromIndex {
                    index_entries: _,
                    source_entry,
                    source_entry_index,
                    source_rela_path,
                ...
                src_status::index_as_worktree_with_renames::RewriteSource::CopyFromDirectoryEntry {
                    source_dirwalk_entry,
    ...
    impl Item {
        ...
        pub fn summary(&self) -> Option<Summary> {
            use src_status::index_as_worktree_with_renames::Summary::*;
            Some(match self {
                ...
                Item::DirectoryContents { entry, .. } => {
                    if matches!(entry.status, src_dir::entry::Status::Untracked) {
                        Added
                    } else {
                        return None;
                    }
    ...
    impl<'index> From<src_status::index_as_worktree_with_renames::Entry<'index, (), SubmoduleStatus>> for Item {
        fn from(value: src_status::index_as_worktree_with_renames::Entry<'index, (), SubmoduleStatus>) -> Self {
            match value {
                src_status::index_as_worktree_with_renames::Entry::Modification {
                    entries: _,
                    entry,
                    entry_index,
                    rela_path,
                ...
                src_status::index_as_worktree_with_renames::Entry::DirectoryContents {
                    entry,
                ...
                src_status::index_as_worktree_with_renames::Entry::Rewrite {
                    source,
                    dirwalk_entry,
    ...
    where
        Progress: src_features::progress::Progress,
    {
