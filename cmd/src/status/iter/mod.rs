    ...
    Progress: src_features::progress::Progress,
{
    ...
    pub fn into_iter(
        ...
        let obtain_tree_id = || -> Result<Option<src_hash::ObjectId>, crate::status::into_iter::Error> {
        ...
        #[cfg(feature = "parallel")]
        {
            ...
            let join_tree_index = if let Some(tree_id) = obtain_tree_id()? {
                std::thread::Builder::new()
                    .name("src::status::tree_index::producer".into())
                    .spawn({
            ...
            let join_index_worktree = std::thread::Builder::new()
                .name("src::status::index_worktree::producer".into())
                .spawn({
                    let repo = self.repo.clone().into_sync();
                    ...
                    move || -> Result<_, index_worktree::Error> {
                        ...
                        let out = repo.index_worktree_status(
                            ...
                            patterns,
                            &mut collect,
                            src_status::index_as_worktree::traits::FastEq,
                            submodule,
                            &mut progress,
                            &should_interrupt,
                            options,
...
impl<'index> src_status::index_as_worktree_with_renames::VisitEntry<'index> for Collect {
    type ContentChange =
        <src_status::index_as_worktree::traits::FastEq as src_status::index_as_worktree::traits::CompareBlobs>::Output;
    type SubmoduleStatus = <BuiltinSubmoduleStatus as src_status::index_as_worktree::traits::SubmoduleStatus>::Output;

    fn visit_entry(
        &mut self,
        entry: src_status::index_as_worktree_with_renames::Entry<'index, Self::ContentChange, Self::SubmoduleStatus>,
    ) {
