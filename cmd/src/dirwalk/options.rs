use src_dir::walk::{CollapsedEntriesEmissionMode, EmissionMode, ForDeletionMode};

use crate::dirwalk::Options;

/// Construction
impl Options {
    pub(crate) fn from_fs_caps(caps: src_fs::Capabilities) -> Self {
        Self {
...
impl From<Options> for src_dir::walk::Options<'static> {
    fn from(v: Options) -> Self {
        src_dir::walk::Options {
...
impl Options {
    ...
    /// If `toggle` is `true`, entries that are pruned and whose [Kind](src_dir::entry::Kind) is known will be emitted.
