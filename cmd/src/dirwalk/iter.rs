...
pub struct Item {
    ...
    /// [src_dir::walk::CollapsedEntriesEmissionMode::OnStatusMismatch]. It will also be `Some(dir_status)` if that option
    /// was [src_dir::walk::CollapsedEntriesEmissionMode::All].
    pub collapsed_directory_status: Option<src_dir::entry::Status>,
}

impl Item {
    fn new(entry: src_dir::EntryRef<'_>, collapsed_directory_status: Option<src_dir::entry::Status>) -> Self {
        Item {
...
pub struct Outcome {
    ...
    pub excludes: src_worktree::Stack,
    ...
    pub dirwalk: src_dir::walk::Outcome,
}
    ...
    pub(crate) fn new(
        ...
        #[cfg(feature = "parallel")]
        {
            ...
            let handle = std::thread::Builder::new()
                .name("src::dirwalk::iter::producer".into())
                .spawn({
                    ...
                    move || -> Result<Outcome, dirwalk::Error> {
                        ...
                        Ok(Outcome {
                            ...
                            pathspec: out.pathspec.detach().map_err(|err| {
                                dirwalk::Error::Walk(src_dir::walk::Error::ReadDir {
...
impl src_dir::walk::Delegate for Collect {
    fn emit(
        &mut self,
        entry: src_dir::EntryRef<'_>,
