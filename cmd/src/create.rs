...
use src_config::parse::section;
use src_discover::DOT_GIT_DIR;
...
pub struct Options {
    ...
    pub fs_capabilities: Option<src_fs::Capabilities>,
}
...
pub fn into(
...
) -> Result<src_discover::repository::Path, Error> {
    ...
    let caps = {
        ...
        let mut config = src_config::File::default();
        let caps = {
            let caps = fs_capabilities.unwrap_or_else(|| src_fs::Capabilities::probe(&dot_git));
    ...
    Ok(src_discover::repository::Path::from_dot_git_dir(
        dot_git,
        if bare {
            src_discover::repository::Kind::PossiblyBare
        } else {
            src_discover::repository::Kind::WorkTree { linked_git_dir: None }
        },
        &src_fs::current_dir(caps.precompose_unicode)?,
    )
