...
mod init {
    ...
    pub unsafe fn init_handler(
        ...
        for sig in signal_hook::consts::TERM_SIGNALS {
            ...
            let action = move || {
                ...
                if msg_idx == grace_count {
                    src_tempfile::registry::cleanup_tempfiles_signal_safe();
                    signal_hook::low_level::emulate_default_handler(*sig).ok();
                }
                interrupt();
                super::trigger();
        ...
        src_tempfile::signal::setup(src_tempfile::signal::handler::Mode::None);
...
pub struct Iter<I, EFN> {
    ...
    inner: src_features::interrupt::IterWithErr<'static, I, EFN>,
}
    ...
    EFN: FnOnce() -> E,
{
    ...
    pub fn new(inner: I, make_err: EFN) -> Self {
        Iter {
            inner: src_features::interrupt::IterWithErr::new(inner, make_err, &IS_INTERRUPTED),
        }
    }
...
pub struct Read<R> {
    ...
    inner: src_features::interrupt::Read<'static, R>,
}
    ...
    R: io::Read,
{
    ...
    pub fn new(read: R) -> Self {
        Read {
            inner: src_features::interrupt::Read {
                inner: read,
