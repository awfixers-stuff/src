...
pub mod open_modules_file {
    ...
    pub enum Error {
        #[error(transparent)]
        Configuration(#[from] src_config::parse::Error),
...
pub mod is_active {
    ...
    pub enum Error {
        #[error(transparent)]
        InitIsActivePlatform(#[from] src_submodule::is_active_platform::Error),
        #[error(transparent)]
        QueryIsActive(#[from] src_config::value::Error),
        #[error(transparent)]
        InitAttributes(#[from] crate::config::attribute_stack::Error),
        #[error(transparent)]
        InitPathspecDefaults(#[from] src_pathspec::defaults::from_environment::Error),
        #[error(transparent)]
...
pub mod fetch_recurse {
    ...
    pub enum Error {
        #[error(transparent)]
        ModuleBoolean(#[from] src_submodule::config::Error),
        #[error(transparent)]
...
pub mod open {
    ...
    pub enum Error {
        ...
        #[error(transparent)]
        PathConfiguration(#[from] src_submodule::config::path::Error),
        #[error(transparent)]
