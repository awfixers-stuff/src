...
pub use src_submodule::*;

use crate::{bstr::BStr, is_dir_to_mode, worktree::IndexPersistedOrInMemory, Repository, Submodule};

pub(crate) type ModulesFileStorage = src_features::threading::OwnShared<src_fs::SharedFileSnapshotMut<File>>;
/// A lazily loaded and auto-updated worktree index.
pub type ModulesSnapshot = src_fs::SharedFileSnapshot<File>;
...
impl<'repo> SharedState<'repo> {
    ...
    fn active_state_mut(
        &self,
    ) -> Result<(RefMut<'_, IsActivePlatform>, RefMut<'_, src_worktree::Stack>), is_active::Error> {
...
impl Submodule<'_> {
    ...
    ///
    /// For details, see [src_submodule::File::path()].
    ...
    pub fn url(&self) -> Result<src_url::Url, config::url::Error> {
    ...
    pub fn shallow(&self) -> Result<Option<bool>, src_config::value::Error> {
    ...
    ///
    /// Please see the [plumbing crate documentation](src_submodule::IsActivePlatform::is_active()) for details.
    ...
    pub fn index_id(&self) -> Result<Option<src_hash::ObjectId>, index_id::Error> {
        let path = self.path()?;
        Ok(self
            .state
            .index()?
            .entry_by_path(&path)
            .and_then(|entry| (entry.mode == src_index::entry::Mode::COMMIT).then_some(entry.id)))
    }
    ...
    pub fn head_id(&self) -> Result<Option<src_hash::ObjectId>, head_id::Error> {
        let path = self.path()?;
        Ok(self
            .state
            .repo
            .head_commit()?
            .tree()?
            .peel_to_entry_by_path(src_path::from_bstr(path.as_ref()))?
    ...
    pub fn git_dir(&self) -> PathBuf {
        self.state
            ...
            .common_dir()
            .join("modules")
            .join(src_path::from_bstr(self.name()))
    }
    ...
    pub fn work_dir(&self) -> Result<PathBuf, config::path::Error> {
        let worktree_git = src_path::from_bstr(self.path()?);
    ...
    fn worktree_gitdir(&self) -> Result<PathBuf, config::path::Error> {
        Ok(self.work_dir()?.join(src_discover::DOT_GIT_DIR))
    }
}
...
pub mod status {
    ...
    impl Submodule<'_> {
        ...
        pub fn status_opts(
            ...
            adjust_options: &mut dyn for<'a> FnMut(
                crate::status::Platform<'a, src_features::progress::Discard>,
            )
                -> crate::status::Platform<'a, src_features::progress::Discard>,
        ) -> Result<Status, Error> {
            ...
            let statuses = adjust_options(sm_repo.status(src_features::progress::Discard)?)
    ...
    pub(super) mod types {
        ...
        pub struct Status {
            ...
            pub index_id: Option<src_hash::ObjectId>,
            ...
            pub checked_out_head_id: Option<src_hash::ObjectId>,
