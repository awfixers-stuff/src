...
pub use src_filter as plumbing;
use src_object::Find;
...
pub mod pipeline {
    ...
    pub mod convert_to_git {
        ...
        pub enum Error {
            ...
            #[error(transparent)]
            Convert(#[from] src_filter::pipeline::convert::to_git::Error),
        }
    }
    ...
    pub mod convert_to_worktree {
        ...
        pub enum Error {
            ...
            WorktreeCacheAtPath(#[from] std::io::Error),
            #[error(transparent)]
            Convert(#[from] src_filter::pipeline::convert::to_worktree::Error),
        }
    }
...
pub struct Pipeline<'repo> {
    inner: src_filter::Pipeline,
    cache: src_worktree::Stack,
...
impl<'repo> Pipeline<'repo> {
    ...
    pub fn options(repo: &'repo Repository) -> Result<src_filter::pipeline::Options, pipeline::options::Error> {
        ...
        let safe_crlf = config
            ...
            .with_lenient_default_value(
                ...
                src_filter::pipeline::CrlfRoundTripCheck::Fail,
            )?;
        ...
        Ok(src_filter::pipeline::Options {
            drivers,
            eol_config: src_filter::eol::Configuration { auto_crlf, eol },
    ...
    pub fn new(repo: &'repo Repository, cache: src_worktree::Stack) -> Result<Self, pipeline::options::Error> {
        let pipeline = src_filter::Pipeline::new(repo.command_context()?, Self::options(repo)?);
        Ok(Pipeline {
            inner: pipeline,
            cache,
            repo,
    ...
    pub fn into_parts(self) -> (src_filter::Pipeline, src_worktree::Stack) {
...
impl Pipeline<'_> {
    ...
    ) -> Result<src_filter::pipeline::convert::ToGitOutcome<'_, R>, pipeline::convert_to_git::Error>
    where
        R: std::io::Read,
    {
        ...
        Ok(self.inner.convert_to_git(
            ...
            &mut |buf| -> Result<_, src_object::find::Error> {
                let entry = match index
                    .entry_by_path(src_path::to_unix_separators_on_windows(src_path::into_bstr(rela_path)).as_ref())
                {
                    None => return Ok(None),
                    Some(entry) => entry,
                };
                let obj = self.repo.objects.try_find(&entry.id, buf)?;
                Ok(obj.filter(|obj| obj.kind == src_object::Kind::Blob).map(|_| ()))
            },
        )?)
    }
    ...
    pub fn convert_to_worktree<'input>(
        ...
        src: &'input [u8],
        rela_path: &BStr,
        can_delay: src_filter::driver::apply::Delay,
    ) -> Result<src_filter::pipeline::convert::ToWorktreeOutcome<'input, '_>, pipeline::convert_to_worktree::Error>
    {
    ...
    pub fn worktree_file_to_object(
        ...
        Option<(src_hash::ObjectId, src_object::tree::EntryKind, std::fs::Metadata)>,
        pipeline::worktree_file_to_object::Error,
    > {
        use pipeline::worktree_file_to_object::Error;

        let rela_path_as_path = src_path::from_bstr(rela_path);
        let repo = self.repo;
        ...
        let md = match std::fs::symlink_metadata(&path) {
            Ok(md) => md,
            Err(err) => {
                if src_fs::io_err::is_not_found(err.kind(), err.raw_os_error()) {
                    return Ok(None);
                } else {
        ...
        let (id, kind) = if md.is_symlink() {
            ...
            let id = repo.write_blob(src_path::into_bstr(target).as_ref())?;
            (id, src_object::tree::EntryKind::Link)
        } else if md.is_file() {
            use src_filter::pipeline::convert::ToGitOutcome;
            ...
            let kind = if src_fs::is_executable(&md) {
                src_object::tree::EntryKind::BlobExecutable
            } else {
                src_object::tree::EntryKind::Blob
            };
            (id, kind)
        } else if md.is_dir() {
            ...
            (id.detach().attach(repo), src_object::tree::EntryKind::Commit)
        } else {
    ...
    pub fn driver_context_mut(&mut self) -> &mut src_filter::pipeline::Context {
...
fn extract_drivers(repo: &Repository) -> Result<Vec<src_filter::Driver>, pipeline::options::Error> {
    repo.config
        .resolved
        ...
        .filter_map(|s| {
            s.header().subsection_name().map(|name| {
                Ok(src_filter::Driver {
                    ...
                    required: s
                        .value("required")
                        .map(|value| src_config::Boolean::try_from(value.as_ref()))
                        .transpose()
