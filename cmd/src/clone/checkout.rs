...
pub mod main_worktree {
    ...
    pub enum Error {
        ...
        #[error(transparent)]
        WriteIndex(#[from] src_index::file::write::Error),
        #[error(transparent)]
        CheckoutOptions(#[from] crate::config::checkout_options::Error),
        #[error(transparent)]
        IndexCheckout(#[from] src_worktree_state::checkout::Error),
        #[error(transparent)]
    ...
    impl From<ProgressId> for src_features::progress::Id {
    ...
    impl PrepareCheckout {
        ...
        ) -> Result<(Repository, src_worktree_state::checkout::Outcome), Error>
        where
            P: src_features::progress::NestedProgress,
            P::SubProgress: src_features::progress::NestedProgress + 'static,
        {
        ...
        fn main_worktree_inner(
            &mut self,
            progress: &mut dyn src_features::progress::DynNestedProgress,
            should_interrupt: &AtomicBool,
        ) -> Result<(Repository, src_worktree_state::checkout::Outcome), Error> {
            let _span = src_trace::coarse!("src::clone::PrepareCheckout::main_worktree()");
            let repo = self
                .repo
                .as_ref()
            ...
            let root_tree = match root_tree_id {
                ...
                None => {
                    return Ok((
                        ...
                        src_worktree_state::checkout::Outcome::default(),
                    ));
                }
            };

            let index = src_index::State::from_tree(&root_tree, &repo.objects, repo.config.protect_options()?)
                .map_err(|err| Error::IndexFromTree {
                    id: root_tree,
                    source: err,
                })?;
            let mut index = src_index::File::from_state(index, repo.index_path());

            let mut opts = repo.checkout_options(src_worktree::stack::state::attributes::Source::IdMapping)?;
            ...
            let outcome = src_worktree_state::checkout(
                &mut index,
                workdir,
