...
pub fn write_remote_to_local_config_file(
    ...
    remote_name: BString,
) -> Result<src_config::File<'static>, Error> {
    let mut config = src_config::File::new(local_config_meta(remote.repo));
...
fn local_config_meta(repo: &Repository) -> src_config::file::Metadata {
    let meta = repo.config.resolved.meta().clone();
    assert_eq!(
        meta.source,
        src_config::Source::Local,
        "local path is the default for new sections"
    );
    meta
}

fn write_to_local_config(config: &src_config::File<'static>, mode: WriteMode) -> std::io::Result<()> {
    assert_eq!(
        config.meta().source,
        src_config::Source::Local,
    ...
    config.write_to_filter(&mut local_config, |s| s.meta().source == src_config::Source::Local)
}

pub fn append_config_to_repo_config(repo: &mut Repository, config: src_config::File<'static>) {
    let repo_config = src_features::threading::OwnShared::make_mut(&mut repo.config.resolved);
...
pub fn update_head(
    ...
    let head_info = match ref_name {
        ...
        None => ref_map.remote_refs.iter().find_map(|r| {
            Some(match r {
                src_protocol::handshake::Ref::Symbolic {
                    full_ref_name,
                    target,
                    tag: _,
                    object,
                } if full_ref_name == "HEAD" => (Some(object.as_ref()), Some(target.as_bstr())),
                src_protocol::handshake::Ref::Direct { full_ref_name, object } if full_ref_name == "HEAD" => {
                    (Some(object.as_ref()), None)
                }
                src_protocol::handshake::Ref::Unborn { full_ref_name, target } if full_ref_name == "HEAD" => {
    ...
    let head: src_ref::FullName = "HEAD".try_into().expect("valid");
    ...
    match head_ref {
        Some(referent) => {
            let referent: src_ref::FullName = referent.try_into().map_err(|err| Error::InvalidHeadRef {
                head_ref_name: referent.to_owned(),
                source: err,
            })?;
            repo.refs
                .transaction()
                .packed_refs(src_ref::file::transaction::PackedRefs::DeletionsAndNonSymbolicUpdates(
                    Box::new(&repo.objects),
                ))
                .prepare(
                    {
                        let mut edits = vec![RefEdit {
                            change: src_ref::transaction::Change::Update {
                                log: reflog_message(),
                    ...
                    src_lock::acquire::Fail::Immediately,
                    src_lock::acquire::Fail::Immediately,
                )
...
pub(super) fn find_custom_refname<'a>(
...
) -> Result<(Option<&'a src_hash::oid>, Option<&'a BStr>), Error> {
    let group = src_refspec::MatchGroup::from_fetch_specs(Some(
        src_refspec::parse(ref_name.as_ref().as_bstr(), src_refspec::parse::Operation::Fetch)
    ...
    let filtered_items: Vec<_> = ref_map
        ...
        .map(|(full_ref_name, target)| src_refspec::match_group::Item {
            full_ref_name,
            target,
            object: None,
        })
        ...
        _ => Err(Error::RefNameAmbiguous {
            ...
            candidates: res
                ...
                .filter_map(|m| match m.lhs {
                    src_refspec::match_group::SourceRef::FullName(name) => Some(name.into_owned()),
                    src_refspec::match_group::SourceRef::ObjectId(_) => None,
                })
                .collect(),
        }),
    }
...
fn setup_branch_config(
    repo: &mut Repository,
    branch: &FullNameRef,
    branch_id: Option<&src_hash::oid>,
    remote_name: &BStr,
) -> Result<(), Error> {
    let short_name = match branch.category_and_short_name() {
        Some((src_ref::Category::LocalBranch, shortened)) => match shortened.to_str() {
            Ok(s) => s,
            Err(_) => return Ok(()),
        },
        _ => return Ok(()),
    ...
    let group = src_refspec::MatchGroup::from_fetch_specs(remote.fetch_specs.iter().map(src_refspec::RefSpec::to_ref));
    let null = src_hash::ObjectId::null(repo.object_hash());
    let res = group.match_lhs(
        Some(src_refspec::match_group::Item {
