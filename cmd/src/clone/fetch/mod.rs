...
pub enum Error {
    ...
    LoadConfig(#[from] src_config::file::init::from_paths::Error),
    ...
    InvalidHeadRef {
        source: src_validate::reference::name::Error,
    ...
    RefNameMissing { wanted: src_ref::PartialName },
    ...
    #[error(transparent)]
    ReferenceName(#[from] src_validate::reference::name::Error),
}
...
impl PrepareFetch {
        ...
        P::SubProgress: 'static,
    {
        ...
        if !self.config_overrides.is_empty() {
            ...
            snapshot.append_config(&self.config_overrides, src_config::Source::Api)?;
        }
            ...
            if let Some(ref_name) = &self.ref_name {
                ...
                let refmap = connection
                    .ref_map_by_ref(
                        ...
                        remote::ref_map::Options {
                            extra_refspecs: vec![src_refspec::parse(
                                "HEAD".into(),
                                src_refspec::parse::Operation::Fetch,
                            )
                            .expect("valid")
                            .to_owned()],
                            ..Default::default()
                ...
                let target = refmap
                    ...
                    .find_map(|r| match r {
                        ...
                        | src_protocol::handshake::Ref::Unborn {
                            full_ref_name, target, ..
                        } if full_ref_name == "HEAD" => src_ref::FullName::try_from(target)
        ...
        let head_refspec = src_refspec::parse(
        ...
        let pending_pack: remote::fetch::Prepare<'_, '_, _> = {
            ...
            match connection.prepare_fetch(&mut progress, fetch_opts.clone()).await {
                ...
                Err(remote::fetch::prepare::Error::RefMap(remote::ref_map::Error::InitRefMap(
                    src_protocol::fetch::refmap::init::Error::MappingValidation(err),
                ))) if err.issues.len() == 1
                    && fetch_opts.extra_refspecs.contains(&head_refspec)
                    && matches!(
                        err.issues.first(),
                        Some(src_refspec::match_group::validate::Issue::Conflict {
