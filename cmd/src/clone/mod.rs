...
pub struct PrepareFetch {
    ...
    #[cfg_attr(not(feature = "blocking-network-client"), allow(dead_code))]
    ref_name: Option<src_ref::PartialName>,
}
...
impl PrepareFetch {
    ...
    where
        Url: TryInto<src_url::Url, Error = E>,
        src_url::parse::Error: From<E>,
    {
        Self::new_inner(
            url.try_into().map_err(src_url::parse::Error::from)?,
            path.as_ref(),
            kind,
            create_opts,
            open_opts,
...
pub struct PrepareCheckout {
    ...
    pub(self) ref_name: Option<src_ref::PartialName>,
}
