...
impl<'repo> Id<'repo> {
    ...
    /// Find the [`header`][src_odb::find::Header] associated with this object id, or an error if it doesn't exist.
    ///
    /// Use this method if there is no interest in the contents of the object, which generally is much faster to obtain.
    pub fn header(&self) -> Result<src_odb::find::Header, find::existing::Error> {
    ...
    /// Find the [`header`][src_odb::find::Header] associated with this object id, or return `None` if it doesn't exist.
    ///
    /// Use this method if there is no interest in the contents of the object, which generally is much faster to obtain.
    pub fn try_header(&self) -> Result<Option<src_odb::find::Header>, find::Error> {
    ...
    pub fn shorten(&self) -> Result<src_hash::Prefix, shorten::Error> {
        let hex_len = self.repo.config.hex_len.map_or_else(
            || self.repo.objects.packed_object_count().map(calculate_auto_hex_len),
            Ok,
        )?;

        let prefix = src_odb::store::prefix::disambiguate::Candidate::new(self.inner, hex_len)
    ...
    pub fn shorten_or_id(&self) -> src_hash::Prefix {
...
pub mod shorten {
    ...
    pub enum Error {
        #[error(transparent)]
        PackedObjectsCount(#[from] src_odb::store::load_index::Error),
        #[error(transparent)]
        DisambiguatePrefix(#[from] src_odb::store::prefix::disambiguate::Error),
        #[error("Id could not be shortened as the object with id {} could not be found", .oid)]
        NotFound { oid: src_hash::ObjectId },
    }
}
