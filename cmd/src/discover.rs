...
impl ThreadSafeRepository {
    ...
    pub fn discover_opts(
        ...
        trust_map: src_sec::trust::Mapping<crate::open::Options>,
    ) -> Result<Self, Error> {
        let _span = src_trace::coarse!("ThreadSafeRepository::discover()");
        ...
        options.current_dir = Some(src_fs::current_dir(false).map_err(upwards::Error::CurrentDir)?);
    ...
    /// ### Note
    ///
    /// Consider to set [`match_ceiling_dir_or_error = false`](src_discover::upwards::Options::match_ceiling_dir_or_error)
    ...
    pub fn discover_with_environment_overrides_opts(
        ...
        fn apply_additional_environment(mut opts: upwards::Options<'_>) -> upwards::Options<'_> {
            ...
            {
                if let Ok(b) = src_config::Boolean::try_from(cross_fs.as_ref()) {
