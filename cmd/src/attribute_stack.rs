...
use src_fs::stack::ToNormalPathComponents;
...
impl<'repo> AttributeStack<'repo> {
    ...
    pub fn new(stack: src_worktree::Stack, repo: &'repo Repository) -> Self {
        AttributeStack { repo, inner: stack }
    }

    /// Detach the repository and return the underlying plumbing datatype.
    pub fn detach(self) -> src_worktree::Stack {
        self.inner
    }
}

impl Deref for AttributeStack<'_> {
    type Target = src_worktree::Stack;
...
impl AttributeStack<'_> {
    ...
    pub fn at_path(
        ...
        mode: Option<src_index::entry::Mode>,
    ) -> std::io::Result<src_worktree::stack::Platform<'_>> {
