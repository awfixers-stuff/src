...
pub fn args_os_opt(precompose_unicode: bool) -> impl Iterator<Item = OsString> {
    std::env::args_os().map(move |arg| {
        if precompose_unicode {
            src_utils::str::precompose_os_string(arg.into()).into_owned()
        } else {
            arg
        }
    })
...
/// ### Usage
///
/// The caller may define a function that specifies the result type as `Result<T, src::env::collate::{operation}::Error>` to collect
            ...
            pub fn is_corrupted(&self) -> bool {
                match self {
                    ...
                    Error::Fetch(
                        ...
                        | crate::remote::fetch::Error::Fetch(src_protocol::fetch::Error::Negotiate(_)),
                    ) => true,
                    _ => false,
                }
            }
