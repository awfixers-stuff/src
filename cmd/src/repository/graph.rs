...
impl crate::Repository {
    ...
    /// ### Performance
    ///
    /// Note that the [Graph][src_revwalk::Graph] can be sensitive to various object database settings that may affect the performance
    /// of the commit walk.
    pub fn revision_graph<'cache, T>(
        &self,
        cache: Option<&'cache src_commitgraph::Graph>,
    ) -> src_revwalk::Graph<'_, 'cache, T> {
        src_revwalk::Graph::new(&self.objects, cache)
    }
    ...
    pub fn commit_graph(&self) -> Result<src_commitgraph::Graph, Error> {
        src_commitgraph::at(self.objects.store_ref().path().join("info")).map_err(Into::into)
    }

    /// Return a newly opened commit-graph if it is available *and* enabled in the Git configuration.
    pub fn commit_graph_if_enabled(
        &self,
    ) -> Result<Option<src_commitgraph::Graph>, super::commit_graph_if_enabled::Error> {
        Ok(self
            .config
            .may_use_commit_graph()?
            .then(|| src_commitgraph::at(self.objects.store_ref().path().join("info")))
            .transpose()
