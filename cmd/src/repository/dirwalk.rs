...
impl Repository {
    ...
    /// lookup. Note that items will only count as tracked if they have the [`src_index::entry::Flags::UPTODATE`]
    /// flag set.
    ///
    ...
    ///
    /// See [`src_dir::walk::delegate::Collect`] for a delegate that collects all seen entries.
    pub fn dirwalk(
        &self,
        ...
        delegate: &mut dyn src_dir::walk::Delegate,
    ) -> Result<dirwalk::Outcome<'_>, dirwalk::Error> {
        let _span = src_trace::coarse!("src::dirwalk");
        ...
        let mut opts = src_dir::walk::Options::from(options);
        ...
        if let Some(workdir) = self.workdir().filter(|_| opts.for_deletion.is_some()) {
            ...
            if !linked_worktrees.is_empty() {
                let real_workdir = src_path::realpath_opts(
                    workdir,
                ...
                worktree_relative_worktree_dirs_storage = linked_worktrees
                    ...
                    .map(|rela_path| {
                        src_path::to_unix_separators_on_windows(src_path::into_bstr(rela_path)).into_owned()
                    })
                    .collect();
                opts.worktree_relative_worktree_dirs = Some(&worktree_relative_worktree_dirs_storage);
            }
        }
        let (outcome, traversal_root) = src_dir::walk(
            workdir,
            src_dir::walk::Context {
