...
impl crate::Repository {
    ...
    pub fn open_mailmap(&self) -> src_mailmap::Snapshot {
        let mut out = src_mailmap::Snapshot::default();
    ...
    pub fn open_mailmap_into(&self, target: &mut src_mailmap::Snapshot) -> Result<(), crate::mailmap::load::Error> {
        ...
        match self.workdir() {
            ...
            Some(root) => {
                if let Ok(mut file) = src_features::fs::open_options_no_follow()
                    .read(true)
                    ...
                    })
                {
                    ...
                    std::io::copy(&mut file, &mut buf)
                        .map_err(|e| err.get_or_insert(e.into()))
                        .ok();
                    target.merge(src_mailmap::parse_ignore_errors(&buf));
                }
            }
        }

        if let Some(blob) = blob_id.and_then(|id| self.find_object(id).map_err(|e| err.get_or_insert(e.into())).ok()) {
            target.merge(src_mailmap::parse_ignore_errors(&blob.data));
        }
