...
impl crate::Repository {
    ...
    pub fn worktree_proxy_by_id<'a>(&self, id: impl Into<&'a BStr>) -> Option<worktree::Proxy<'_>> {
        worktree::Proxy::new_if_gitdir_file_exists(
            self,
            self.common_dir().join("worktrees").join(src_path::from_bstr(id.into())),
        )
    }
    ...
    pub fn worktree_stream(
        &self,
        id: impl Into<src_hash::ObjectId>,
    ) -> Result<(src_worktree_stream::Stream, src_index::File), crate::repository::worktree_stream::Error> {
        use src_odb::HeaderExt;
        let id = id.into();
        ...
        let mut cache = self
            .attributes_only(&index, src_worktree::stack::state::attributes::Source::IdMapping)?
            .detach();
        let pipeline = src_filter::Pipeline::new(self.command_context()?, crate::filter::Pipeline::options(self)?);
        let objects = self.objects.clone().into_arc().expect("TBD error handling");
        let stream = src_worktree_stream::from_tree(
            id,
            objects.clone(),
            pipeline,
    ...
    pub fn worktree_archive(
        &self,
        mut stream: src_worktree_stream::Stream,
        out: impl std::io::Write + std::io::Seek,
        blobs: impl src_features::progress::Count,
        should_interrupt: &std::sync::atomic::AtomicBool,
        options: src_archive::Options,
    ) -> Result<(), crate::repository::worktree_archive::Error> {
        let mut out = src_features::interrupt::Write {
            inner: out,
            should_interrupt,
        };
        if options.format == src_archive::Format::InternalTransientNonPersistable {
            std::io::copy(&mut stream.into_read(), &mut out)
                .or_raise(|| src_error::message("Could not copy stream"))?;
            return Ok(());
        }
        src_archive::write_stream_seek(
            &mut stream,
            |stream| {
                if should_interrupt.load(std::sync::atomic::Ordering::Relaxed) {
                    return Err(src_error::ErrorExt::raise_erased(src_error::message(
                        "Cancelled by user",
                    )));
                }
