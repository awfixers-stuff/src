    ...
    #[cfg(feature = "max-performance-safe")]
    {
        match config.pack_cache_bytes {
            None => match config.static_pack_cache_limit_bytes {
                None => objects.set_pack_cache(|| Box::<src_pack::cache::lru::StaticLinkedList<64>>::default()),
                Some(limit) => {
                    objects.set_pack_cache(move || Box::new(src_pack::cache::lru::StaticLinkedList::<64>::new(limit)));
                }
            },
            Some(0) => objects.unset_pack_cache(),
            Some(bytes) => objects.set_pack_cache(move || -> Box<src_odb::cache::PackCache> {
                Box::new(src_pack::cache::lru::MemoryCappedHashmap::new(bytes))
            }),
        }
        if config.object_cache_bytes == 0 {
            ...
            objects.set_object_cache(move || Box::new(src_pack::cache::object::MemoryCappedHashmap::new(bytes)));
        }
    }
}
