...
use src_merge::blob::builtin_driver::text;
use src_object::Write;
...
impl Repository {
    ...
    pub fn merge_resource_cache(
        &self,
        worktree_roots: src_merge::blob::pipeline::WorktreeRoots,
    ) -> Result<src_merge::blob::Platform, merge_resource_cache::Error> {
        ...
        let mode = {
            let renormalize = self
                ...
                .unwrap_or_default();
            if renormalize {
                src_merge::blob::pipeline::Mode::Renormalize
            } else {
                src_merge::blob::pipeline::Mode::ToGit
            }
        };
        let attrs = self
            .attributes_only(
                ...
                if worktree_roots.is_unset() {
                    src_worktree::stack::state::attributes::Source::IdMapping
                } else {
                    src_worktree::stack::state::attributes::Source::WorktreeThenIdMapping
                },
            )?
            .inner;
        let filter = src_filter::Pipeline::new(self.command_context()?, crate::filter::Pipeline::options(self)?);
        let filter = src_merge::blob::Pipeline::new(worktree_roots, filter, self.config.merge_pipeline_options()?);
        let options = src_merge::blob::platform::Options {
            default_driver: self.config.resolved.string(&tree::Merge::DEFAULT).map(Cow::into_owned),
        };
        let drivers = self.config.merge_drivers()?;
        Ok(src_merge::blob::Platform::new(filter, mode, attrs, drivers, options))
    }

    /// Return options for use with [`src_merge::blob::PlatformRef::merge()`], accessible through
    /// [merge_resource_cache()](Self::merge_resource_cache).
    pub fn blob_merge_options(&self) -> Result<src_merge::blob::platform::merge::Options, blob_merge_options::Error> {
        Ok(src_merge::blob::platform::merge::Options {
            is_virtual_ancestor: false,
            resolve_binary_with: None,
            text: src_merge::blob::builtin_driver::text::Options {
    ...
    ///
    /// Note that `ancestor_tree` can be the [empty tree hash](src_hash::ObjectId::empty_tree) to indicate no common ancestry.
    ///
    ...
    pub fn merge_trees(
        &self,
        ancestor_tree: impl AsRef<src_hash::oid>,
        our_tree: impl AsRef<src_hash::oid>,
        their_tree: impl AsRef<src_hash::oid>,
        labels: src_merge::blob::builtin_driver::text::Labels<'_>,
        ...
        let src_merge::tree::Outcome {
            ...
            labels,
            self,
            |buf| self.write_buf(src_object::Kind::Blob, buf),
            &mut Default::default(),
            &mut diff_cache,
            &mut blob_merge,
            options.into(),
    ...
    pub fn merge_commits(
        &self,
        our_commit: impl Into<src_hash::ObjectId>,
        their_commit: impl Into<src_hash::ObjectId>,
        labels: src_merge::blob::builtin_driver::text::Labels<'_>,
        ...
        let src_merge::commit::Outcome {
            tree_merge:
                src_merge::tree::Outcome {
                    tree,
                    conflicts,
                    failed_on_first_unresolved_conflict,
                },
    ...
    pub fn virtual_merge_base(
        &self,
        merge_bases: impl IntoIterator<Item = impl Into<src_hash::ObjectId>>,
        options: crate::merge::tree::Options,
    ...
    pub fn virtual_merge_base_with_graph(
        &self,
        merge_bases: impl IntoIterator<Item = impl Into<src_hash::ObjectId>>,
        graph: &mut src_revwalk::Graph<'_, '_, src_revwalk::graph::Commit<src_revision::merge_base::Flags>>,
        options: crate::merge::tree::Options,
        ...
        let src_merge::commit::virtual_merge_base::Outcome {
            virtual_merge_bases,
            commit_id,
            tree_id,
        } = src_merge::commit::virtual_merge_base(
            first,
            second,
            merge_bases,
            graph,
