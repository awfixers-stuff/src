...
use src_object::{Exists, Find, FindExt, Write};
use src_odb::{Header, HeaderExt};
use src_ref::{
...
impl crate::Repository {
    ...
    pub fn find_header(&self, id: impl Into<ObjectId>) -> Result<src_odb::find::Header, object::find::existing::Error> {
        let id = id.into();
        if id == ObjectId::empty_tree(self.object_hash()) {
            return Ok(src_odb::find::Header::Loose {
                kind: src_object::Kind::Tree,
                size: 0,
            });
        }
    ...
    /// an unsuitable [RefreshMode](src_odb::store::RefreshMode) and `id` is not likely to exist.
    /// Use [`repo.objects.refresh_never()`](src_odb::store::Handle::refresh_never) to avoid expensive
    /// IO-bound refreshes if an object wasn't found.
    #[doc(alias = "exists", alias = "git2")]
    pub fn has_object(&self, id: impl AsRef<src_hash::oid>) -> bool {
        let id = id.as_ref();
    ...
    pub fn try_find_header(
        &self,
        id: impl Into<ObjectId>,
    ) -> Result<Option<src_odb::find::Header>, object::find::Error> {
        let id = id.into();
        if id == ObjectId::empty_tree(self.object_hash()) {
            return Ok(Some(src_odb::find::Header::Loose {
                kind: src_object::Kind::Tree,
                size: 0,
            }));
        }
...
impl crate::Repository {
    ...
    pub fn write_object(&self, object: impl src_object::WriteTo) -> Result<Id<'_>, object::write::Error> {
    ...
    fn write_object_inner(&self, buf: &[u8], kind: src_object::Kind) -> Result<Id<'_>, object::write::Error> {
        let oid = src_object::compute_hash(self.object_hash(), kind, buf)
    ...
    pub fn write_blob(&self, bytes: impl AsRef<[u8]>) -> Result<Id<'_>, object::write::Error> {
        ...
        let oid = src_object::compute_hash(self.object_hash(), src_object::Kind::Blob, bytes)
            .map_err(|err| Box::new(err) as Box<dyn std::error::Error + Send + Sync>)?;
        if self.objects.exists(&oid) {
            return Ok(oid.attach(self));
        }
        self.objects
            .write_buf(src_object::Kind::Blob, bytes)
            .map_err(Into::into)
            .map(|oid| oid.attach(self))
    }
    ...
    fn write_blob_stream_inner(&self, buf: &[u8]) -> Result<Id<'_>, object::write::Error> {
        let oid = src_object::compute_hash(self.object_hash(), src_object::Kind::Blob, buf)
            .map_err(|err| Box::new(err) as Box<dyn std::error::Error + Send + Sync>)?;
        if self.objects.exists(&oid) {
            return Ok(oid.attach(self));
        }
        ...
        self.objects
            .write_buf(src_object::Kind::Blob, buf)
            .map_err(Into::into)
            .map(|oid| oid.attach(self))
    }
}
...
impl crate::Repository {
    ...
    pub fn tag(
        &self,
        name: impl AsRef<str>,
        target: impl AsRef<src_hash::oid>,
        target_kind: src_object::Kind,
        tagger: Option<src_actor::SignatureRef<'_>>,
        message: impl AsRef<str>,
        constraint: PreviousValue,
    ) -> Result<Reference<'_>, tag::Error> {
        let tag = src_object::Tag {
    ...
    pub fn commit_as<'a, 'c, Name, E>(
        &self,
        committer: impl Into<src_actor::SignatureRef<'c>>,
        author: impl Into<src_actor::SignatureRef<'a>>,
        reference: Name,
        message: impl AsRef<str>,
    ...
    fn commit_as_inner(
        &self,
        committer: src_actor::SignatureRef<'_>,
        author: src_actor::SignatureRef<'_>,
        reference: FullName,
        message: &str,
        tree: ObjectId,
