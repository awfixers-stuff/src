...
impl Repository {
    ...
    pub fn attributes(
        ...
        ignore_source: src_worktree::stack::state::ignore::Source,
        exclude_overrides: Option<src_ignore::Search>,
    ) -> Result<AttributeStack<'_>, Error> {
        let case = if self.config.ignore_case {
            src_glob::pattern::Case::Fold
        } else {
            src_glob::pattern::Case::Sensitive
        };
        ...
        let state = src_worktree::stack::State::AttributesAndIgnoreStack { attributes, ignore };
        let attribute_list = state.id_mappings_from_index(index, index.path_backing(), case);
        Ok(AttributeStack::new(
            src_worktree::Stack::new(
    ...
    pub fn attributes_only(
        ...
        let state = src_worktree::stack::State::AttributesStack(attributes);
        let attribute_list = state.id_mappings_from_index(index, index.path_backing(), case);
        Ok(AttributeStack::new(
            src_worktree::Stack::new(
                // this is alright as we don't cause mutation of that directory, it's virtual.
    ...
    pub fn excludes(
        ...
        let state = src_worktree::stack::State::IgnoreStack(ignore);
        let attribute_list = state.id_mappings_from_index(index, index.path_backing(), case);
        Ok(AttributeStack::new(
            src_worktree::Stack::new(
                // this is alright as we don't cause mutation of that directory, it's virtual.
