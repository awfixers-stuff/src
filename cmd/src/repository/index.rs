...
impl crate::Repository {
    ...
    pub fn open_index(&self) -> Result<src_index::File, worktree::open_index::Error> {
        ...
        let index = src_index::File::at(
            self.index_path(),
            self.object_hash(),
            skip_hash,
            src_index::decode::Options {
                thread_limit,
    ...
    pub fn index(&self) -> Result<worktree::Index, worktree::open_index::Error> {
        self.try_index().and_then(|opt| match opt {
            ...
            None => Err(worktree::open_index::Error::IndexFile(
                src_index::file::init::Error::Io(std::io::Error::new(
    ...
    pub fn index_or_empty(&self) -> Result<worktree::Index, worktree::open_index::Error> {
        Ok(self.try_index()?.unwrap_or_else(|| {
            worktree::Index::new(src_fs::FileSnapshot::new(src_index::File::from_state(
        ...
        self.index.recent_snapshot(
            ...
            || {
                self.open_index().map(Some).or_else(|err| match err {
                    worktree::open_index::Error::IndexFile(src_index::file::init::Error::Io(err))
    ...
    pub fn index_or_load_from_head_or_empty(
        ...
        Ok(match self.try_index()? {
            ...
            None => match self.head()?.id() {
                ...
                None => IndexPersistedOrInMemory::InMemory(src_index::File::from_state(
                    src_index::State::new(self.object_hash()),
                    self.index_path(),
                )),
            },
    ...
    pub fn index_from_tree(&self, tree: &src_hash::oid) -> Result<src_index::File, super::index_from_tree::Error> {
        Ok(src_index::File::from_state(
            src_index::State::from_tree(tree, self, self.config.protect_options()?).map_err(|err| {
...
impl IndexPersistedOrInMemory {
    ...
    pub fn into_owned(self) -> src_index::File {
        match self {
            IndexPersistedOrInMemory::Persisted(i) => src_index::File::clone(&i),
            IndexPersistedOrInMemory::InMemory(i) => i,
        }
    }
}
