...
impl crate::Repository {
    ...
    #[cfg(feature = "revision")]
    pub fn rev_parse<'a>(&self, spec: impl Into<&'a BStr>) -> Result<revision::Spec<'_>, src_error::Error> {
    ...
    pub fn merge_base(
        &self,
        one: impl Into<src_hash::ObjectId>,
        two: impl Into<src_hash::ObjectId>,
        ...
        let bases = src_revision::merge_base(one, &[two], &mut graph)?.ok_or(super::merge_base::Error::NotFound {
            first: one,
            second: two,
        })?;
    ...
    pub fn merge_base_with_graph(
        ...
        let two = two.into();
        let bases =
            src_revision::merge_base(one, &[two], graph)?.ok_or(super::merge_base_with_graph::Error::NotFound {
                first: one,
                second: two,
            })?;
        Ok(bases.first().attach(self))
    ...
    pub fn merge_bases_many_with_graph(
        &self,
        one: impl Into<src_hash::ObjectId>,
        others: &[src_hash::ObjectId],
        graph: &mut src_revwalk::Graph<'_, '_, src_revwalk::graph::Commit<src_revision::merge_base::Flags>>,
    ) -> Result<Vec<Id<'_>>, src_revision::merge_base::Error> {
        use crate::prelude::ObjectIdExt;
        let one = one.into();
        Ok(match src_revision::merge_base(one, others, graph)? {
            Some(bases) => bases.into_iter().map(|id| id.attach(self)).collect(),
            None => Vec::new(),
        })
    }

    /// Like [`merge_bases_many_with_graph()`](Self::merge_bases_many_with_graph), but without the ability to speed up consecutive calls with a [graph](src_revwalk::Graph).
    ///
    /// # Performance
    ///
    ...
    pub fn merge_base_octopus_with_graph(
        &self,
        commits: impl IntoIterator<Item = impl Into<src_hash::ObjectId>>,
        graph: &mut src_revwalk::Graph<'_, '_, src_revwalk::graph::Commit<src_revision::merge_base::Flags>>,
        ...
        src_revision::merge_base::octopus(first, &commits[1..], graph)?
    ...
    pub fn rev_walk(
        &self,
        tips: impl IntoIterator<Item = impl Into<src_hash::ObjectId>>,
