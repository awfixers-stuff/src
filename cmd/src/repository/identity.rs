...
impl crate::Repository {
    ...
    pub fn committer(&self) -> Option<Result<src_actor::SignatureRef<'_>, config::time::Error>> {
        let p = self.config.personas();

        Ok(src_actor::SignatureRef {
    ...
    pub fn committer_or_set_generic_fallback(&mut self) -> Result<src_actor::SignatureRef<'_>, config::time::Error> {
        if self.committer().is_none() {
            let mut config = src_config::File::new(src_config::file::Metadata::api());
            config
    ...
    ///
    /// The values are cached when the repository is instantiated.
    pub fn author(&self) -> Option<Result<src_actor::SignatureRef<'_>, config::time::Error>> {
        let p = self.config.personas();
...
impl Personas {
    pub fn from_config_and_env(config: &src_config::File<'_>) -> Self {
            ...
            config
                ...
                .and_then(|config_date| {
                    config_date
                        .to_str()
                        .ok()
                        .and_then(|date| src_date::parse(date, Some(SystemTime::now())).ok())
                })
                .or_else(|| Some(src_date::Time::now_local_or_utc()))
                .map(|time| time.format_or_unix(src_date::time::Format::Raw))
        };
