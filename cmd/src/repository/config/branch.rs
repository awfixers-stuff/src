...
use src_ref::{FullName, FullNameRef};
...
impl crate::Repository {
    ...
    pub fn upstream_branch_and_remote_for_tracking_branch(
        ...
        if tracking_branch.category() != Some(src_ref::Category::RemoteBranch) {
        ...
        let item_to_search = src_refspec::match_group::Item {
        ...
        for remote_name in self.remote_names() {
            ...
            let match_group = src_refspec::MatchGroup::from_fetch_specs(
                remote
        ...
        if candidates.len() == 1 {
            ...
            let upstream_branch = match candidate {
                src_refspec::match_group::SourceRef::FullName(name) => src_ref::FullName::try_from(name.into_owned())?,
                src_refspec::match_group::SourceRef::ObjectId(_) => {
    ...
    pub fn branch_remote<'a>(
        ...
        self.try_find_remote(name.as_bstr())
            .map(|res| res.map_err(Into::into))
            .or_else(|| match name {
                remote::Name::Url(url) => src_url::parse(url.as_ref())
                    .map_err(Into::into)
                    .and_then(|url| {
                        self.remote_at(url)
...
fn matching_remote<'a>(
    lhs: &FullNameRef,
    specs: impl IntoIterator<Item = &'a src_refspec::RefSpec>,
    object_hash: src_hash::Kind,
) -> Option<Result<Cow<'static, FullNameRef>, src_validate::reference::name::Error>> {
    let search = src_refspec::MatchGroup {
        specs: specs
            .into_iter()
            .map(src_refspec::RefSpec::to_ref)
