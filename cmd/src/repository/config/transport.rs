...
impl crate::Repository {
    ...
    /// [configure()][src_transport::client::TransportWithoutIO::configure()] (via `&**config` to pass the contained `Any` and not the `Box`).
    ...
    pub fn transport_options<'a>(
        ...
        let url = src_url::parse(url.into())?;
        use src_url::Scheme::*;
        ...
        match &url.scheme {
            Http | Https => {
                ...
                ))]
                {
                    ...
                    use src_transport::client::blocking_io::http::{
                        self,
                    ...
                    fn ssl_version(
                        ...
                        mut filter: fn(&src_config::file::Metadata) -> bool,
                        lenient: bool,
                    ...
                    opts.proxy_authenticate = opts
                        ...
                        .map(|url| src_url::parse(url.into()))
                        .transpose()?
                    ...
                    #[cfg(feature = "blocking-http-transport-curl")]
                    {
                        ...
                        let schannel_check_revoke = config
                            ...
                            .map_err(config::transport::http::Error::from)?;
                        let backend =
                            src_protocol::transport::client::blocking_io::http::curl::Options { schannel_check_revoke };
                        opts.backend =
