use src_path::realpath::MAX_SYMLINKS;
use std::borrow::Cow;
...
impl crate::Repository {
    ...
    pub fn git_dir_trust(&self) -> src_sec::Trust {
    ...
    pub fn workdir_path(&self, rela_path: impl AsRef<BStr>) -> Option<PathBuf> {
        self.workdir().and_then(|wd| {
            src_path::try_from_bstr(rela_path.as_ref())
                .ok()
    ...
    pub fn prefix(&self) -> Result<Option<&Path>, src_path::realpath::Error> {
        let (root, current_dir) = match self.workdir().zip(self.options.current_dir.as_deref()) {
            Some((work_dir, cwd)) => (work_dir, cwd),
            None => return Ok(None),
        };

        let root = src_path::realpath_opts(root, current_dir, MAX_SYMLINKS)?;
    ...
    pub fn kind(&self) -> crate::repository::Kind {
        use src_discover::path::RepositoryKind::*;
        match src_discover::path::repository_kind(self.git_dir()) {
    ...
    /// Returns `Some(true)` if the reference database [is untouched](src_ref::file::Store::is_pristine()).
    ...
    pub fn is_pristine(&self) -> Option<bool> {
        ...
        let default_branch_ref_name: src_ref::FullName = format!("refs/heads/{name}")
            .try_into()
            .unwrap_or_else(|_| src_ref::FullName::try_from("refs/heads/master").expect("known to be valid"));
