...
pub struct ThreadSafeRepository {
    ...
    pub objects: src_features::threading::OwnShared<src_odb::Store>,
...
pub struct Remote<'repo> {
    ...
    pub(crate) url: Option<src_url::Url>,
    /// The rewritten `url`, if it was rewritten.
    pub(crate) url_alias: Option<src_url::Url>,
    /// The url to use for pushing specifically.
    pub(crate) push_url: Option<src_url::Url>,
    /// The rewritten `push_url`, if it was rewritten.
    pub(crate) push_url_alias: Option<src_url::Url>,
    /// Refspecs for use when fetching.
    pub(crate) fetch_specs: Vec<src_refspec::RefSpec>,
    /// Refspecs for use when pushing.
    pub(crate) push_specs: Vec<src_refspec::RefSpec>,
...
pub struct Pathspec<'repo> {
    ...
    pub(crate) stack: Option<src_worktree::Stack>,
    /// The prepared search to use for checking matches.
    pub(crate) search: src_pathspec::Search,
}
...
pub struct PathspecDetached {
    /// The cache to power attribute access. It's only initialized if we have a pattern with attributes.
    pub stack: Option<src_worktree::Stack>,
    /// The prepared search to use for checking matches.
    pub search: src_pathspec::Search,
...
pub struct AttributeStack<'repo> {
    /// The owning repository.
    pub repo: &'repo Repository,
    pub(crate) inner: src_worktree::Stack,
}
