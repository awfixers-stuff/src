...
use src_object::FindExt;
use src_traverse::commit::simple::CommitTimeOrder;
...
pub enum Error {
    #[error(transparent)]
    SimpleTraversal(#[from] src_traverse::commit::simple::Error),
    #[error(transparent)]
...
pub enum Sorting {
    ...
    ByCommitTimeCutoff {
        ...
        seconds: src_date::SecondsSinceUnixEpoch,
    },
}

impl Sorting {
    fn into_simple(self) -> Option<src_traverse::commit::simple::Sorting> {
        Some(match self {
            Sorting::BreadthFirst => src_traverse::commit::simple::Sorting::BreadthFirst,
            Sorting::ByCommitTime(order) => src_traverse::commit::simple::Sorting::ByCommitTime(order),
            Sorting::ByCommitTimeCutoff { seconds, order } => {
                src_traverse::commit::simple::Sorting::ByCommitTimeCutoff { order, seconds }
            }
        })
    }
}
...
pub struct Info<'repo> {
    ...
    pub id: src_hash::ObjectId,
    /// All parent ids we have encountered. Note that these will be at most one if [`Parents::First`][src_traverse::commit::Parents::First] is enabled.
    pub parent_ids: src_traverse::commit::ParentIds,
    ...
    pub commit_time: Option<src_date::SecondsSinceUnixEpoch>,
...
impl<'repo> Info<'repo> {
    ...
    pub fn commit_time(&self) -> src_date::SecondsSinceUnixEpoch {
...
impl<'repo> Info<'repo> {
    ...
    pub fn new(info: src_traverse::commit::Info, repo: &'repo Repository) -> Self {
        Info {
            id: info.id,
    ...
    pub fn detach(self) -> src_traverse::commit::Info {
        src_traverse::commit::Info {
            id: self.id,
...
pub struct Platform<'repo> {
    ...
    pub(crate) parents: src_traverse::commit::Parents,
    pub(crate) use_commit_graph: Option<bool>,
    pub(crate) commit_graph: Option<src_commitgraph::Graph>,
}
...
impl Platform<'_> {
    ...
    pub fn first_parent_only(mut self) -> Self {
        self.parents = src_traverse::commit::Parents::First;
        self
    }
    ...
    pub fn with_commit_graph(mut self, graph: Option<src_commitgraph::Graph>) -> Self {
...
impl<'repo> Platform<'repo> {
    ...
    pub fn selected(
        self,
        mut filter: impl FnMut(&src_hash::oid) -> bool + 'repo,
        ...
        Ok(revision::Walk {
            repo,
            inner: Box::new(
                src_traverse::commit::Simple::filtered(tips, &repo.objects, {
...
pub(crate) mod iter_impl {
    ...
    pub struct Walk<'repo> {
        ...
        pub(crate) inner: Box<dyn Iterator<Item = Result<src_traverse::commit::Info, super::iter::Error>> + 'repo>,
    }
