        ...
        match self {
            ...
            CandidateInfo::Commit { date, title } => {
                write!(
                    f,
                    "commit {} {title:?}",
                    src_date::parse_header(date)
                        .unwrap_or_default()
                        .format_or_unix(src_date::time::format::SHORT)
                )
            }
        }
    }
...
pub(crate) fn ambiguous(candidates: Vec<ObjectId>, prefix: src_hash::Prefix, repo: &Repository) -> src_error::Message {
            ...
            .map(|oid| {
                ...
                let order = match &obj {
                    ...
                    Ok(obj) => match obj.kind {
                        src_object::Kind::Tag => Order::Tag,
                        src_object::Kind::Commit => Order::Commit,
                        src_object::Kind::Tree => Order::Tree,
                        src_object::Kind::Blob => Order::Blob,
                    },
                };
                (oid, obj, order)
            })
    ...
    let info: Vec<_> = candidates
        ...
        .map(|(oid, find_result, _)| {
            let info = match find_result {
                Ok(obj) => match obj.kind {
                    src_object::Kind::Tree | src_object::Kind::Blob => CandidateInfo::Object { kind: obj.kind },
                    src_object::Kind::Tag => {
