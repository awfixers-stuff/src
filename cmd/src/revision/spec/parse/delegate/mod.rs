...
use src_error::{message, ErrorExt, Exn, ResultExt};
use src_hash::ObjectId;
use src_revision::spec::{parse, parse::delegate};
...
impl<'repo> Delegate<'repo> {
    ...
    pub fn into_rev_spec(mut self) -> Result<crate::revision::Spec<'repo>, src_error::Error> {
        fn zero_or_one_objects_or_ambiguity_err(
            mut candidates: [Option<Vec<ObjectId>>; 2],
            prefix: [Option<src_hash::Prefix>; 2],
            repo: &Repository,
        ) -> Result<[Option<ObjectId>; 2], src_error::Error> {
        ...
        fn kind_to_spec(
            kind: Option<src_revision::spec::Kind>,
            [first, second]: [Option<ObjectId>; 2],
        ) -> Result<src_revision::Spec, src_error::Error> {
            pub fn malformed() -> src_error::Error {
            ...
            Ok(match kind.unwrap_or_default() {
                IncludeReachable => src_revision::Spec::Include(first.ok_or_else(malformed)?),
                ExcludeReachable => src_revision::Spec::Exclude(first.ok_or_else(malformed)?),
                RangeBetween => src_revision::Spec::Range {
                    from: first.ok_or_else(malformed)?,
                    to: second.ok_or_else(malformed)?,
                },
                ReachableToMergeBase => src_revision::Spec::Merge {
                    theirs: first.ok_or_else(malformed)?,
                    ours: second.ok_or_else(malformed)?,
                },
                IncludeReachableFromParents => src_revision::Spec::IncludeOnlyParents(first.ok_or_else(malformed)?),
                ExcludeReachableFromParents => src_revision::Spec::ExcludeParents(first.ok_or_else(malformed)?),
            })
        }
...
impl delegate::Kind for Delegate<'_> {
    fn kind(&mut self, kind: src_revision::spec::Kind) -> Result<(), Exn> {
        use src_revision::spec::Kind::*;
        self.kind = Some(kind);
...
impl Delegate<'_> {
    ...
    fn kind_implies_committish(&self) -> bool {
        self.kind.unwrap_or(src_revision::spec::Kind::IncludeReachable) != src_revision::spec::Kind::IncludeReachable
    }

    fn disambiguate_objects_by_fallback_hint_delay_errors(&mut self, hint: Option<ObjectKindHint>) {
        fn require_object_kind(repo: &Repository, obj: &src_hash::oid, kind: src_object::Kind) -> Result<(), Exn> {
                    ...
                    Some(kind_hint) => match kind_hint {
                        ObjectKindHint::Treeish | ObjectKindHint::Committish => {
                            let kind = match kind_hint {
                                ObjectKindHint::Treeish => src_object::Kind::Tree,
                                ObjectKindHint::Committish => src_object::Kind::Commit,
                        ...
                        ObjectKindHint::Tree | ObjectKindHint::Commit | ObjectKindHint::Blob => {
                            let kind = match kind_hint {
                                ObjectKindHint::Tree => src_object::Kind::Tree,
                                ObjectKindHint::Commit => src_object::Kind::Commit,
                                ObjectKindHint::Blob => src_object::Kind::Blob,
                                _ => unreachable!("BUG: we narrow possibilities above"),
                            };
                            objs.iter()
...
fn peel(repo: &Repository, obj: &src_hash::oid, kind: src_object::Kind) -> Result<ObjectId, Exn> {
