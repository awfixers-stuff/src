use src_error::{bail, message, ErrorExt, Exn, OptionExt, ResultExt};
use src_hash::ObjectId;
use src_index::entry::Stage;
use src_revision::spec::parse::{
    delegate,
...
impl delegate::Navigate for Delegate<'_> {
        ...
        match kind {
            ...
            PeelTo::Path(path) => {
                let lookup_path = |obj: &ObjectId| {
                    let tree_id = peel(repo, obj, src_object::Kind::Tree)?;
                    if path.is_empty() {
                        return Ok::<_, Exn>((tree_id, src_object::tree::EntryKind::Tree.into()));
                    }
                    let mut tree = repo.find_object(tree_id).or_erased()?.into_tree();
                    let entry = tree
                        .peel_to_entry_by_path(src_path::from_bstr(path))
                        .or_erased()?
                        .ok_or_raise_erased(|| {
                            message!(
    ...
    fn index_lookup(&mut self, path: &BStr, stage: u8) -> Result<(), Exn> {
        let stage = match stage {
            ...
            3 => Stage::Theirs,
            _ => unreachable!(
                "BUG: driver will not pass invalid stages (and it uses integer to avoid src-index as dependency)"
            ),
        };
        self.unset_disambiguate_call();
            ...
            Some(entry) => {
                ...
                self.paths[self.idx] = Some((
                    ...
                    entry
                        .mode
                        .to_tree_entry_mode()
                        .unwrap_or(src_object::tree::EntryKind::Blob.into()),
                ));
                Ok(())
            }
            None => {
                ...
                let exists = self
                    .repo
                    .workdir()
                    .is_some_and(|root| root.join(src_path::from_bstr(path)).exists());
                Err(message!(
