use src_error::{bail, message, ErrorExt, Exn, ResultExt};
...
impl delegate::Revision for Delegate<'_> {
    ...
    fn disambiguate_prefix(
        ...
        let empty_tree_id = src_hash::ObjectId::empty_tree(prefix.as_oid().kind());
    ...
    fn nth_checked_out_branch(&mut self, branch_no: usize) -> Result<(), Exn> {
        ...
        fn prior_checkouts_iter<'a>(
            platform: &'a mut src_ref::file::log::iter::Platform<'static, '_>,
        ) -> Result<impl Iterator<Item = (BString, ObjectId)> + 'a, src_error::Error> {
            match platform.rev().ok().flatten() {
