...
impl<'repo> Spec<'repo> {
    ...
    pub fn from_bstr<'a>(
        ...
        opts: Options,
    ) -> Result<Self, src_error::Error> {
        let mut delegate = Delegate::new(repo, opts);
        match src_revision::spec::parse(spec.into(), &mut delegate) {
            Err(mut err) => {
...
struct Delegate<'repo> {
    refs: [Option<src_ref::Reference>; 2],
    ...
    paths: [Option<(BString, src_object::tree::EntryMode)>; 2],
