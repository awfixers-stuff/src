...
impl<'repo> Spec<'repo> {
    ...
    pub fn from_id(id: Id<'repo>) -> Self {
        Spec {
            inner: src_revision::Spec::Include(id.inner),
            path: None,
            repo: id.repo,
            first_ref: None,
            second_ref: None,
...
impl<'repo> Spec<'repo> {
    ...
    pub fn detach(self) -> src_revision::Spec {
        self.inner
    }
    ...
    pub fn path_and_mode(&self) -> Option<(&BStr, src_object::tree::EntryMode)> {
    ...
    pub fn first_reference(&self) -> Option<&src_ref::Reference> {
    ...
    /// yields `None`.
    pub fn second_reference(&self) -> Option<&src_ref::Reference> {
    ...
    pub fn single(&self) -> Option<Id<'repo>> {
        match self.inner {
            src_revision::Spec::Include(id) | src_revision::Spec::ExcludeParents(id) => {
                Id::from_id(id, self.repo).into()
            }
            src_revision::Spec::Exclude(_)
            | src_revision::Spec::Range { .. }
            | src_revision::Spec::Merge { .. }
            | src_revision::Spec::IncludeOnlyParents { .. } => None,
        }
    }
}
