...
pub mod init {
    ...
    pub enum Error {
        ...
        #[error(transparent)]
        ParseSpec(#[from] src_pathspec::parse::Error),
        #[error(
            "Could not obtain the repository prefix as the relative path of the CWD as seen from the working tree"
        )]
        NormalizeSpec(#[from] src_pathspec::normalize::Error),
        #[error(transparent)]
        RepoPrefix(#[from] src_path::realpath::Error),
    }
}
...
impl<'repo> Pathspec<'repo> {
    ...
    pub fn new(
        ...
        make_attributes: impl FnOnce() -> Result<src_worktree::Stack, Box<dyn std::error::Error + Send + Sync + 'static>>,
        ...
        src_trace::debug!(
            ...
            patterns = ?search.patterns().map(src_pathspec::Pattern::path).collect::<Vec<_>>()
        );
...
impl<'repo> Pathspec<'repo> {
    ...
    pub fn pattern_matching_relative_path<'a>(
        ...
        is_dir: Option<bool>,
    ) -> Option<src_pathspec::search::Match<'_>> {
    ...
    pub fn index_entries_with_paths<'s: 'repo, 'a: 'repo>(
        &'s mut self,
        index: &'a src_index::State,
    ) -> Option<impl Iterator<Item = (&'a BStr, &'a src_index::Entry)> + 'repo + 's> {
...
pub(crate) fn is_dir_to_mode(is_dir: bool) -> src_index::entry::Mode {
    if is_dir {
