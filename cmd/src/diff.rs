use src_diff::tree::recorder::Location;
pub use src_diff::*;
...
pub struct Options {
    ...
    #[cfg(feature = "blob-diff")]
    rewrites: Option<src_diff::Rewrites>,
}
...
#[cfg(feature = "blob-diff")]
impl From<Options> for src_diff::tree_with_rewrites::Options {
    fn from(opts: Options) -> Self {
        src_diff::tree_with_rewrites::Options {
...
impl Options {
    ...
    #[cfg(feature = "blob-diff")]
    pub fn track_rewrites(&mut self, renames: Option<src_diff::Rewrites>) -> &mut Self {
...
impl Options {
    ...
    /// if nothing is configured, which turns rename tracking with 50% similarity on, while not tracking copies at all.
    #[cfg(feature = "blob-diff")]
    pub fn with_rewrites(mut self, renames: Option<src_diff::Rewrites>) -> Self {
        self.rewrites = renames;
        self
    }
}
...
#[cfg(feature = "blob-diff")]
pub(crate) mod utils {
    use src_diff::{rewrites::Copies, Rewrites};
    ...
    pub fn resource_cache(
        repo: &Repository,
        mode: src_diff::blob::pipeline::Mode,
        attr_stack: src_worktree::Stack,
        roots: src_diff::blob::pipeline::WorktreeRoots,
    ) -> Result<src_diff::blob::Platform, resource_cache::Error> {
        let diff_algo = repo.config.diff_algorithm()?;
        let diff_cache = src_diff::blob::Platform::new(
            src_diff::blob::platform::Options {
                algorithm: Some(diff_algo),
                skip_internal_diff_if_external_is_configured: false,
            },
            src_diff::blob::Pipeline::new(
                roots,
