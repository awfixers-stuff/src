...
pub mod diff {
    ...
    use src_diff::blob::platform::prepare_diff::Operation;
    ...
    pub struct Platform<'a> {
        ...
        pub resource_cache: &'a mut src_diff::blob::Platform,
    }

    ///
    pub mod init {
        /// The error returned by [`object::tree::diff::Change::diff`](crate::object::tree::diff::Change::diff()).
        pub type Error = src_diff::blob::platform::set_resource::Error;
    }
    ...
    pub mod lines {
            ...
            E: std::error::Error + Send + Sync + 'static,
        {
            ...
            ProcessHunk(E),
            #[error(transparent)]
            PrepareDiff(#[from] src_diff::blob::platform::prepare_diff::Error),
        }
    ...
    impl Platform<'_> {
        ...
        ) -> Result<src_diff::blob::platform::prepare_diff::Outcome<'_>, lines::Error<E>>
        where
            ...
            E: std::error::Error + Send + Sync + 'static,
        {
            ...
            match prep.operation {
                Operation::InternalDiff { algorithm } => {
                    ...
                    src_diff::blob::diff(algorithm, &input, |before: Range<u32>, after: Range<u32>| {
                        if err.is_some() {
                            return;
                        }
                        lines.clear();
        ...
        pub fn line_counts(
            &mut self,
        ) -> Result<Option<src_diff::blob::sink::Counter<()>>, src_diff::blob::platform::prepare_diff::Error> {
            self.resource_cache.options.skip_internal_diff_if_external_is_configured = false;
            ...
            match prep.operation {
                Operation::InternalDiff { algorithm } => {
                    ...
                    let counter = src_diff::blob::diff(algorithm, &tokens, src_diff::blob::sink::Counter::default());
                    Ok(Some(counter))
                }
                Operation::ExternalCommand { .. } => {
                    unreachable!("we disabled that")
