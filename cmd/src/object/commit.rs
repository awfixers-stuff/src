...
mod error {
    ...
    pub enum Error {
        ...
        Decode(#[from] src_object::decode::Error),
        #[error("The commit date could not be parsed")]
        ParseDate(#[from] src_date::Error),
        #[error("Expected object of type {}, but got {}", .expected, .actual)]
        ObjectKind {
            expected: src_object::Kind,
            actual: src_object::Kind,
        },
    }
}
...
impl<'repo> Commit<'repo> {
    ...
    pub fn short_id(&self) -> Result<src_hash::Prefix, crate::id::shorten::Error> {
        use crate::ext::ObjectIdExt;
        self.id.attach(self.repo).shorten()
    }

    /// Parse the commits message into a [`MessageRef`][src_object::commit::MessageRef]
    pub fn message(&self) -> Result<src_object::commit::MessageRef<'_>, src_object::decode::Error> {
        Ok(src_object::commit::MessageRef::from_bytes(self.message_raw()?))
    }
    /// Decode the commit object until the message and return it.
    pub fn message_raw(&self) -> Result<&'_ BStr, src_object::decode::Error> {
        src_object::CommitRefIter::from_bytes(&self.data).message()
    }
    ...
    pub fn time(&self) -> Result<src_date::Time, Error> {
    ...
    /// more than once.
    pub fn decode(&self) -> Result<src_object::CommitRef<'_>, src_object::decode::Error> {
        src_object::CommitRef::from_bytes(&self.data)
    }

    /// Return an iterator over tokens, representing this commit piece by piece.
    pub fn iter(&self) -> src_object::CommitRefIter<'_> {
        src_object::CommitRefIter::from_bytes(&self.data)
    }

    /// Return the commits author, with surrounding whitespace trimmed.
    pub fn author(&self) -> Result<src_actor::SignatureRef<'_>, src_object::decode::Error> {
        src_object::CommitRefIter::from_bytes(&self.data)
            .author()
            .map(|s| s.trim())
    }
    ...
    pub fn committer(&self) -> Result<src_actor::SignatureRef<'_>, src_object::decode::Error> {
        src_object::CommitRefIter::from_bytes(&self.data)
            .committer()
            .map(|s| s.trim())
    }
    ...
    pub fn tree_id(&self) -> Result<crate::Id<'repo>, src_object::decode::Error> {
        src_object::CommitRefIter::from_bytes(&self.data)
            .tree_id()
    ...
    pub fn signature(
        &self,
    ) -> Result<Option<(std::borrow::Cow<'_, BStr>, src_object::commit::SignedData<'_>)>, src_object::decode::Error>
    {
        src_object::CommitRefIter::signature(&self.data)
    }
}
