...
pub use src_object::Kind;

use crate::{Blob, Commit, Id, Object, ObjectDetached, Tag, Tree};

mod errors;
pub(crate) mod cache {
    pub use src_pack::cache::object::MemoryCappedHashmap;
}
...
pub mod try_into {
    ...
    pub struct Error {
        pub actual: src_object::Kind,
        pub expected: src_object::Kind,
...
impl<'repo> Object<'repo> {
    ...
    pub fn try_into_commit(self) -> Result<Commit<'repo>, try_into::Error> {
        self.try_into().map_err(|this: Self| try_into::Error {
            id: this.id,
            actual: this.kind,
            expected: src_object::Kind::Commit,
        })
    }
...
impl<'repo> Object<'repo> {
    ...
    pub fn to_commit_ref(&self) -> src_object::CommitRef<'_> {
        self.try_to_commit_ref().expect("BUG: need a commit")
    }

    /// Obtain a fully parsed commit whose fields reference our data buffer.
    pub fn try_to_commit_ref(&self) -> Result<src_object::CommitRef<'_>, conversion::Error> {
        src_object::Data::new(self.kind, &self.data)
            .decode()?
            .into_commit()
    ...
    ///
    /// - this object is not a commit
    pub fn to_commit_ref_iter(&self) -> src_object::CommitRefIter<'_> {
        src_object::Data::new(self.kind, &self.data)
    ...
    pub fn try_to_commit_ref_iter(&self) -> Option<src_object::CommitRefIter<'_>> {
        src_object::Data::new(self.kind, &self.data).try_into_commit_iter()
    }
    ...
    pub fn to_tag_ref_iter(&self) -> src_object::TagRefIter<'_> {
        src_object::Data::new(self.kind, &self.data)
            .try_into_tag_iter()
    ...
    ///
    /// - this object is not a tag
    pub fn try_to_tag_ref_iter(&self) -> Option<src_object::TagRefIter<'_>> {
        src_object::Data::new(self.kind, &self.data).try_into_tag_iter()
    }
    ...
    pub fn to_tag_ref(&self) -> src_object::TagRef<'_> {
        self.try_to_tag_ref().expect("BUG: need tag")
    }

    /// Obtain a fully parsed tag object whose fields reference our data buffer.
    pub fn try_to_tag_ref(&self) -> Result<src_object::TagRef<'_>, conversion::Error> {
        src_object::Data::new(self.kind, &self.data)
            .decode()?
            .into_tag()
            .ok_or(conversion::Error::UnexpectedType {
                expected: src_object::Kind::Tag,
                actual: self.kind,
            })
    }
