...
pub mod init {
    ...
    pub enum Error {
        #[error(transparent)]
        DecodeTree(#[from] src_object::decode::Error),
        #[error(transparent)]
...
pub mod write {
    ...
    pub enum Error {
        ...
        MissingObject {
            filename: BString,
            kind: src_object::tree::EntryKind,
        ...
        InvalidFilename {
            ...
            kind: src_object::tree::EntryKind,
            id: src_hash::ObjectId,
            source: src_validate::path::component::Error,
        },
    }
}
...
pub struct Cursor<'a, 'repo> {
    inner: src_object::tree::editor::Cursor<'a, 'repo>,
    validate: src_validate::path::component::Options,
...
impl<'repo> super::Editor<'repo> {
    ...
    pub fn new(tree: &crate::Tree<'repo>) -> Result<Self, init::Error> {
        ...
        Ok(super::Editor {
            inner: src_object::tree::Editor::new(tree_ref.into(), &repo.objects, repo.object_hash()),
            validate,
            repo,
        })
    }
    ...
    pub fn detach(self) -> src_object::tree::Editor<'repo> {
        self.inner
    }
}
...
impl<'repo> super::Editor<'repo> {
    ...
    ) -> Result<Cursor<'_, 'repo>, src_object::tree::editor::Error> {
        Ok(Cursor {
...
impl<'repo> Cursor<'_, 'repo> {
    ...
    ) -> Result<&mut Self, src_object::tree::editor::Error> {
    ...
    pub fn remove(&mut self, rela_path: impl ToComponents) -> Result<&mut Self, src_object::tree::editor::Error> {
    ...
    cursor
        ...
        .write(|tree| -> Result<ObjectId, write::Error> {
            for entry in &tree.entries {
                src_validate::path::component(
                    entry.filename.as_ref(),
                    entry
                        .mode
                        .is_link()
                        .then_some(src_validate::path::component::Mode::Symlink),
                    cursor.validate,
                )
