...
impl Change<'_, '_, '_> {
    ...
    ///
    /// `resource_cache` only grows, so one should call [`src_diff::blob::Platform::clear_resource_cache`] occasionally.
    pub fn diff<'b>(
        &self,
        resource_cache: &'b mut src_diff::blob::Platform,
...
impl<'a> From<Change<'a, '_, '_>> for src_diff::tree_with_rewrites::ChangeRef<'a> {
    fn from(value: Change<'a, '_, '_>) -> Self {
        use src_diff::tree_with_rewrites::ChangeRef;
        match value {
            Change::Addition {
                location,
                entry_mode,
...
impl<'a, 'old, 'new> Change<'a, 'old, 'new> {
    ...
    pub fn from_change_ref(
        change: src_diff::tree_with_rewrites::ChangeRef<'a>,
...
impl crate::ext::TreeDiffChangeExt for src_diff::tree_with_rewrites::Change {
...
impl Change<'_, '_, '_> {
    ...
    pub fn entry_mode(&self) -> src_object::tree::EntryMode {
        match self {
