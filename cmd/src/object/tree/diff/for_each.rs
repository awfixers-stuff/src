...
pub enum Error {
    #[error(transparent)]
    Diff(#[from] src_diff::tree_with_rewrites::Error),
...
impl<'old> Platform<'_, 'old> {
    ...
    ) -> Result<Option<src_diff::rewrites::Outcome>, Error>
    where
    ...
    /// Note that it's recommended to call [`src_diff::blob::Platform::clear_resource_cache()`] between the calls
    ...
    /// a conversion pipeline that uses [`src_diff::blob::pipeline::Mode::ToGit`].
    pub fn for_each_to_obtain_tree_with_cache<'new, E>(
        &mut self,
        other: &Tree<'new>,
        resource_cache: &mut src_diff::blob::Platform,
        for_each: impl FnMut(Change<'_, 'old, 'new>) -> Result<Action, E>,
    ) -> Result<Option<src_diff::rewrites::Outcome>, Error>
    where
        E: Into<Box<dyn std::error::Error + Sync + Send + 'static>>,
    ...
    fn for_each_to_obtain_tree_inner<'new, E>(
        ...
        resource_cache: Option<&mut src_diff::blob::Platform>,
    ) -> Result<Option<src_diff::rewrites::Outcome>, Error>
    where
        E: Into<Box<dyn std::error::Error + Sync + Send + 'static>>,
    {
        ...
        let cache = match resource_cache {
            None => {
                storage = repo.diff_resource_cache(src_diff::blob::pipeline::Mode::ToGit, Default::default())?;
                &mut storage
            }
            Some(cache) => cache,
        };
        let opts = self.options.into();
        Ok(src_diff::tree_with_rewrites(
