...
pub use src_object::tree::{EntryKind, EntryMode};
use src_object::{bstr::BStr, FindExt, TreeRefIter};
...
pub struct Editor<'repo> {
    pub(crate) inner: src_object::tree::Editor<'repo>,
    pub(crate) validate: src_validate::path::component::Options,
...
impl<'repo> Tree<'repo> {
    ...
    pub fn decode(&self) -> Result<src_object::TreeRef<'_>, src_object::decode::Error> {
        src_object::TreeRef::from_bytes(&self.data)
    }
    ...
    pub fn lookup_entry_by_path(
        ...
        self.lookup_entry(relative_path.as_ref().components().map(|c: std::path::Component<'_>| {
            src_path::os_str_into_bstr(c.as_os_str())
...
mod iter {
    ...
    impl<'repo> Tree<'repo> {
        ...
        pub fn iter(&self) -> impl Iterator<Item = Result<EntryRef<'repo, '_>, src_object::decode::Error>> {
            let repo = self.repo;
            src_object::TreeRefIter::from_bytes(&self.data).map(move |e| e.map(|entry| EntryRef { inner: entry, repo }))
        }
    }
}
...
pub struct EntryRef<'repo, 'a> {
    ...
    pub inner: src_object::tree::EntryRef<'a>,
    /// The owning repository.
...
pub struct Entry<'repo> {
    pub(crate) inner: src_object::tree::Entry,
    /// The owning repository.
    pub repo: &'repo crate::Repository,
}
...
mod _impls {
    ...
    impl TryFrom<Tree<'_>> for src_object::Tree {
        type Error = src_object::decode::Error;
