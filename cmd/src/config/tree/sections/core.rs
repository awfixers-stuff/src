...
impl Core {
    ...
    pub const DELTA_BASE_CACHE_LIMIT: keys::UnsignedInteger =
        keys::UnsignedInteger::new_unsigned_integer("deltaBaseCacheLimit", &config::Tree::CORE)
            .with_environment_override("src_PACK_CACHE_MEMORY")
    ...
    pub const PRECOMPOSE_UNICODE: keys::Boolean = keys::Boolean::new_boolean("precomposeUnicode", &config::Tree::CORE)
        .with_note("application needs to conform all program input by using src::env::args_os()");
...
mod filter {
    ...
    mod check_round_trip_encoding {
        ...
        impl CheckRoundTripEncoding {
            ...
            ) -> Result<Vec<&'static src_filter::encoding::Encoding>, config::encoding::Error> {
                Ok(match value {
                    None => vec![src_filter::encoding::SHIFT_JIS],
                    Some(value) => {
                        let mut out = Vec::new();
                        ...
                        {
                            out.push(
                                src_filter::encoding::Encoding::for_label(encoding.trim()).ok_or_else(|| {
    ...
    mod eol {
        ...
        impl Eol {
            ...
            pub fn try_into_eol(
                &'static self,
                value: Cow<'_, BStr>,
            ) -> Result<src_filter::eol::Mode, config::key::GenericErrorWithValue> {
    ...
    mod safecrlf {
        ...
        use src_filter::pipeline::CrlfRoundTripCheck;
        ...
        impl SafeCrlf {
            ...
            pub fn try_into_safecrlf(
                ...
                let value = src_config::Boolean::try_from(value.as_ref()).map_err(|err| {
...
mod log_all_ref_updates {
    ...
    impl LogAllRefUpdates {
        ...
        pub fn try_into_ref_updates(
            &'static self,
            value: Option<Result<bool, src_config::value::Error>>,
        ) -> Result<Option<src_ref::store::WriteReflog>, config::key::GenericErrorWithValue> {
            match value {
                Some(Ok(bool)) => Ok(Some(if bool {
                    src_ref::store::WriteReflog::Normal
                } else {
                    src_ref::store::WriteReflog::Disable
                })),
                Some(Err(err)) => match err.input {
                    val if val.eq_ignore_ascii_case(b"always") => Ok(Some(src_ref::store::WriteReflog::Always)),
    ...
    impl Abbrev {
        ...
        pub fn try_into_abbreviation(
            ...
            if hex_len_str.trim().eq_ignore_ascii_case(b"auto") {
                ...
                if let Ok(false) = src_config::Boolean::try_from(value_bytes).map(Into::into) {
                    Ok(object_hash.len_in_hex().into())
                } else {
                    let value = src_config::Integer::try_from(value_bytes)
                        .map_err(|_| Error {
...
mod validate {
    ...
    impl keys::Validate for LogAllRefUpdates {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            super::Core::LOG_ALL_REF_UPDATES
                .try_into_ref_updates(Some(src_config::Boolean::try_from(value).map(|b| b.0)))?;
            Ok(())
        }
    }
    ...
    impl keys::Validate for Abbrev {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            ...
            super::Core::ABBREV.try_into_abbreviation(value.into(), src_hash::Kind::Sha1)?;
            Ok(())
        }
    }
