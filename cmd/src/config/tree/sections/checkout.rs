...
pub mod validate {
    ...
    impl keys::Validate for Workers {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            super::Checkout::WORKERS.try_from_workers(src_config::Integer::try_from(value).and_then(|i| {
                i.to_decimal()
                    .ok_or_else(|| src_config::value::Error::new("Integer overflow", value.to_owned()))
            }))?;
            Ok(())
        }
    }
