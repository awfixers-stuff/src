...
mod algorithm {
    ...
    impl Ignore {
        ...
        pub fn try_into_ignore(
            &'static self,
            value: Cow<'_, BStr>,
        ) -> Result<src_submodule::config::Ignore, key::GenericErrorWithValue> {
            src_submodule::config::Ignore::try_from(value.as_ref())
    ...
    impl Algorithm {
        ...
        pub fn try_into_algorithm(&self, name: Cow<'_, BStr>) -> Result<src_diff::blob::Algorithm, algorithm::Error> {
            let algo = if name.eq_ignore_ascii_case(b"myers") || name.eq_ignore_ascii_case(b"default") {
                src_diff::blob::Algorithm::Myers
            } else if name.eq_ignore_ascii_case(b"minimal") {
                src_diff::blob::Algorithm::MyersMinimal
            } else if name.eq_ignore_ascii_case(b"histogram") {
                src_diff::blob::Algorithm::Histogram
...
mod binary {
    ...
    impl Binary {
        ...
        /// with [`src_config::file::section::Body::value_implicit()`].
            ...
            Ok(match value {
                ...
                Some(value) => {
                    if value.as_ref() == "auto" {
                        ...
                        Some(
                            src_config::Boolean::try_from(value.as_ref())
                                .map(|b| b.0)
                                .map_err(|err| {
...
pub(super) mod validate {
    ...
    impl keys::Validate for Ignore {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            src_submodule::config::Ignore::try_from(value)
    ...
    impl keys::Validate for Renames {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            let boolean = src_config::Boolean::try_from(value).map(|b| b.0);
