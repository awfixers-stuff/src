...
impl<T: Validate> Any<T> {
    ...
    pub fn try_into_refspec(
        ...
        op: src_refspec::parse::Operation,
    ) -> Result<src_refspec::RefSpec, config::refspec::Error> {
        src_refspec::parse(value.as_ref(), op)
...
impl<T: Validate> src_config::AsKey for Any<T> {
    fn as_key(&self) -> src_config::KeyRef<'_> {
...
/// one can run it with [command::prepare()](src_command::prepare), possibly after
/// [obtaining](crate::Repository::command_context) and [setting](src_command::Prepare::with_context)
/// a git [command context](src_command::Context) (depending on the commands needs).
...
mod duration {
    ...
    impl DurationInMilliseconds {
        ...
        pub fn try_into_duration(
            &'static self,
            value: Result<i64, src_config::value::Error>,
...
mod lock_timeout {
    ...
    impl LockTimeout {
        ...
        pub fn try_into_lock_timeout(
            &'static self,
            value: Result<i64, src_config::value::Error>,
        ) -> Result<src_lock::acquire::Fail, config::lock_timeout::Error> {
...
mod url {
    ...
    impl Url {
        ...
        pub fn try_into_url(&'static self, value: Cow<'_, BStr>) -> Result<src_url::Url, config::url::Error> {
            src_url::parse(value.as_ref())
...
mod time {
    ...
    impl Time {
        ...
        ) -> Result<src_date::Time, Exn<src_date::Error>> {
            src_date::parse(
                value.as_ref().to_str().map_err(|_| {
                    src_date::Error::new_with_input("UTF8 conversion failed", value.clone().into_owned())
                })?,
                now,
            )
        }
...
mod boolean {
    ...
    impl Boolean {
        ...
        ///
        /// `value` is expected to be provided by [`src_config::File::boolean()`].
        pub fn enrich_error(
            &'static self,
            value: Result<bool, src_config::value::Error>,
...
pub mod validate {
    ...
    impl Validate for Time {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
            src_date::parse(value.to_str()?, std::time::SystemTime::now().into())
                .map_err(src_error::Exn::into_inner)?;
            Ok(())
        }
    }
    ...
    impl Validate for UnsignedInteger {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
            usize::try_from(
                src_config::Integer::try_from(value)?
                    .to_decimal()
    ...
    impl Validate for Boolean {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
            src_config::Boolean::try_from(value)?;
            Ok(())
        }
    }
    ...
    impl Validate for PushRefSpec {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            src_refspec::parse(value, src_refspec::parse::Operation::Push)?;
            Ok(())
        }
    }
    ...
    impl Validate for FetchRefSpec {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            src_refspec::parse(value, src_refspec::parse::Operation::Fetch)?;
            Ok(())
        }
    }
    ...
    impl Validate for LockTimeout {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            let value = src_config::Integer::try_from(value)?
                .to_decimal()
