...
impl Cache {
    ...
    pub fn from_stage_one(
        ...
        branch_name: Option<&src_ref::FullNameRef>,
        ...
        let options = src_config::file::init::Options {
            includes: if use_includes {
                src_config::file::includes::Options::follow(
                    interpolate_context(git_install_dir, home),
                    src_config::file::includes::conditional::Context {
                        git_dir: git_dir.into(),
                        branch_name,
                    },
                )
            } else {
                src_config::file::includes::Options::no_follow()
            },
        ...
        let config = {
            ...
            let mut metas = [
                src_config::source::Kind::GitInstallation,
                src_config::source::Kind::System,
                src_config::source::Kind::Global,
            ]
            .iter()
            .flat_map(|kind| kind.sources())
            .filter_map(|source| {
                match source {
                    src_config::Source::GitInstallation if !use_installation => return None,
                    src_config::Source::System if !use_system => return None,
                    src_config::Source::Git if !use_git => return None,
                    src_config::Source::User if !use_user => return None,
                    _ => {}
                }
                source
            ...
            .map(|(source, path)| src_config::file::Metadata {
                path: Some(path),
                source: *source,
                level: 0,
                trust: src_sec::Trust::Full,
            });

            let err_on_nonexisting_paths = false;
            let mut globals = src_config::File::from_paths_metadata_buf(
                &mut metas,
                &mut buf,
                err_on_nonexisting_paths,
                src_config::file::init::Options {
                    includes: src_config::file::includes::Options::no_follow(),
                    ..options
                },
            )
            .map_err(|err| match err {
                src_config::file::init::from_paths::Error::Init(err) => Error::from(err),
                src_config::file::init::from_paths::Error::Io { source, path } => Error::Io { source, path },
            })?
            .unwrap_or_default();
            ...
            if use_env {
                globals.append(src_config::File::from_env(options)?.unwrap_or_default());
            }
            if !cli_config_overrides.is_empty() {
                config::overrides::append(&mut globals, cli_config_overrides, src_config::Source::Cli, |_| None)
                    .map_err(|err| Error::ConfigOverrides {
                        err,
                        source: src_config::Source::Cli,
                    })?;
            }
            if !api_config_overrides.is_empty() {
                config::overrides::append(&mut globals, api_config_overrides, src_config::Source::Api, |_| None)
                    .map_err(|err| Error::ConfigOverrides {
                        err,
                        source: src_config::Source::Api,
                    })?;
            }
    ...
    pub(crate) fn make_source_env(
        ...
        move |name| {
            match name {
                ...
                "HOME" => {
                    return if home.is_allowed() {
                        src_path::env::home_dir().map(Into::into)
                    } else {
                        None
                    }
                }
            ...
            .and_then(|perm| perm.check_opt(name).and_then(src_path::env::var))
        }
    }
}
...
fn apply_environment_overrides(
    config: &mut src_config::File<'static>,
    ...
    fn var_as_bstring(var: &str, perm: Permission) -> Option<BString> {
        perm.check_opt(var)
            ...
            .and_then(|val| src_path::os_string_into_bstring(val).ok())
    }

    let mut env_override = src_config::File::new(src_config::file::Metadata::from(src_config::Source::EnvOverride));
