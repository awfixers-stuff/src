...
pub(crate) struct StageOne {
    pub git_dir_config: src_config::File<'static>,
    pub buf: Vec<u8>,
...
impl StageOne {
    pub fn new(
        ...
        let object_hash = (repo_format_version != 1)
            .then_some(Ok(src_hash::Kind::Sha1))
            .or_else(|| {
                config
...
fn load_config(
    ...
    let metadata = src_config::file::Metadata::from(source)
        .at(&config_path)
        .with(git_dir_trust);
    let mut file = match std::fs::File::open(&config_path) {
        Ok(f) => f,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Ok(src_config::File::new(metadata)),
        Err(err) => {
            let err = Error::Io {
                source: err,
                path: config_path,
            ...
            if lenient {
                src_trace::warn!("ignoring: {err:#?}");
                return Ok(src_config::File::new(metadata));
            } else {
                return Err(err);
            }
        }
    ...
    let config = src_config::File::from_bytes_owned(
        buf,
        metadata,
