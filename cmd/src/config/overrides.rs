...
pub enum Error {
    ...
    SectionKey {
        key: BString,
        source: src_config::parse::section::value_name::Error,
    },
    #[error(transparent)]
    SectionHeader(#[from] src_config::parse::section::header::Error),
}
...
pub(crate) fn append(
    ...
    let mut file = src_config::File::new(src_config::file::Metadata::from(source));
    for key_value in values {
        ...
        let key = src_config::KeyRef::parse_unvalidated(key).ok_or_else(|| Error::InvalidKey { input: key.into() })?;
        let mut section = file.section_mut_or_create_new(key.section_name, key.subsection_name)?;
        let value_name = src_config::parse::section::ValueName::try_from(key.value_name.to_owned()).map_err(|err| {
            Error::SectionKey {
                source: err,
