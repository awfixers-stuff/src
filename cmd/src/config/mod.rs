pub use src_config::*;
use src_features::threading::OnceCell;
...
pub struct SnapshotMut<'repo> {
    ...
    pub(crate) config: src_config::File<'static>,
}
...
pub mod section {
    ...
    pub fn is_trusted(meta: &src_config::file::Metadata) -> bool {
        meta.trust == src_sec::Trust::Full || meta.source.kind() != src_config::source::Kind::Repository
    }
}
...
pub mod set_value {
    ...
    pub enum Error {
        #[error(transparent)]
        SetRaw(#[from] src_config::file::set_raw_value::Error),
        #[error(transparent)]
...
pub enum Error {
    ...
    #[error(transparent)]
    Init(#[from] src_config::file::init::Error),
    #[error(transparent)]
    ResolveIncludes(#[from] src_config::file::includes::Error),
    #[error(transparent)]
    FromEnv(#[from] src_config::file::init::from_env::Error),
    #[error("The path {path:?} at the 'core.worktree' configuration could not be interpolated")]
    PathInterpolation {
        path: BString,
        source: src_config::path::interpolate::Error,
    },
...
pub mod merge {
    ...
    pub mod pipeline_options {
        /// The error produced when obtaining options needed to fill in [src_merge::blob::pipeline::Options].
        #[derive(Debug, thiserror::Error)]
        #[allow(missing_docs)]
        pub enum Error {
            #[error(transparent)]
    ...
    ///
    pub mod drivers {
        /// The error produced when obtaining a list of [Drivers](src_merge::blob::Driver).
        #[derive(Debug, thiserror::Error)]
        #[allow(missing_docs)]
        pub enum Error {
            #[error(transparent)]
...
pub mod diff {
    ...
    ///
    pub mod pipeline_options {
        /// The error produced when obtaining options needed to fill in [src_diff::blob::pipeline::Options].
        #[derive(Debug, thiserror::Error)]
        #[allow(missing_docs)]
        pub enum Error {
            #[error(transparent)]
    ...
    pub mod drivers {
        ...
        /// The error produced when obtaining a list of [Drivers](src_diff::blob::Driver).
        #[derive(Debug, thiserror::Error)]
...
pub mod command_context {
    ...
    pub enum Error {
        ...
        #[error(transparent)]
        ParseBool(#[from] src_config::value::Error),
    }
}
...
pub mod exclude_stack {
    ...
    pub enum Error {
        ...
        #[error(transparent)]
        EnvironmentPermission(#[from] src_sec::permission::Error<PathBuf>),
        #[error("The value for `core.excludesFile` could not be read from configuration")]
        ExcludesFilePathInterpolation(#[from] src_config::path::interpolate::Error),
        #[error(transparent)]
...
pub mod attribute_stack {
    ...
    pub enum Error {
        ...
        AttributesFileInterpolation(#[from] src_config::path::interpolate::Error),
    }
}
...
pub mod key {
    ...
    pub type GenericError<E = src_config::value::Error> = Error<E, 'k', 'i'>;

    /// A generic key error which will also contain a value.
    pub type GenericErrorWithValue<E = src_config::value::Error> = Error<E, 'v', 'i'>;
}
...
pub mod checkout {
    ...
    pub mod workers {
        ...
        pub type Error = config::key::Error<src_config::value::Error, 'n', 'd'>;
    }
}
...
pub mod time {
    ...
    pub type Error = super::key::Error<src_date::Error, 't', 'i'>;
}

///
pub mod lock_timeout {
    /// The error produced when failing to parse timeout for locks.
    pub type Error = super::key::Error<src_config::value::Error, 'i', 'i'>;
}

///
pub mod duration {
    /// The error produced when failing to parse durations (in milliseconds).
    pub type Error = super::key::Error<src_config::value::Error, 'd', 'i'>;
}

///
pub mod boolean {
    /// The error produced when failing to parse time from configuration.
    pub type Error = super::key::Error<src_config::value::Error, 'b', 'i'>;
}

///
pub mod unsigned_integer {
    /// The error produced when failing to parse a signed integer from configuration.
    pub type Error = super::key::Error<src_config::value::Error, 'k', 'u'>;
}

///
pub mod url {
    /// The error produced when failing to parse a url from the configuration.
    pub type Error = super::key::Error<src_url::parse::Error, 'u', 'p'>;
}
...
pub mod refspec {
    ...
    pub type Error = super::key::Error<src_refspec::parse::Error, 'r', 'p'>;
}

///
pub mod refs_namespace {
    /// The error produced when failing to parse a refspec from the configuration.
    pub type Error = super::key::Error<src_validate::reference::name::Error, 'v', 'i'>;
}
...
pub mod transport {
    ...
    pub enum Error {
        ...
        ParseUrl(#[from] src_url::parse::Error),
...
pub(crate) struct Cache {
    ...
    pub reflog: Option<src_ref::store::WriteReflog>,
    /// The representation of `gitoxide.core.refsNamespace`, or `None` if the variable wasn't set.
    pub refs_namespace: Option<src_ref::Namespace>,
    ...
    #[cfg(feature = "blob-diff")]
    pub(crate) diff_algorithm: OnceCell<src_diff::blob::Algorithm>,
    ...
    filter_config_section: fn(&src_config::file::Metadata) -> bool,
