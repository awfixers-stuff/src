...
mod error {
    ...
    pub enum Error {
        ...
        CoreAskpass(#[from] src_config::path::interpolate::Error),
        #[error(transparent)]
...
impl Snapshot<'_> {
    ...
    pub fn credential_helpers(
        ...
        (
            src_credentials::helper::Cascade,
            src_credentials::helper::Action,
            src_prompt::Options<'static>,
        ),
        Error,
    > {
        let repo = self.repo;
...
pub(super) mod function {
    ...
    pub fn credential_helpers(
        mut url: src_url::Url,
        config: &src_config::File<'_>,
        is_lenient_config: bool,
        mut filter: impl FnMut(&src_config::file::Metadata) -> bool,
    ...
    > {
        ...
        if let Some(credential_sections) = config.sections_by_name_and_filter("credential", &mut filter) {
            for section in credential_sections {
                let section = match section.header().subsection_name() {
                    Some(pattern) => src_url::parse(pattern).ok().and_then(|mut pattern| {
                        normalize(&mut pattern);
                        let is_http = matches!(pattern.scheme, src_url::Scheme::Https | src_url::Scheme::Http);
                ...
                if let Some((section, helper_key, username_key, use_http_path_key)) = section {
                    for value in section.values(helper_key.name) {
                        if value.trim().is_empty() {
                            ...
                            programs.push(src_credentials::Program::from_custom_definition(value.into_owned()));
                        }
                    }
                    ...
                    if let Some(toggle) = section
                        ...
                        .map(|val| {
                            src_config::Boolean::try_from(val)
        ...
        let prompt_options = src_prompt::Options {
            ...
            mode: config
                ...
                .and_then(|val| (!val).then_some(src_prompt::Mode::Disable))
                .unwrap_or_default(),
        }
        ...
        Ok((
            src_credentials::helper::Cascade {
                ...
                query_user_only: url.scheme == src_url::Scheme::Ssh,
                stderr: config
            ...
            src_credentials::helper::Action::get_for_url(url.to_bstring()),
            prompt_options,
        ))
    }
        ...
        match (pattern, host) {
            (Some(pattern), Some(host)) => {
                ...
                lfields.zip(rfields).all(|(pat, value)| {
                    src_glob::wildmatch(pat.into(), value.into(), src_glob::wildmatch::Mode::empty())
                })
            }
            (None, None) => true,
    ...
    fn normalize(url: &mut src_url::Url) {
    ...
    impl IgnoreEmptyPath for Result<Option<std::borrow::Cow<'_, std::path::Path>>, src_config::path::interpolate::Error> {
        fn ignore_empty(self) -> Self {
            match self {
                Ok(maybe_path) => Ok(maybe_path),
                Err(src_config::path::interpolate::Error::Missing { .. }) => Ok(None),
                Err(err) => Err(err),
            }
        }
    }
