...
impl<'repo> Snapshot<'repo> {
    ...
    pub fn boolean(&self, key: impl src_config::AsKey) -> Option<bool> {
    ...
    pub fn try_boolean(&self, key: impl src_config::AsKey) -> Option<Result<bool, src_config::value::Error>> {
    ...
    ///
    /// Note that this method takes the most recent value at `key` even if it is from a file with reduced trust.
    pub fn integer(&self, key: impl src_config::AsKey) -> Option<i64> {
    ...
    pub fn try_integer(&self, key: impl src_config::AsKey) -> Option<Result<i64, src_config::value::Error>> {
    ...
    ///
    /// Note that this method takes the most recent value at `key` even if it is from a file with reduced trust.
    pub fn string(&self, key: impl src_config::AsKey) -> Option<Cow<'repo, BStr>> {
    ...
    ) -> Option<Result<Cow<'repo, std::path::Path>, src_config::path::interpolate::Error>> {
        self.repo.config.trusted_file_path(key)
    }

    /// Return the trusted string at `key` for launching using [command::prepare()](src_command::prepare()),
    /// or `None` if there is no such value or if no value was found in a trusted file.
    pub fn trusted_program(&self, key: impl src_config::AsKey) -> Option<Cow<'repo, OsStr>> {
        let value = self
            .repo
            .config
            .resolved
            .string_filter(key, &mut self.repo.config.filter_config_section.clone())?;
        Some(match src_path::from_bstr(value) {
...
impl Snapshot<'_> {
    ...
    pub fn plumbing(&self) -> &src_config::File<'static> {
...
impl<'repo> SnapshotMut<'repo> {
    ...
    pub fn set_subsection_value<'a, 'b>(
        ...
        let key = src_config::KeyRef::parse_unvalidated((**name).as_bstr())
    ...
    pub fn forget(mut self) -> src_config::File<'static> {
        self.repo.take();
