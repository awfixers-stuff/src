...
use src_config::file::Metadata;
use src_features::threading::OwnShared;
use src_object::bstr::ByteSlice;
use src_path::RelativePath;
use std::path::Path;
use std::{
    borrow::Cow,
...
impl EnvironmentOverrides {
    fn from_env() -> Result<Self, src_sec::permission::Error<std::path::PathBuf>> {
...
impl ThreadSafeRepository {
    ...
    ///
    /// Whereas `open_ext()` is the jack-of-all-trades that can do anything depending on its options, `src` will always differentiate
    ...
    pub fn open_opts(path: impl Into<PathBuf>, mut options: Options) -> Result<Self, Error> {
        let _span = src_trace::coarse!("ThreadSafeRepository::open()");
        let (path, kind) = {
            let path = path.into();
            let looks_like_git_dir =
                path.ends_with(src_discover::DOT_GIT_DIR) || path.extension() == Some(std::ffi::OsStr::new("git"));
            let candidate = if !options.open_path_as_is && !looks_like_git_dir {
                Cow::Owned(path.join(src_discover::DOT_GIT_DIR))
            } else {
                Cow::Borrowed(&path)
            };
            match src_discover::is_git(candidate.as_ref()) {
                ...
                Err(err) => {
                    ...
                    match src_discover::is_git(&path) {
                        Ok(kind) => (path, kind),
        ...
        let cwd = src_fs::current_dir(false)?;
        let (git_dir, worktree_dir) = src_discover::repository::Path::from_dot_git_dir(path, kind, &cwd)
            .expect("we have sanitized path with is_git()")
            .into_repository_and_work_tree_directories();
        if options.git_dir_trust.is_none() {
            options.git_dir_trust = src_sec::Trust::from_path_ownership(&git_dir)?.into();
        }
    ...
    /// custom hooks.
    // TODO: tests, with hooks, GIT_QUARANTINE for ref-log and transaction control (needs src-sec support to remove write access in src-ref)
    ...
    pub fn open_with_environment_overrides(
        ...
        trust_map: src_sec::trust::Mapping<Options>,
    ) -> Result<Self, Error> {
        let _span = src_trace::coarse!("ThreadSafeRepository::open_with_environment_overrides()");
        let overrides = EnvironmentOverrides::from_env()?;
        let (path, path_kind): (PathBuf, _) = match overrides.git_dir {
            Some(git_dir) => src_discover::is_git(&git_dir)
            ...
            None => {
                ...
                src_discover::is_git(&fallback_directory)
                    .map_err(|err| Error::NotARepository {
                        source: err,
        ...
        // To be altered later based on `core.precomposeUnicode`.
        let cwd = src_fs::current_dir(false)?;
        let (git_dir, worktree_dir) = src_discover::repository::Path::from_dot_git_dir(path, path_kind, &cwd)
            .expect("we have sanitized path with is_git()")
            .into_repository_and_work_tree_directories();
        let worktree_dir = worktree_dir.or(overrides.worktree_dir);

        let git_dir_trust = src_sec::Trust::from_path_ownership(&git_dir)?;
    ...
    pub(crate) fn open_from_paths(
        ...
        let _span = src_trace::detail!("open_from_paths()");
        let Options {
        ...
        let mut common_dir = src_discover::path::from_plain_file(git_dir.join("commondir").as_ref())
            .transpose()?
        ...
        if repo_config.precompose_unicode {
            git_dir = src_utils::str::precompose_path(git_dir.into()).into_owned();
            if let Some(common_dir) = common_dir.as_mut() {
                if let Cow::Owned(precomposed) = src_utils::str::precompose_path((&*common_dir).into()) {
                    *common_dir = precomposed;
                }
            }
            if let Some(worktree_dir) = worktree_dir.as_mut() {
                if let Cow::Owned(precomposed) = src_utils::str::precompose_path((&*worktree_dir).into()) {
        ...
        let current_dir = {
            ...
            if repo_config.precompose_unicode {
                if let Cow::Owned(precomposed) = src_utils::str::precompose_path((&*current_dir_ref).into()) {
        ...
        let mut refs = {
            let reflog = repo_config.reflog.unwrap_or(src_ref::store::WriteReflog::Disable);
            let object_hash = repo_config.object_hash;
            let ref_store_init_opts = src_ref::store::init::Options {
                write_reflog: reflog,
                object_hash,
        ...
        let home = src_path::env::home_dir().and_then(|home| env.home.check_opt(home));
        ...
        let worktree_dir_override_from_configuration = if !config.is_bare_but_assume_bare_if_unconfigured() {
            ...
            fn assure_config_is_from_current_repo(
                section: &src_config::file::Metadata,
                git_dir: &Path,
                current_dir: &Path,
                ...
                section
                    .path
                    .as_deref()
                    .and_then(|p| src_path::normalize(p.into(), current_dir))
            ...
            if let Some((wt, key_source)) = worktree_path {
                ...
                let wt_path = match key_source {
                    src_config::Source::Env
                    | src_config::Source::Cli
                    | src_config::Source::Api
                    | src_config::Source::EnvOverride => wt_path,
                    _ => git_dir.join(wt_path).into(),
                };
                worktree_dir = src_path::normalize(wt_path, current_dir).map(Cow::into_owned);
                #[allow(unused_variables)]
                if let Some(worktree_path) = worktree_dir.as_deref().filter(|wtd| !wtd.is_dir()) {
                    src_trace::warn!("The configured worktree path '{}' is not a directory or doesn't exist - `core.worktree` may be misleading", worktree_path.display());
                }
...

        {
            let looks_like_standard_git_dir =
                || refs.git_dir().file_name() == Some(OsStr::new(src_discover::DOT_GIT_DIR));
            match worktree_dir {
        ...
        if *git_dir_trust != src_sec::Trust::Full
            || worktree_dir
                .as_deref()
                .is_some_and(|wd| !src_sec::identity::is_path_owned_by_current_user(wd).unwrap_or(false))
        {
            ...
            if res.is_ok() {
                *git_dir_trust = src_sec::Trust::Full;
            } else if bail_if_untrusted {
                res?;
            } else {
                // This is how the worktree-trust can reduce the git-dir trust.
                *git_dir_trust = src_sec::Trust::Reduced;
            }

            let Ok(mut resolved) = src_features::threading::OwnShared::try_unwrap(config.resolved) else {
            ...
            for id in section_ids {
                ...
                if section_trusted_by_default || section.meta().trust == src_sec::Trust::Full {
                    continue;
                }
                ...
                match is_valid_by_path.entry(meta_path.to_owned()) {
                    Entry::Occupied(entry) => {
                        if *entry.get() {
                            section.set_trust(src_sec::Trust::Full);
                        } else {
                            continue;
                        }
                    }
        ...
        let replacements = match prefix {
            Some(prefix) => {
                ...
                Some(prefix).and_then(|prefix| {
                    let _span = src_trace::detail!("find replacement objects");
                    let platform = refs.iter().ok()?;
                    let iter = platform.prefixed(prefix).ok()?;
                    let replacements = iter
                        .filter_map(Result::ok)
                        .filter_map(|r: src_ref::Reference| {
                            let target = r.target.try_id()?.to_owned();
                            let source =
                                src_hash::ObjectId::from_hex(r.name.as_bstr().strip_prefix(prefix.as_ref())?).ok()?;
                            Some((source, target))
                        })
                        .collect::<Vec<_>>();
                    Some(replacements)
        ...
        Ok(ThreadSafeRepository {
            objects: OwnShared::new(src_odb::Store::at_opts(
                common_dir_ref.join("objects"),
                &mut replacements.into_iter(),
                src_odb::store::init::Options {
            ...
            #[cfg(feature = "index")]
            index: src_fs::SharedFileSnapshotMut::new().into(),
            shallow_commits: src_fs::SharedFileSnapshotMut::new().into(),
            #[cfg(feature = "attributes")]
            modules: src_fs::SharedFileSnapshotMut::new().into(),
        })
    }
}
...
fn replacement_objects_refs_prefix(
    config: &src_config::File<'static>,
    lenient: bool,
    mut filter_config_section: fn(&src_config::file::Metadata) -> bool,
...
fn check_safe_directories(
    ...
    let path_to_test = match src_path::realpath_opts(path_to_test, current_dir, src_path::realpath::MAX_SYMLINKS) {
        Ok(p) => p,
    ...
    for safe_dir in safe_dirs {
        ...
        if !is_safe {
            let safe_dir = match src_config::Path::from(Cow::Borrowed(safe_dir))
                .interpolate(interpolate_context(git_install_dir, home))
            {
                Ok(path) => path,
                Err(_) => src_path::from_bstr(safe_dir),
            };
