...
impl Options {
    ...
    /// The configuration is marked with [source API][src_config::Source::Api].
    ...
    /// as CLI overrides to the repository configuration, marked with [source CLI][src_config::Source::Cli].
    ...
    pub fn object_store_slots(mut self, slots: src_odb::store::init::Slots) -> Self {
    ...
    /// ownership via [`src_sec::Trust::from_path_ownership()`].
    ///
    /// # Security Warning
    ///
    ...
    pub fn with(mut self, trust: src_sec::Trust) -> Self {
    ...
    ///
    /// The default filter selects sections whose trust level is [`full`][src_sec::Trust::Full] or
    /// whose source is not [`repository-local`][src_config::source::Kind::Repository].
    pub fn filter_config_section(mut self, filter: fn(&src_config::file::Metadata) -> bool) -> Self {
...
impl src_sec::trust::DefaultForLevel for Options {
    fn default_for_level(level: src_sec::Trust) -> Self {
        match level {
            src_sec::Trust::Full => Options {
                object_store_slots: Default::default(),
                permissions: Permissions::default_for_level(level),
                git_dir_trust: src_sec::Trust::Full.into(),
            ...
            src_sec::Trust::Reduced => Options {
                object_store_slots: src_odb::store::init::Slots::Given(32), // limit resource usage
                permissions: Permissions::default_for_level(level),
                git_dir_trust: src_sec::Trust::Reduced.into(),
                filter_config_section: Some(config::section::is_trusted),
                bail_if_untrusted: false,
                lenient_config: true,
                open_path_as_is: false,
