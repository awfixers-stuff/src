...
#[derive(Clone)]
pub struct Options {
    pub(crate) object_store_slots: src_odb::store::init::Slots,
    /// Define what is allowed while opening a repository.
    pub permissions: Permissions,
    pub(crate) git_dir_trust: Option<src_sec::Trust>,
    /// Warning: this one is copied to config::Cache - don't change it after repo open or keep in sync.
    pub(crate) filter_config_section: Option<fn(&src_config::file::Metadata) -> bool>,
...
pub enum Error {
    ...
    NotARepository {
        source: src_discover::is_git::Error,
        path: PathBuf,
    },
    #[error(transparent)]
    ...
    #[error(transparent)]
    EnvironmentAccessDenied(#[from] src_sec::permission::Error<std::path::PathBuf>),
    #[error(transparent)]
    PrefixNotRelative(#[from] src_path::relative_path::Error),
}
