...
impl<'repo> Remote<'repo> {
    ...
    pub(crate) fn from_preparsed_config(
        ...
        url: Option<src_url::Url>,
        push_url: Option<src_url::Url>,
...
) -> Result<Option<src_url::Url>, Error> {
    url.and_then(|url| config.url_rewrite().longest(url, direction))
        .map(|url| {
            src_url::parse(url.as_ref()).map_err(|err| Error::RewrittenUrlInvalid {
...
pub(crate) fn rewrite_urls(
    ...
    url: Option<&src_url::Url>,
    push_url: Option<&src_url::Url>,
) -> Result<(Option<src_url::Url>, Option<src_url::Url>), Error> {
