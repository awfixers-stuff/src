...
impl Remote<'_> {
    ...
    fn url_inner(mut self, url: src_url::Url, should_rewrite_urls: bool) -> Result<Self, remote::init::Error> {
        self.url = url.into();
    ...
    ) -> Result<Self, src_refspec::parse::Error>
    where
        Spec: AsRef<BStr>,
    {
        ...
        let new_specs = specs
            ...
            .map(|spec| {
                src_refspec::parse(
                    spec.as_ref(),
                    match direction {
                        Push => src_refspec::parse::Operation::Push,
                        Fetch => src_refspec::parse::Operation::Fetch,
                    },
                )
                .map(|s| s.to_owned())
            })
