use src_features::progress::Progress;
...
pub enum Error {
    #[error(transparent)]
    InitRefMap(#[from] src_protocol::fetch::refmap::init::Error),
    ...
    #[error(transparent)]
    Handshake(#[from] src_protocol::handshake::Error),
    #[error(transparent)]
    Transport(#[from] src_protocol::transport::client::Error),
    #[error(transparent)]
...
pub struct Options {
    ...
    pub extra_refspecs: Vec<src_refspec::RefSpec>,
}
    ...
    T: Transport,
{
    ...
    pub async fn ref_map(
        ...
        options: Options,
    ) -> Result<(fetch::RefMap, src_protocol::Handshake), Error> {
    ...
    pub(crate) async fn ref_map_by_ref(
        ...
        let _span = src_trace::coarse!("remote::Connection::ref_map()");
        ...
        let authenticate = match self.authenticate.as_mut() {
            ...
            None => {
                let url = self.remote.url(Direction::Fetch).map_or_else(
                    || src_url::parse(url.as_ref()).expect("valid URL to be provided by transport"),
                    ToOwned::to_owned,
                );
        ...
        let mut handshake = src_protocol::handshake(
            &mut self.transport.inner,
            src_transport::Service::UploadPack,
            authenticate,
            handshake_parameters,
            &mut progress,
        )
