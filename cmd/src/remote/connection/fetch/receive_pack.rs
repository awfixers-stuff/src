...
use src_odb::store::RefreshMode;
use src_protocol::fetch::{negotiate, Arguments};
#[cfg(feature = "async-network-client")]
use src_transport::client::async_io::Transport;
#[cfg(feature = "blocking-network-client")]
use src_transport::client::blocking_io::Transport;
    ...
    T: Transport,
{
        ...
        P::SubProgress: 'static,
    {
        ...
        let fetch_options = src_protocol::fetch::Options {
        ...
        let context = src_protocol::fetch::Context {
        ...
        let write_pack_options = src_pack::bundle::write::Options {
            thread_limit: config::index_threads(repo)?,
            index_version: config::pack_index_version(repo)?,
            iteration_mode: src_pack::data::input::Mode::Verify,
            object_hash: con.remote.repo.object_hash(),
        };
        let mut write_pack_bundle = None;

        let res = src_protocol::fetch(
            &mut negotiate,
            |reader, progress, should_interrupt| -> Result<bool, src_pack::bundle::write::Error> {
                let mut may_read_to_end = false;
                write_pack_bundle = if matches!(self.dry_run, fetch::DryRun::No) {
                    let res = src_pack::Bundle::write_to_directory(
                        reader,
        ...
        if matches!(handshake.server_protocol_version, src_protocol::transport::Protocol::V2) {
            src_protocol::indicate_end_of_interaction(&mut con.transport.inner, con.trace)
                .await
                .ok();
        }
...
struct Negotiate<'a, 'b, 'c> {
    ...
    refs: &'a src_ref::file::Store,
    graph: &'a mut src_negotiate::Graph<'b, 'c>,
    alternates: Vec<PathBuf>,
    ref_map: &'a src_protocol::fetch::RefMap,
    shallow: &'a src_protocol::fetch::Shallow,
    tags: src_protocol::fetch::Tags,
    negotiator: Box<dyn src_negotiate::Negotiator>,
    open_options: crate::open::Options,
}

impl src_protocol::fetch::Negotiate for Negotiate<'_, '_, '_> {
    ...
    fn one_round(
        ...
        previous_response: Option<&src_protocol::fetch::Response>,
