...
fn hex_to_id(hex: &str) -> src_hash::ObjectId {
    src_hash::ObjectId::from_hex(hex.as_bytes()).expect("40 bytes hex")
}

mod update {
    use src_testtools::Result;
    ...
    fn repo(name: &str) -> src::Repository {
        let dir = src_testtools::scripted_fixture_read_only_with_args_single_archive(
            "make_fetch_repos.sh",
            [base_repo_path()],
        )
        .unwrap();
        src::open_opts(dir.join(name), restricted()).unwrap()
    }
    fn named_repo(name: &str) -> src::Repository {
        let dir = src_testtools::scripted_fixture_read_only("make_remote_repos.sh").unwrap();
        src::open_opts(dir.join(name), restricted()).unwrap()
    }
    fn repo_rw(name: &str) -> (src::Repository, src_testtools::tempfile::TempDir) {
        ...
        .unwrap();
        let repo = src::open_opts(dir.path().join(name), restricted()).unwrap();
        (repo, dir)
    }
    use src_ref::{
    ...
    fn checked_out_branches_in_worktrees_are_rejected_with_additional_information() -> Result {
        let root = src_path::realpath(src_testtools::scripted_fixture_read_only_with_args_single_archive(
            "make_fetch_repos.sh",
            [base_repo_path()],
        )?)?;
        let repo = root.join("worktree-root");
        let repo = src::open_opts(repo, restricted())?;
    ...
    fn remote_symbolic_refs_can_always_be_set_as_there_is_no_scenario_where_it_could_be_nonexisting_and_rejected() {
        ...
        mappings.push(Mapping {
            remote: Source::Ref(src_protocol::handshake::Ref::Direct {
    ...
    fn mapping_from_spec(
        spec: &str,
        remote_repo: &src::Repository,
    ) -> (Vec<fetch::refmap::Mapping>, Vec<src::refspec::RefSpec>) {
        let spec = src_refspec::parse(spec.into(), src_refspec::parse::Operation::Fetch).unwrap();
        let group = src_refspec::MatchGroup::from_fetch_specs(Some(spec));
        ...
        let mappings = group
            ...
            .map(|m| fetch::refmap::Mapping {
                remote: m.item_index.map_or_else(
                    || match m.lhs {
                        src_refspec::match_group::SourceRef::ObjectId(id) => fetch::refmap::Source::ObjectId(id),
    ...
    fn into_remote_ref(mut r: src::Reference<'_>) -> src_protocol::handshake::Ref {
        let full_ref_name = r.name().as_bstr().into();
        match r.target() {
            TargetRef::Object(id) => src_protocol::handshake::Ref::Direct {
                full_ref_name,
                object: id.into(),
            },
            TargetRef::Symbolic(name) => {
                ...
                match r.peel_to_id() {
                    Ok(id) => src_protocol::handshake::Ref::Symbolic {
                        full_ref_name,
                        target,
                        tag: None,
                        object: id.detach(),
                    },
                    Err(_) => src_protocol::handshake::Ref::Unborn { full_ref_name, target },
                }
            }
        }
    }

    fn remote_ref_to_item(r: &src_protocol::handshake::Ref) -> src_refspec::match_group::Item<'_> {
        let (full_ref_name, target, object) = r.unpack();
        static NULL: src_hash::ObjectId = src_hash::Kind::Sha1.null();
        src_refspec::match_group::Item {
            full_ref_name,
