...
mod error {
    ...
    pub enum Error {
        ...
        InvalidRefName(#[from] src_validate::reference::name::Error),
        ...
        FollowSymref(#[from] src_ref::file::find::existing::Error),
        #[error(transparent)]
...
pub struct Outcome {
    ...
    pub edits: Vec<src_ref::transaction::RefEdit>,
...
impl Outcome {
    ...
    pub fn iter_mapping_updates<'a, 'b>(
        ...
        refspecs: &'b [src_refspec::RefSpec],
        extra_refspecs: &'b [src_refspec::RefSpec],
    ) -> impl Iterator<
        Item = (
            &super::Update,
            &'a fetch::refmap::Mapping,
            Option<&'b src_refspec::RefSpec>,
            Option<&src_ref::transaction::RefEdit>,
        ),
    > {
