...
pub(crate) fn update(
    ...
    refspecs: &[src_refspec::RefSpec],
    extra_refspecs: &[src_refspec::RefSpec],
    fetch_tags: fetch::Tags,
    dry_run: fetch::DryRun,
    write_packed_refs: fetch::WritePackedRefs,
) -> Result<update::Outcome, update::Error> {
    let _span = src_trace::detail!("update_refs()", mappings = mappings.len());
        ...
        let (mode, edit_index, type_change) = match local {
            Some(name) => {
                let (mode, reflog_message, name, previous_value) = match repo.try_find_reference(name)? {
                    Some(existing) => {
                            ...
                            .map(crate::Id::detach)
                        {
                            Ok(local_id) => {
                                ...
                                } else if let Some(src_ref::Category::Tag) = existing.name().category() {
                                            ...
                                            let ancestors = repo
                                                ...
                                                .and_then(|local_commit_time| {
                                                    remote_id
                                                        ...
                                                        .sorting(
                                                            src_traverse::commit::simple::Sorting::ByCommitTimeCutoff {
                            ...
                            Err(crate::reference::peel::Error::ToId(src_ref::peel::to_id::Error::FollowToObject(
                                src_ref::peel::to_object::Error::Follow(_),
                            ))) => {
                                // An unborn reference, always allow it to be changed to whatever the remote wants.
                                (
                                    if existing.target().try_name().map(src_ref::FullNameRef::as_bstr)
                                        == remote.as_target()
                                    {
                                        Mode::NoChangeNeeded
                                    } else {
                    ...
                    None => {
                        let name: src_ref::FullName = name.try_into()?;
                        let reflog_msg = match name.category() {
                            Some(src_ref::Category::Tag) => "storing tag",
                            Some(src_ref::Category::LocalBranch) => "storing head",
                            _ => "storing ref",
                        };
                        (
                            Mode::New,
    ...
    let edits = match dry_run {
        fetch::DryRun::No => {
            let _span = src_trace::detail!("apply", edits = edits.len());
                ...
                .packed_refs(
                    match write_packed_refs {
                        fetch::WritePackedRefs::Only => {
                            src_ref::file::transaction::PackedRefs::DeletionsAndNonSymbolicUpdatesRemoveLooseSourceReference(Box::new(&repo.objects))},
                        fetch::WritePackedRefs::Never => src_ref::file::transaction::PackedRefs::DeletionsOnly
                    }
                )
...
fn new_value_by_remote(
    ...
    Ok(
        if let Source::Ref(
            src_protocol::handshake::Ref::Symbolic { target, .. } | src_protocol::handshake::Ref::Unborn { target, .. },
        ) = &remote
        {
...
fn insert_head(
    ...
    out: &mut BTreeMap<src_ref::FullName, Vec<PathBuf>>,
) -> Result<(), update::Error> {
...
fn worktree_branches(repo: &Repository) -> Result<BTreeMap<src_ref::FullName, Vec<PathBuf>>, update::Error> {
