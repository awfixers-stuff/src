...
use src_transport::client::async_io::{connect, Transport};
#[cfg(feature = "blocking-network-client")]
use src_transport::client::blocking_io::{connect, Transport};
...
mod error {
    ...
    pub enum Error {
        ...
        ProtocolDenied { url: BString, scheme: src_url::Scheme },
        #[error(transparent)]
        ...
        FileUrl {
            source: Box<src_discover::is_git::Error>,
            url: src_url::Url,
        },
    }

    impl src_protocol::transport::IsSpuriousError for Error {
...
impl<'repo> Remote<'repo> {
        ...
        T: Transport,
    {
        ...
        Connection {
            ...
            handshake: None,
            transport: src_protocol::SendFlushOnDrop::new(transport, trace),
            trace,
        }
    }
    ...
    pub async fn connect(
        ...
        #[cfg(feature = "blocking-network-client")]
        let scheme_is_ssh = url.scheme == src_url::Scheme::Ssh;
    ...
    pub fn sanitized_url_and_version(
        &self,
        direction: crate::remote::Direction,
    ) -> Result<(src_url::Url, src_protocol::transport::Protocol), Error> {
        fn sanitize(mut url: src_url::Url) -> Result<src_url::Url, Error> {
            if url.scheme == src_url::Scheme::File {
                let mut dir = src_path::to_native_path_on_windows(Cow::Borrowed(url.path.as_ref()));
                let kind = src_discover::is_git(dir.as_ref())
                    .or_else(|_| {
                        dir.to_mut().push(src_discover::DOT_GIT_DIR);
                        src_discover::is_git(dir.as_ref())
                    })
                    .map_err(|err| Error::FileUrl {
                        source: err.into(),
                        url: url.clone(),
                    })?;
                let (git_dir, _work_dir) = src_discover::repository::Path::from_dot_git_dir(
