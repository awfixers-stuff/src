...
impl Proxy<'_> {
    ...
    pub fn base(&self) -> std::io::Result<PathBuf> {
        ...
        let base_dot_git = src_discover::path::from_plain_file(&git_dir).ok_or_else(|| {
            std::io::Error::new(
        ...
        Ok(src_discover::path::without_dot_git_dir(base_dot_git))
    }
    ...
    pub fn id(&self) -> &BStr {
        src_path::os_str_into_bstr(self.git_dir.file_name().expect("worktrees/ parent dir"))
