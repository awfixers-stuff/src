...
pub use src_archive as archive;
#[cfg(feature = "excludes")]
pub use src_worktree::*;
#[cfg(feature = "worktree-mutation")]
pub use src_worktree_state as state;
#[cfg(feature = "worktree-stream")]
pub use src_worktree_stream as stream;
...
#[cfg(feature = "index")]
pub(crate) type IndexStorage = src_features::threading::OwnShared<src_fs::SharedFileSnapshotMut<src_index::File>>;
/// A lazily loaded and auto-updated worktree index.
#[cfg(feature = "index")]
pub type Index = src_fs::SharedFileSnapshot<src_index::File>;
...
#[cfg(feature = "index")]
impl From<src_index::File> for IndexPersistedOrInMemory {
    fn from(value: src_index::File) -> Self {
...
impl<'repo> crate::Worktree<'repo> {
    ...
    pub fn dot_git_exists(&self) -> bool {
        self.path.join(src_discover::DOT_GIT_DIR).exists()
    }
}
...
pub(crate) fn id(git_dir: &std::path::Path, has_common_dir: bool) -> Option<&BStr> {
    ...
    let candidate = src_path::os_str_into_bstr(git_dir.file_name().expect("at least one directory level"))
...
pub mod open_index {
    ...
    pub enum Error {
        ...
        #[error(transparent)]
        IndexFile(#[from] src_index::file::init::Error),
        #[error(transparent)]
        IndexCorrupt(#[from] src_index::file::verify::Error),
    }

    impl crate::Worktree<'_> {
        /// A shortcut to [`crate::Repository::open_index()`].
        pub fn open_index(&self) -> Result<src_index::File, Error> {
...
pub mod excludes {
    ...
    impl crate::Worktree<'_> {
        ...
        pub fn excludes(&self, overrides: Option<src_ignore::Search>) -> Result<AttributeStack<'_>, Error> {
            let index = self.index()?;
            Ok(self.parent.excludes(
                &index,
                overrides,
                src_worktree::stack::state::ignore::Source::WorktreeThenIdMappingIfNotSkipped,
            )?)
        }
    }
}
...
pub mod attributes {
    ...
    impl<'repo> Worktree<'repo> {
        ...
        pub fn attributes(&self, overrides: Option<src_ignore::Search>) -> Result<AttributeStack<'repo>, Error> {
            let index = self.index()?;
