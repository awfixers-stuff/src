...
pub struct Platform<'r> {
    pub(crate) platform: src_ref::file::iter::Platform<'r>,
...
pub struct Iter<'packed, 'repo> {
    inner: src_ref::file::iter::LooseThenPacked<'packed, 'repo>,
    peel_with_packed: Option<src_ref::file::packed::SharedBufferSnapshot>,
    peel: bool,
    repo: &'repo crate::Repository,
}

impl<'packed, 'repo> Iter<'packed, 'repo> {
    fn new(repo: &'repo crate::Repository, platform: src_ref::file::iter::LooseThenPacked<'packed, 'repo>) -> Self {
        Iter {
            inner: platform,
...
impl<'repo> Platform<'repo> {
    ...
    pub fn prefixed<'a>(
        &self,
        prefix: impl TryInto<&'a RelativePath, Error = src_path::relative_path::Error>,
...
impl Iter<'_, '_> {
    ...
    pub fn peeled(mut self) -> Result<Self, src_ref::packed::buffer::open::Error> {
...
pub mod init {
    ...
    pub enum Error {
        ...
        #[error(transparent)]
        RelativePath(#[from] src_path::relative_path::Error),
    }
}

/// The error returned by [references()][crate::Repository::references()].
pub type Error = src_ref::packed::buffer::open::Error;
