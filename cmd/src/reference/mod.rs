...
impl<'repo> Reference<'repo> {
    ...
    pub fn try_id(&self) -> Option<Id<'repo>> {
        match self.inner.target {
            src_ref::Target::Symbolic(_) => None,
            src_ref::Target::Object(oid) => oid.to_owned().attach(self.repo).into(),
        }
    }
    ...
    pub fn target(&self) -> src_ref::TargetRef<'_> {
        self.inner.target.to_ref()
    }

    /// Return the reference's full name.
    pub fn name(&self) -> &src_ref::FullNameRef {
        self.inner.name.as_ref()
    }

    /// Turn this instances into a stand-alone reference.
    pub fn detach(self) -> src_ref::Reference {
        self.inner
    }
}
...
impl<'repo> Reference<'repo> {
    pub(crate) fn from_ref(reference: src_ref::Reference, repo: &'repo crate::Repository) -> Self {
...
impl<'repo> Reference<'repo> {
    ...
    pub fn peel_to_id_in_place_packed(
        &mut self,
        packed: Option<&src_ref::packed::Buffer>,
    ...
    /// Note that this method mutates `self` in place if it does not already point to a
    /// non-symbolic object.
    pub fn peel_to_id_packed(&mut self, packed: Option<&src_ref::packed::Buffer>) -> Result<Id<'repo>, peel::Error> {
        let oid = self
            .inner
            .peel_to_id_packed(&self.repo.refs, &self.repo.objects, packed)?;
        Ok(Id::from_id(oid, self.repo))
    ...
    pub fn peel_to_kind(&mut self, kind: src_object::Kind) -> Result<Object<'repo>, peel::to_kind::Error> {
        let packed = self.repo.refs.cached_packed_buffer().map_err(|err| {
            peel::to_kind::Error::FollowToObject(src_ref::peel::to_object::Error::Follow(
    ...
    pub fn peel_to_commit(&mut self) -> Result<Commit<'repo>, peel::to_kind::Error> {
        Ok(self.peel_to_kind(src_object::Kind::Commit)?.into_commit())
    }
    ...
    pub fn peel_to_tag(&mut self) -> Result<Tag<'repo>, peel::to_kind::Error> {
        Ok(self.peel_to_kind(src_object::Kind::Tag)?.into_tag())
    }
    ...
    pub fn peel_to_tree(&mut self) -> Result<Tree<'repo>, peel::to_kind::Error> {
        Ok(self.peel_to_kind(src_object::Kind::Tree)?.into_tree())
    }
    ...
    pub fn peel_to_blob(&mut self) -> Result<Blob<'repo>, peel::to_kind::Error> {
        Ok(self.peel_to_kind(src_object::Kind::Blob)?.into_blob())
    }
    ...
    pub fn follow_to_object(&mut self) -> Result<Id<'repo>, follow::to_object::Error> {
        let packed = self.repo.refs.cached_packed_buffer().map_err(|err| {
            follow::to_object::Error::FollowToObject(src_ref::peel::to_object::Error::Follow(
                file::find::existing::Error::Find(file::find::Error::PackedOpen(err)),
            ))
        })?;
    ...
    pub fn follow(&self) -> Option<Result<Reference<'repo>, src_ref::file::find::existing::Error>> {
