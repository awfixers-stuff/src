...
/// An empty array of a type usable with the `src::easy` API to help declaring no parents should be used
pub const NO_PARENT_IDS: [src_hash::ObjectId; 0] = [];
...
pub mod describe {
    ...
    pub struct Resolution<'repo> {
        ...
        pub outcome: src_revision::describe::Outcome<'static>,
        /// The id to describe.
    ...
    impl Resolution<'_> {
        ...
        pub fn format(self) -> Result<src_revision::describe::Format<'static>, Error> {
        ...
        ) -> Result<src_revision::describe::Format<'static>, Error> {
            let prefix = self.id.shorten()?;
    ...
    pub enum Error {
        ...
        #[error(transparent)]
        Describe(#[from] src_revision::describe::Error),
    ...
    pub struct Platform<'repo> {
        pub(crate) id: src_hash::ObjectId,
    ...
    impl<'repo> Platform<'repo> {
        ...
        pub fn try_format(&self) -> Result<Option<src_revision::describe::Format<'static>>, Error> {
        ...
        pub fn try_resolve_with_cache(
            &self,
            cache: Option<&'_ src_commitgraph::Graph>,
        ) -> Result<Option<Resolution<'repo>>, Error> {
            let mut graph = self.repo.revision_graph(cache);
            let outcome = src_revision::describe(
                &self.id,
                &mut graph,
                src_revision::describe::Options {
        ...
        pub fn format(&mut self) -> Result<src_revision::describe::Format<'static>, Error> {
