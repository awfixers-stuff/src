...
mod needs_feature {
    ...
    #[test]
    fn multi_registration() -> src_testtools::Result {
        ...
        let reg1 = unsafe {
            src::interrupt::init_handler(3, || {
                V1.fetch_add(1, Ordering::SeqCst);
            })
        }
        .expect("succeeds");
        assert!(!src::interrupt::is_triggered());
        assert_eq!(V1.load(Ordering::Relaxed), 0);
        // SAFETY: The closure doesn't use mutexes or memory allocation, so it should be safe to call from a signal handler.
        let reg2 = unsafe { src::interrupt::init_handler(2, || V2.store(true, Ordering::SeqCst)) }
        ...
        assert!(src::interrupt::is_triggered(), "this happens automatically");
