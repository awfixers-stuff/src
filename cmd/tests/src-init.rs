...
use src::open::Permissions;
use src::{Repository, ThreadSafeRepository};
...
pub fn named_subrepo_opts(
    fixture: &str,
    name: &str,
    opts: src::open::Options,
) -> std::result::Result<Repository, src::open::Error> {
    let repo_path = src_testtools::scripted_fixture_read_only(fixture).unwrap().join(name);
...
mod with_overrides {
    ...
    use src_sec::Permission;
    use src_testtools::Env;
    use serial_test::serial;

    #[test]
    #[serial]
    fn order_from_api_and_cli_and_environment() -> src_testtools::Result {
        ...
        let _env = Env::new()
            ...
            .set("src_PACK_CACHE_MEMORY", "0")
            .set("src_PARSE_PRECIOUS", "1")
            .set("src_OBJECT_CACHE_MEMORY", "5m")
            .set("src_CREDENTIALS_HELPER_STDERR", "creds-stderr")
            .set("src_EXTERNAL_COMMAND_STDERR", "filter-stderr")
        ...
        let mut opts = src::open::Options::isolated()
            .cli_overrides([
...
#[test]
#[serial]
fn git_worktree_and_strict_config() -> src_testtools::Result {
    let _restore_env_on_drop = src_testtools::Env::new().set("GIT_WORK_TREE", ".");
