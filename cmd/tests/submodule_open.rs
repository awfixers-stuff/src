...
fn on_nested_symlink() -> src_testtools::Result {
    let symlink_root =
        src_testtools::scripted_fixture_read_only("make_submodules.sh")?.join("link-to-dir-in-changed-parent-repo");
    ...
    for item in repo.status(src::progress::Discard)?.into_iter(None)? {
        assert!(
            item.is_ok(),
