...
use src::{open, Repository, ThreadSafeRepository};
use src_testtools::tempfile;
pub use src_testtools::Result;

/// Convert a hexadecimal hash into its corresponding `ObjectId` or _panic_.
pub fn hex_to_id(hex: &str) -> src_hash::ObjectId {
    src_hash::ObjectId::from_hex(hex.as_bytes()).expect("40 bytes hex")
}

pub fn freeze_time() -> src_testtools::Env<'static> {
    let frozen_time = "42 +0030";
    src_testtools::Env::new()
...
pub fn repo(name: &str) -> Result<ThreadSafeRepository> {
    let repo_path = src_testtools::scripted_fixture_read_only(name)?;
...
pub fn repo_opts(name: &str, opts: open::Options) -> std::result::Result<ThreadSafeRepository, open::Error> {
    let repo_path = src_testtools::scripted_fixture_read_only(name).unwrap();
...
pub fn named_subrepo_opts(
    ...
    name: &str,
    opts: open::Options,
) -> std::result::Result<Repository, src::open::Error> {
    let repo_path = src_testtools::scripted_fixture_read_only(fixture)
        .map_err(|err| src::open::Error::Io(std::io::Error::other(err)))?
        .join(name);
...
pub fn restricted_and_git() -> open::Options {
    ...
    opts.permissions.env.git_prefix = src_sec::Permission::Allow;
    opts.permissions.env.identity = src_sec::Permission::Allow;
    opts
}
...
pub fn repo_rw_opts(name: &str, opts: src::open::Options) -> Result<(Repository, tempfile::TempDir)> {
    let repo_path = src_testtools::scripted_fixture_writable(name)?;
    Ok((
