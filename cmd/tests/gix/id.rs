...
fn prefix() -> crate::Result {
    ...
    let repo = src::open_opts(
        work_dir,
        src::open::Options::isolated().config_overrides(Core::ABBREV.validated_assignment("5".into())),
    )
    .unwrap();
    ...
    assert!(
        src::open_opts(
            work_dir,
            src::open::Options::isolated().config_overrides(Some(BString::from("core.abbrev=invalid")))
        )
        .is_ok(),
    ...
    assert!(
        matches!(
            ...
            .unwrap_err(),
            src::open::Error::Config(src::config::Error::CoreAbbrev(_))
        ),
...
mod ancestors {
    ...
    fn all() -> crate::Result {
        ...
        for use_commit_graph in [false, true] {
            ...
            let commits_graph_order = head
                ...
                .all()?
                .map(|c| c.map(src::revision::walk::Info::detach))
            ...
            let commits_by_commit_date = head
                .ancestors()
                .use_commit_graph(!use_commit_graph)
                .sorting(src::revision::walk::Sorting::ByCommitTime(Default::default()))
                .all()?
                .map(|c| c.map(src::revision::walk::Info::detach))
                .collect::<Result<Vec<_>, _>>()?;
            assert_eq!(
    ...
    fn pre_epoch() -> crate::Result {
        ...
        for use_commit_graph in [false, true] {
            ...
            let commits = head
                .ancestors()
                .sorting(src::revision::walk::Sorting::ByCommitTime(Default::default())) // assure we have time set
                .use_commit_graph(use_commit_graph)
                .all()?
                .collect::<Result<Vec<_>, _>>()?;
    ...
    fn filtered() -> crate::Result {
        ...
        for use_commit_graph in [false, true] {
            for sorting in [
                src::revision::walk::Sorting::BreadthFirst,
                src::revision::walk::Sorting::ByCommitTime(Default::default()),
                src::revision::walk::Sorting::ByCommitTimeCutoff {
