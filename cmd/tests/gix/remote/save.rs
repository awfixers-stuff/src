mod save_to {
    ...
    fn named_remotes_save_as_is() -> crate::Result {
        ...
        let mut config = src::config::File::default();
...
mod save_as_to {
    ...
    fn anonymous_remotes_cannot_be_saved_lacking_a_name() -> crate::Result {
        ...
        assert!(matches!(
            remote.save_to(&mut src::config::File::default()).unwrap_err(),
            src::remote::save::Error::NameMissing { .. }
        ));
        Ok(())
    }
