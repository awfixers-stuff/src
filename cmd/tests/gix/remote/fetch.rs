...
mod blocking_and_async_io {
    ...
    use src_testtools::tempfile::TempDir;
    ...
    pub(crate) fn base_repo_path() -> String {
        src::path::realpath(
            src_testtools::scripted_fixture_read_only("make_remote_repos.sh")
                .unwrap()
                .join("base"),
        )
        .unwrap()
    ...
    pub(crate) fn repo_path(name: &str) -> std::path::PathBuf {
        let dir =
            src_testtools::scripted_fixture_read_only_with_args("make_fetch_repos.sh", [base_repo_path()]).unwrap();
        dir.join(name)
    }
    ...
    pub(crate) fn try_repo_rw(
        name: &str,
    ) -> Result<(src::Repository, src_testtools::tempfile::TempDir), src::open::Error> {
    ...
    pub(crate) fn try_repo_rw_args<S: Into<String>>(
        ...
        let dir = src_testtools::scripted_fixture_writable_with_args_single_archive(
            "make_fetch_repos.sh",
            [{
            ...
            src_testtools::Creation::Execute,
        )
        .unwrap();
        let repo = src::open_opts(dir.path().join(name), crate::restricted())?;
        Ok((repo, dir))
    }

    pub(crate) fn repo_rw(name: &str) -> (src::Repository, src_testtools::tempfile::TempDir) {
    ...
    fn fetch_more_packs_than_can_be_handled() -> src_testtools::Result {
        use src::{config::tree::User, interrupt::IS_INTERRUPTED};
        use src_odb::store::init::Slots;
        use src_testtools::tempfile;
        fn create_empty_commit(repo: &src::Repository) -> anyhow::Result<()> {
            let name = repo.head_name()?.expect("no detached head");
            repo.commit(
                name.as_bstr(),
                "empty",
                src::hash::ObjectId::empty_tree(repo.object_hash()),
        ...
        fn check_fetch_output(
            repo: &src::Repository,
            out: src::remote::fetch::Outcome,
            expected_count: usize,
        ) -> src_testtools::Result {
        ...
        fn check_odb_accessability(repo: &src::Repository, expected_count: usize) -> src_testtools::Result {
            let mut count_unique = 0;
            // TODO: somehow there is a lot of duplication when receiving objects.
            let mut seen = src_hashtable::HashSet::default();
        ...
        for max_packs in 1..=3 {
            ...
            let mut remote_repo = src::init_bare(remote_dir.path())?;
            {
            ...
            let (local_repo, _) = src::clone::PrepareFetch::new(
                remote_repo.path(),
                local_dir.path(),
                src::create::Kind::Bare,
                Default::default(),
                src::open::Options::isolated().object_store_slots(Slots::Given(max_packs)),
            )?
            .fetch_only(src::progress::Discard, &IS_INTERRUPTED)?;
            ...
            for round_to_create_pack in 1..one_more_than_minimum {
                ...
                match remote
                    ...
                    .receive(src::progress::Discard, &IS_INTERRUPTED)
                {
                    ...
                    Err(err) => {
                        ...
                        let local_repo = src::open_opts(
                            local_repo.path(),
                            src::open::Options::isolated().object_store_slots(slots),
                        )?;
    ...
    #[cfg(feature = "blocking-network-client")]
    #[allow(clippy::result_large_err)]
    fn collate_fetch_error() -> Result<(), src::env::collate::fetch::Error<std::io::Error>> {
        ...
        assert!(
            repo.path()
                .join("HEAD")
                .metadata()
                .map_err(src::env::collate::fetch::Error::Other)?
                .is_file(),
    ...
    fn fetch_with_alternates_adds_tips_from_alternates() -> crate::Result<()> {
        ...
        let (_repo, out) = src::clone::PrepareFetch::new(
            remote::repo("multi_round/server").path(),
            tmp.path(),
            src::create::Kind::Bare,
            Default::default(),
            src::open::Options::isolated(),
        )?
        .configure_remote({
            move |r| {
                std::fs::write(
                    ...
                    format!(
                        "{}\n",
                        src::path::realpath(remote_repo.objects.store_ref().path())?.display()
                    )
                    .as_bytes(),
                )?;
                Ok(r)
        ...
        .fetch_only(src::progress::Discard, &AtomicBool::default())?;
    ...
    async fn fetch_with_multi_round_negotiation() -> crate::Result {
        for (algorithm, expected_negotiation_rounds) in [
            (src::negotiate::Algorithm::Consecutive, 4),
            (src::negotiate::Algorithm::Skipping, 2),
        ] {
            for version in [
                src::protocol::transport::Protocol::V1,
                ...
                let (mut client_repo, _tmp) = {
                    ...
                    let tmp = TempDir::new()?;
                    let repo = src::prepare_clone_bare(
...

                {
                    ...
                    config.set_value(
                        &src::config::tree::Protocol::VERSION,
                        (version as u8).to_string().as_str(),
                    )?;
                    config.set_value(
                        &src::config::tree::Fetch::NEGOTIATION_ALGORITHM,
    ...
    async fn fetch_empty_pack() -> crate::Result {
        for version in [
            ...
            for (shallow_args, expected) in [(None, [1, 7, 7]), (Some("--depth=2"), [1, 2, 2])] {
                if version == src::protocol::transport::Protocol::V1 && shallow_args.is_some() {
    ...
    async fn fetch_pack_without_local_destination() -> crate::Result {
        ...
        for (fetch_tags, expected_data_hash, num_objects_offset, expected_ref_edits) in [
            ...
            let res: src::remote::fetch::Outcome = remote
                .connect(Fetch)
                .await?
                .prepare_fetch(src::progress::Discard, Default::default())
                .await?
            ...
            match res.status {
                src::remote::fetch::Status::Change { write_pack_bundle, update_refs, negotiate } => {
    ...
    async fn fetch_pack() -> crate::Result {
        ...
        for version in [
            ...
            // No updates
            {
                ...
                assert!(matches!(
                    outcome.status,
                    src::remote::fetch::Status::NoPackReceived { .. }
                ));
            }
            ...
            for dry_run in [true, false] {
                let remote = into_daemon_remote_if_async(
                    repo.find_remote("changes-on-top-of-origin")?
                        .with_fetch_tags(src::remote::fetch::Tags::None),
                    daemon.as_ref(),
                    "clone-as-base-with-changes",
                );
                let outcome: src::remote::fetch::Outcome = remote
                    .connect(Fetch)
                    .await?
                    .prepare_fetch(progress::Discard, Default::default())
                    .await?
                ...
                let refs = match outcome.status {
                    fetch::Status::Change {
                        ...
                        assert_eq!(negotiate.rounds.len(), 1);
                        assert_eq!(write_pack_bundle.pack_version, src::odb::pack::data::Version::V2);
                ...
                for (_update, mapping, _spec, edit) in refs.iter_mapping_updates(
                    ...
                    if dry_run {
                        match edit.change.new_value().expect("no deletions") {
                            src_ref::TargetRef::Object(id) => {
                                assert_eq!(id, mapping.remote.as_id().expect("no unborn"));
                            }
                            src_ref::TargetRef::Symbolic(target) => {
