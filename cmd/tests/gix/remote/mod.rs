...
use src_testtools::scripted_fixture_read_only;
...
pub(crate) fn repo(name: &str) -> src::Repository {
    src::open_opts(repo_path(name), src::open::Options::isolated()).unwrap()
}
...
pub(crate) fn spawn_git_daemon_if_async(
...
) -> std::io::Result<Option<src_testtools::GitDaemon>> {
    ...
    {
        src_testtools::spawn_git_daemon(_base_dir).map(Some)
    }
}
...
pub(crate) fn into_daemon_remote_if_async<'repo, 'a>(
    remote: src::Remote<'repo>,
    _daemon: Option<&src_testtools::GitDaemon>,
    ...
    #[cfg(feature = "async-network-client-async-std")]
    {
        ...
        for direction in [src::remote::Direction::Fetch, src::remote::Direction::Push] {
            new_remote
                .replace_refspecs(
...
mod name {
    ...
    #[test]
    fn origin_is_valid() {
        assert!(src::remote::name::validated("origin").is_ok());
    }

    #[test]
    fn multiple_slashes_are_valid() {
        assert!(src::remote::name::validated("origin/another").is_ok());
    }

    #[test]
    fn empty_is_invalid() {
        assert!(src::remote::name::validated("").is_err());
    }
}
