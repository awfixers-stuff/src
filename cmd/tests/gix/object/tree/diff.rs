...
use src::object::{blob::diff::lines, tree::diff::Change};
use src_object::{bstr::ByteSlice, tree::EntryKind};
...
fn changes_against_tree_modified() -> crate::Result {
    ...
    let mut cache = repo.diff_resource_cache(src_diff::blob::pipeline::Mode::ToGit, Default::default())?;
...
mod track_rewrites {
    ...
    fn jj_realistic_needs_to_be_more_clever() -> crate::Result {
        let repo = named_subrepo_opts("make_diff_repos.sh", "jj-trackcopy-1", src::open::Options::isolated())?;
        ...
        let actual: Vec<_> = repo
            .diff_tree_to_tree(
                &from,
                &to,
                Some(src::diff::Options::default().with_rewrites(Some(rewrites))),
            )?
            .into_iter()
    ...
    fn jj_realistic_directory_rename() -> crate::Result {
        ...
        let actual: Vec<_> = repo
            .diff_tree_to_tree(
                &from,
                &to,
                Some(src::diff::Options::default().with_rewrites(Some(Rewrites::default()))),
            )?
            .into_iter()
            .collect();
        insta::assert_debug_snapshot!(actual, @r#"
...
fn tree_named(repo: &src::Repository, rev_spec: impl AsRef<str>) -> src::Tree<'_> {
    repo.rev_parse_single(rev_spec.as_ref())
        .unwrap()
        .object()
        .unwrap()
        .peel_to_kind(src::object::Kind::Tree)
        .unwrap()
        .into_tree()
}
