...
mod with_core_worktree_config {
    ...
    fn relative() -> crate::Result {
        for (name, is_relative) in [("absolute-worktree", false), ("relative-worktree", true)] {
            ...
            if is_relative {
                ...
                assert_eq!(
                    repo.workdir().unwrap(),
                    src_path::realpath(repo.git_dir().parent().unwrap().parent().unwrap().join("worktree"))?,
            ...
            assert_eq!(
                baseline[0].root,
                src_path::realpath(repo.git_dir().parent().unwrap())?,
    ...
    fn repo(name: &str) -> src::Repository {
        let dir = src_testtools::scripted_fixture_read_only("make_core_worktree_repo.sh").unwrap();
        src::open_opts(dir.join(name), crate::restricted()).unwrap()
    }
...
mod baseline {
    ...
    use src::bstr::{BString, ByteSlice};
    ...
    impl Iterator for Baseline<'_> {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            let root = src_path::from_bstr(Cow::Borrowed(fields(self.lines.next()?).1)).into_owned();
            let mut bare = false;
            let mut branch = None;
            let mut peeled = src_hash::ObjectId::null(src_hash::Kind::Sha1);
            let mut locked = None;
            ...
            for line in self.lines.by_ref() {
                ...
                match field {
                    f if f == "HEAD" => peeled = src_hash::ObjectId::from_hex(value).expect("valid hash"),
...
fn from_bare_parent_repo() {
    if src_testtools::should_skip_as_git_version_is_smaller_than(2, 31, 0) {
        return;
    }
    let dir = src_testtools::scripted_fixture_read_only_with_args("make_worktree_repo.sh", ["bare"]).unwrap();
    let repo = src::open(dir.join("repo.git")).unwrap();
...
fn from_nonbare_parent_repo() {
    ...
    let dir = src_testtools::scripted_fixture_read_only("make_worktree_repo.sh").unwrap();
    let repo = src::open(dir.join("repo")).unwrap();

    run_assertions(repo, false /* bare */);
}

#[test]
fn from_nonbare_parent_repo_set_workdir() -> src_testtools::Result {
    if src_testtools::should_skip_as_git_version_is_smaller_than(2, 31, 0) {
        return Ok(());
    }

    let dir = src_testtools::scripted_fixture_read_only("make_worktree_repo.sh").unwrap();
    let mut repo = src::open(dir.join("repo")).unwrap();
...
fn run_assertions(main_repo: src::Repository, should_be_bare: bool) {
    assert_eq!(main_repo.is_bare(), should_be_bare);
    assert_eq!(main_repo.kind(), src::repository::Kind::Common);
    ...
    for actual in actual {
        ...
        let repo = if base.is_dir() {
            ...
            assert!(
                matches!(
                    ...
                    Err(src::worktree::proxy::into_repo::Error::MissingWorktree { .. })
                ),
        ...
        assert_eq!(
            src_path::realpath(proxy_by_id.git_dir()).ok(),
            src_path::realpath(actual.git_dir()).ok(),
