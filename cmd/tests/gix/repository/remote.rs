...
mod find_remote {
    ...
    use src::{remote::Direction, Repository};
    ...
    fn tags_option() -> crate::Result {
        ...
        for (remote_name, expected) in [
            ("origin", src::remote::fetch::Tags::None),
            ("myself-no-tags", src::remote::fetch::Tags::None),
            ("myself-with-tags", src::remote::fetch::Tags::All),
        ] {
    ...
    fn typical() -> crate::Result {
        ...
        for (name, (url, refspec)) in repo.remote_names().into_iter().zip(expected) {
            ...
            let url = src::url::parse(url.into())?;
        ...
        assert!(matches!(
            ...
            src::remote::find::existing::Error::NotFound { .. }
        ));
        Ok(())
    }
    ...
    fn fetchspec(spec: &str) -> src_refspec::RefSpec {
        src::refspec::parse(spec.into(), src::refspec::parse::Operation::Fetch)
            .unwrap()
            .to_owned()
    }

    fn pushspec(spec: &str) -> src_refspec::RefSpec {
        src::refspec::parse(spec.into(), src::refspec::parse::Operation::Push)
            .unwrap()
            .to_owned()
    }

    fn base_dir(repo: &Repository) -> String {
        src_path::to_unix_separators_on_windows(src::path::into_bstr(
            src::path::realpath(repo.workdir().unwrap())
                .unwrap()
                .parent()
                .unwrap()
                .join("base"),
...
mod find_default_remote {
    ...
    fn works_on_detached_heads() -> crate::Result {
        ...
        assert_eq!(
            repo.find_default_remote(src::remote::Direction::Fetch)
                .transpose()?
                .expect("present")
                .name()
                .expect("always named")
