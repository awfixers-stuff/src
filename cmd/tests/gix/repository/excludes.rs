use src_worktree::stack::state::ignore::Source;
...
fn empty_core_excludes() -> crate::Result {
    ...
    let repo = src::open_opts(repo.git_dir(), repo.open_options().clone().strict_config(false))?;
...
fn missing_core_excludes_is_ignored() -> crate::Result {
    ...
    repo.config_snapshot_mut()
        .set_value(&src::config::tree::Core::EXCLUDES_FILE, "definitely-missing")?;
