...
mod object_database_impl {
    use src_object::{Exists, Find, FindHeader};

    #[test]
    fn empty_tree_is_always_present() -> crate::Result {
        let repo = crate::named_subrepo_opts("make_basic_repo.sh", "unborn", src::open::Options::isolated())?;
        let empty_tree = src::ObjectId::empty_tree(repo.object_hash());
...
mod edit_tree {
    use src::bstr::{BStr, BString};
    use src_object::tree::EntryKind;
...
",
    ...
    fn submodules_are_not_checked_for_existence() -> crate::Result {
        let repo = crate::named_subrepo_opts("make_submodules.sh", "with-submodules", src::open::Options::isolated())?
            .with_object_memory();
...
mod write_object {
    ...
    fn empty_tree() -> crate::Result {
        ...
        let oid = repo.write_object(src::objs::TreeRef::empty())?;
        assert_eq!(
            oid,
    ...
    fn commit_with_invalid_author() -> crate::Result {
        let repo = empty_bare_in_memory_repo()?;
        let actor = src::actor::Signature {
            name: "1 < 0".into(),
            email: Default::default(),
            time: Default::default(),
        };
        let commit = src::objs::Commit {
...
mod write_blob {
    ...
    fn from_slice() -> crate::Result {
        ...
        let mut other_repo = src::open_opts(repo.path(), src::open::Options::isolated())?;
...
fn writes_avoid_io_using_duplicate_check() -> crate::Result {
    ...
    let store = src::odb::loose::Store::at(repo.git_dir().join("objects"), repo.object_hash());
...
mod find {
    ...
    fn find_and_try_find_with_and_without_object_cache() -> crate::Result {
        ...
        for round in 1..=2 {
            ...
            for commit_id in repo.head()?.into_peeled_id()?.ancestors().all()? {
                ...
                assert_eq!(commit.id().object()?.kind, src_object::Kind::Commit);
                assert_eq!(commit.id().header()?.kind(), src_object::Kind::Commit);
                if round == 2 {
                    assert_eq!(
                        commit.id().object()?.kind,
                        src_object::Kind::Commit,
    ...
    fn empty_tree_can_always_be_found() -> crate::Result {
        ...
        assert_eq!(
            ...
            Some(src_odb::find::Header::Loose {
                kind: src_object::Kind::Tree,
                size: 0,
            }),
            "empty tree is considered a loose object"
    ...
    fn empty_blob_can_be_found_if_it_exists() -> crate::Result {
        let repo = basic_repo()?;
        let empty_blob = src::hash::ObjectId::empty_blob(repo.object_hash());
...
mod tag {
    ...
    fn simple() -> crate::Result {
        ...
        let tag_ref = repo.tag(
            ...
            message,
            src_ref::transaction::PreviousValue::MustNotExist,
        )?;
        ...
        assert_eq!(tag.target_kind, src_object::Kind::Commit);
        assert_eq!(
...
mod commit_as {
    ...
    fn specify_committer_and_author() -> crate::Result {
        ...
        let repo = src::ThreadSafeRepository::init_opts(
            &tmp,
        ...
        let committer = src::actor::Signature {
            name: "c".into(),
            email: "c@example.com".into(),
            time: src_date::parse_header("1 +0030").unwrap(),
        };
        let author = src::actor::Signature {
            name: "a".into(),
            email: "a@example.com".into(),
            time: src_date::parse_header("3 +0100").unwrap(),
        };
...
mod commit {
    ...
    fn parent_in_initial_commit_causes_failure() -> crate::Result {
        ...
        .to_thread_local();
        let empty_tree_id = repo.write_object(src::objs::Tree::empty())?.detach();
        let err = repo
    ...
    fn single_line_initial_commit_empty_tree_ref_nonexisting() -> crate::Result {
        ...
        .to_thread_local();
        let empty_tree_id = repo.write_object(src::objs::Tree::empty())?;
        let commit_id = repo.commit("HEAD", "initial", empty_tree_id, src::commit::NO_PARENT_IDS)?;
        assert_eq!(
            commit_id,
...
fn new_commit() -> crate::Result {
    ...
    config.set_value(&src::config::tree::User::NAME, "user")?;
    config.set_value(&src::config::tree::User::EMAIL, "user@example.com")?;
    config.commit()?;

    let empty_tree_id = repo.object_hash().empty_tree();
    let commit = repo.new_commit("initial", empty_tree_id, src::commit::NO_PARENT_IDS)?;
    let commit = commit.decode()?;
...
fn empty_bare_in_memory_repo() -> crate::Result<src::Repository> {
    Ok(named_subrepo_opts("make_basic_repo.sh", "bare.git", src::open::Options::isolated())?.with_object_memory())
}

fn empty_bare_repo() -> crate::Result<(tempfile::TempDir, src::Repository)> {
    let tmp = tempfile::tempdir()?;
    let repo = src::ThreadSafeRepository::init_opts(
        tmp.path(),
        src::create::Kind::Bare,
        src::create::Options::default(),
        src::open::Options::isolated(),
    )?
    .into();
    Ok((tmp, repo))
