...
fn on_root_with_decomposed_unicode() -> crate::Result {
    ...
    if precompose_unicode {
        assert!(
            matches!(
                src::utils::str::precompose_path(repo.git_dir().into()),
                Cow::Borrowed(_),
            ),
            "there is no change, as the path is already precomposed"
        );
        assert!(matches!(
            src::utils::str::precompose_path(work_dir.into()),
            Cow::Borrowed(_),
        ));
    } else {
        assert!(
...
fn non_bare_reftable() -> crate::Result {
    let repo = match named_subrepo_opts(
        ...
        Ok(r) => r,
        Err(_) if *src_testtools::GIT_VERSION < (2, 44, 0) => {
...
fn worktree_of_bare_repo() -> crate::Result {
    ...
    assert_eq!(repo.kind(), src::repository::Kind::LinkedWorkTree);
    Ok(())
}
...
fn non_bare_split_worktree() -> crate::Result {
    for (name, worktree_exists) in [
        ...
        let repo = named_subrepo_opts("make_worktree_repo.sh", name, src::open::Options::isolated())?;
        assert!(repo.git_dir().is_dir());
        assert!(
            !repo.is_bare(),
...
fn non_bare_split_worktree_invalid_worktree_path_boolean() -> crate::Result {
    let err = named_subrepo_opts(
        ...
        src::open::Options::isolated().strict_config(true),
    )
    .unwrap_err();
    assert_eq!(
...
fn non_bare_split_worktree_invalid_worktree_path_empty() -> crate::Result {
    ...
    .unwrap_err();
    assert!(
            matches!(err, src::open::Error::Config(src::config::Error::PathInterpolation{..})),
...
fn bare_with_worktree_is_still_bare() -> crate::Result {
    let repo = named_subrepo_opts("make_config_repos.sh", "bare-link", src::open::Options::isolated())?;
    assert!(
        repo.is_bare(),
...
mod missing_config_file {
    ...
    fn bare() -> crate::Result {
        let repo = named_subrepo_opts("make_config_repos.sh", "bare-no-config", src::open::Options::isolated())?;
        assert!(
            repo.is_bare(),
...
mod not_a_repository {
    ...
    fn shows_proper_error() -> crate::Result {
        for name in ["empty-dir", "with-files"] {
            ...
            let repo_path = src_testtools::scripted_fixture_read_only("make_config_repos.sh")?.join(name);
            let err = src::open_opts(&repo_path, src::open::Options::isolated()).unwrap_err();
            assert!(matches!(err, src::open::Error::NotARepository { path, .. } if path == repo_path));
        }
        Ok(())
    }
}
...
mod open_path_as_is {
    ...
    fn open_path_as_is() -> src::open::Options {
        src::open::Options::isolated().open_path_as_is(true)
    }
    ...
    fn worktrees_cannot_be_opened() -> crate::Result {
        ...
        assert!(matches!(err, src::open::Error::NotARepository { .. }));
        Ok(())
    }
...
mod submodules {
    ...
    fn by_their_worktree_checkout_and_git_modules_dir() {
        let dir = src_testtools::scripted_fixture_read_only("make_submodules.sh").unwrap();
        ...
        for module in ["m1", "dir/m1"] {
            ...
            for discover_dir in [
                ...
                assert_eq!(repo.kind(), src::repository::Kind::Submodule);
                assert_eq!(repo.workdir().expect("non-bare"), dir.join(&submodule_m1_workdir));
                assert_eq!(repo.git_dir(), dir.join(&submodule_m1_gitdir));

                let repo = src::open_opts(repo.workdir().expect("non-bare"), src::open::Options::isolated()).unwrap();
                assert_eq!(repo.kind(), src::repository::Kind::Submodule);
                assert_eq!(repo.workdir().expect("non-bare"), dir.join(&submodule_m1_workdir));
                assert_eq!(repo.git_dir(), dir.join(&submodule_m1_gitdir));
            }
    ...
    fn discover_repo(name: impl AsRef<Path>) -> crate::Result<src::Repository> {
        let dir = src_testtools::scripted_fixture_read_only("make_submodules.sh")?;
        let repo_dir = dir.join(name);
        Ok(src::ThreadSafeRepository::discover_opts(
            repo_dir,
            Default::default(),
            src_sec::trust::Mapping {
...
mod object_caches {
    ...
    fn default_git_and_custom_caches() -> crate::Result {
        let opts = src::open::Options::isolated();
...
mod worktree {
    ...
    #[test]
    fn with_worktree_configs() -> src_testtools::Result {
        let manifest_dir = std::path::PathBuf::from(std::env::var("CARGO_MANIFEST_DIR")?);
        let fixture_dir = src_testtools::scripted_fixture_read_only("make_worktree_repo_with_configs.sh")?;
