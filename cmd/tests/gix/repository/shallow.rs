...
fn shallow_ids(repo: &src::Repository) -> crate::Result<Vec<src::ObjectId>> {
...
mod traverse {
    ...
    fn boundary_is_detected_triggering_no_error() -> crate::Result {
        for sorting in [
            ...
            src::revision::walk::Sorting::ByCommitTime(CommitTimeOrder::NewestFirst),
    ...
    fn complex_graphs_can_be_iterated_despite_multiple_shallow_boundaries() -> crate::Result {
        let base = src_path::realpath(src_testtools::scripted_fixture_read_only("make_remote_repos.sh")?.join("base"))?;
        let shallow_base = src_testtools::scripted_fixture_read_only_with_args_single_archive(
            "make_complex_shallow_repo.sh",
            Some(base.to_string_lossy()),
        )?;
        for toggle in [false, true] {
            for name in ["shallow.git", "shallow"] {
                let repo = src::open_opts(shallow_base.join(name), crate::restricted())?;
                assert_eq!(
                ...
                let commits: Vec<_> = repo
                    ...
                    .ancestors()
                    .use_commit_graph(toggle)
                    .sorting(src::revision::walk::Sorting::ByCommitTime(CommitTimeOrder::NewestFirst))
                    .all()?
                    .map(|c| c.map(|c| c.id))
                    .collect::<Result<_, _>>()?;
                assert_eq!(
