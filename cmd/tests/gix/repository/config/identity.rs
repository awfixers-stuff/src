...
fn author_included_by_hasconfig() -> crate::Result {
    let repo = named_subrepo_opts("make_config_repos.sh", "with-hasconfig", src::open::Options::isolated())?;
    let _env = Env::new()
        .set(
            "GIT_CONFIG_SYSTEM",
    ...
    let repo = src::open_opts(repo.git_dir(), allow_system_options(repo.open_options().clone()))?;
...
fn author_and_committer_and_fallback() -> crate::Result {
    for trust in [src_sec::Trust::Full, src_sec::Trust::Reduced] {
    ...
    let repo = src::open_opts(
        ...
        allow_system_options(
            repo.open_options()
                ...
                .with(src_sec::Trust::Full),
        ),
    )?;

    assert_eq!(
        repo.author().transpose()?,
        Some(src_actor::SignatureRef {
...
fn allow_system_options(opts: src::open::Options) -> src::open::Options {
    opts.permissions(src::open::Permissions {
