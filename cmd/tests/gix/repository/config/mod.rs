...
mod ssh_options {
    ...
    fn with_command_and_variant() -> crate::Result {
        ...
        assert_eq!(
            opts.kind,
            Some(src::protocol::transport::client::blocking_io::ssh::ProgramKind::Ssh)
        );
    ...
    fn with_command_fallback_which_disallows_shell() -> crate::Result {
        ...
        assert_eq!(
            opts.kind,
            Some(src::protocol::transport::client::blocking_io::ssh::ProgramKind::Putty)
        );
        assert!(
            opts.disallow_shell,
...
pub fn repo(name: &str) -> src::Repository {
    repo_opts(name, |opts| opts.strict_config(true))
}

pub fn repo_opts(name: &str, modify: impl FnOnce(src::open::Options) -> src::open::Options) -> src::Repository {
    let dir = src_testtools::scripted_fixture_read_only("make_config_repos.sh").unwrap();
    src::open_opts(dir.join(name), modify(src::open::Options::isolated())).unwrap()
}
