...
fn remote_and_branch_names() {
    ...
    assert_eq!(repo.remote_default_name(src::remote::Direction::Fetch), None);
    assert_eq!(repo.remote_default_name(src::remote::Direction::Push), None);
    ...
    assert_eq!(
        repo.remote_default_name(src::remote::Direction::Fetch),
        Some(remote_name("origin"))
    );
    assert_eq!(
        repo.remote_default_name(src::remote::Direction::Push),
        Some(remote_name("origin"))
    );
...
mod branch_remote {
    use src::{config::tree::Push, remote};
    ...
    fn repo(name: &str) -> Result<src::Repository, src::open::Error> {
        named_subrepo_opts("make_remote_config_repos.sh", name, src::open::Options::isolated())
    }
}
