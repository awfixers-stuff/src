...
mod http {
    use src_transport::client::blocking_io::http::options::{
    ...
    fn http_options(
        ...
        url: &str,
    ) -> src_transport::client::blocking_io::http::Options {
        let opts = repo
            .transport_options(url, remote_name.map(Into::into))
            .expect("valid configuration")
            .expect("configuration available for http");
        opts.downcast_ref::<src_transport::client::blocking_io::http::Options>()
    ...
    fn remote_overrides() {
        ...
        let src_transport::client::blocking_io::http::Options {
            proxy,
            proxy_auth_method,
            follow_redirects,
            ..
    ...
    fn simple_configuration() {
        ...
        #[cfg(feature = "blocking-http-transport-curl")]
        {
            ...
            match backend.downcast_ref::<src_protocol::transport::client::blocking_io::http::curl::Options>() {
                Some(opts) => {
