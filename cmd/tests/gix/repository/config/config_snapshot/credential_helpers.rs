...
mod baseline {
    ...
    pub fn works_but_we_dont_parse_invalid_url(url: &str) {
        assert!(src::url::parse(url.into()).is_err(), "{url:?} should not be parseable");
        assert!(
    ...
    fn agrees_with_inner(url: &str, ignore_expected_prompt_port: bool, lowercase_prompt_host: bool) {
        ...
        let (cascade, mut action, prompt_options) = repo
            .config_snapshot()
            .credential_helpers(src::url::parse(url.into()).expect("valid input URL"))
            .unwrap();

        assert_ne!(
            prompt_options.mode,
            src_prompt::Mode::Disable,
        ...
        let actual_helpers: Vec<BString> = cascade
            ...
            .iter()
            .map(|p| match &p.kind {
                src_credentials::program::Kind::ExternalName { name_and_args } => name_and_args.to_owned(),
