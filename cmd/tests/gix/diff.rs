use src_diff::blob::{Algorithm, Driver};
...
fn resource_cache() -> crate::Result {
    ...
    let cache = src::diff::resource_cache(
        &repo,
        src::diff::blob::pipeline::Mode::ToWorktreeAndBinaryToText,
        repo.attributes_only(&index, src_worktree::stack::state::attributes::Source::IdMapping)?
            .detach(),
        Default::default(),
    )?;
    assert_eq!(
