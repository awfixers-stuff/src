...
mod keys {
    ...
    use src::config::tree::{Key, Section};
    use src_object::bstr::ByteSlice;
    ...
    fn string() -> crate::Result {
        assert_eq!(
            src::config::tree::Http::USER_AGENT.try_into_string(bcow("agent"))?,
            "agent"
        );
        assert!(src::config::tree::Http::USER_AGENT.validate("agent".into()).is_ok());

        let invalid = b"\xF0\x80\x80".as_bstr();
        assert_eq!(
            src::config::tree::Http::USER_AGENT
                .try_into_string(Cow::Borrowed(invalid))
                .unwrap_err()
                .to_string(),
            "The utf-8 string at \"http.userAgent=���\" could not be decoded"
        );
        assert!(src::config::tree::Http::USER_AGENT.validate(invalid).is_err());
    ...
    fn any() {
        assert!(
            !src::config::Tree.sections().is_empty(),
            "the root has at least one section"
        );
        assert_eq!(src::config::Tree::AUTHOR.name(), "author");
        assert_eq!(src::config::tree::Author.keys().len(), 2);
        assert_eq!(src::config::tree::Author::NAME.name(), "name");
        assert_eq!(src::config::tree::Author::EMAIL.name(), "email");
        assert_eq!(
            src::config::tree::Author::NAME
    ...
    #[test]
    fn remote_name() {
        assert!(src::config::tree::Remote::PUSH_DEFAULT
    ...
    fn unsigned_integer() {
        for valid in [0, 1, 100_124] {
            assert!(src::config::tree::Core::DELTA_BASE_CACHE_LIMIT
        ...
        for invalid in [-1, -100] {
            assert_eq!(
                src::config::tree::Core::DELTA_BASE_CACHE_LIMIT
...
mod branch {
    use src::config::tree::{branch, Branch, Key};
...
mod ssh {
    ...
    fn variant() -> crate::Result {
        use src::config::tree::Ssh;
        use src_protocol::transport::client::blocking_io::ssh::ProgramKind;
...
#[cfg(feature = "status")]
mod status {
    use src::{config::tree::Status, status::UntrackedFiles};
...
mod fetch {
    ...
    fn recurse_submodule() -> crate::Result {
        ...
        for (actual, expected) in [
            ("true", src_submodule::config::FetchRecurse::Always),
            ("false", src_submodule::config::FetchRecurse::Never),
            ("on-demand", src_submodule::config::FetchRecurse::OnDemand),
        ] {
            assert_eq!(
                Fetch::RECURSE_SUBMODULES.try_into_recurse_submodules(
                    src_config::Boolean::try_from(actual.as_bytes().as_bstr()).map(|b| b.0)
                )?,
                expected
            );
        ...
        assert_eq!(
            Fetch::RECURSE_SUBMODULES
                .try_into_recurse_submodules(src_config::Boolean::try_from(b"foo".as_bstr()).map(|b| b.0))
                .unwrap_err()
                .to_string(),
...
mod diff {
    ...
    fn renames() -> crate::Result {
        ...
        assert_eq!(
            Diff::RENAMES.try_into_renames(Err(src_config::value::Error::new("err", "copy")))?,
            Tracking::RenamesAndCopies
        );
        assert!(Diff::RENAMES.validate("copy".into()).is_ok());
        assert_eq!(
            Diff::RENAMES.try_into_renames(Err(src_config::value::Error::new("err", "copies")))?,
            Tracking::RenamesAndCopies
        );
        ...
        assert_eq!(
            Diff::RENAMES
                .try_into_renames(Err(src_config::value::Error::new("err", "foo")))
                .unwrap_err()
                .to_string(),
...
#[cfg(feature = "merge")]
mod merge {
    use src::config::tree::{Key, Merge};
    use src_merge::blob::builtin_driver::text::ConflictStyle;
...
mod core {
    ...
    use src::config::tree::{Core, Key};
    use src_lock::acquire::Fail;

    use crate::config::tree::bcow;

    fn signed(value: i64) -> Result<i64, src_config::value::Error> {
        Ok(value)
    }
    ...
    fn timeouts() -> crate::Result {
        ...
        assert_eq!(
            Core::FILES_REF_LOCK_TIMEOUT
                .try_into_lock_timeout(Err(src_config::value::Error::new("err", "bogus")))
                .unwrap_err()
                .to_string(),
    ...
    fn disambiguate() -> crate::Result {
        use src::revision::spec::parse::ObjectKindHint;
    ...
    fn abbrev() -> crate::Result {
        let object_hash = src_hash::Kind::Sha1;
    ...
    fn delta_base_cache_limit() -> crate::Result {
        ...
        assert_eq!(
            Core::DELTA_BASE_CACHE_LIMIT
                ...
                .unwrap_err()
                .to_string(),
            "The value of key \"core.deltaBaseCacheLimit\" (possibly from src_PACK_CACHE_MEMORY) could not be parsed as unsigned integer"
        );
    ...
    fn safecrlf() -> crate::Result {
        for (value, expected) in [
            ("false", src_filter::pipeline::CrlfRoundTripCheck::Skip),
            ("true", src_filter::pipeline::CrlfRoundTripCheck::Fail),
            ("warn", src_filter::pipeline::CrlfRoundTripCheck::Warn),
        ] {
    ...
    fn autocrlf() -> crate::Result {
        for (value, expected) in [
            ("false", src_filter::eol::AutoCrlf::Disabled),
            ("true", src_filter::eol::AutoCrlf::Enabled),
            ("input", src_filter::eol::AutoCrlf::Input),
        ] {
    ...
    fn eol() -> crate::Result {
        for (value, expected) in [
            ("lf", src_filter::eol::Mode::Lf),
            ("crlf", src_filter::eol::Mode::CrLf),
            ("native", src_filter::eol::Mode::default()),
        ] {
    ...
    fn check_round_trip_encoding() -> crate::Result {
        for (value, expected) in [
            (
                Some("UTF-8 utf-16BE"),
                &[src_filter::encoding::UTF_8, src_filter::encoding::UTF_16BE][..],
            ),
            (
                Some("SHIFT-JIS,UTF-8"),
                &[src_filter::encoding::SHIFT_JIS, src_filter::encoding::UTF_8],
            ),
            (
                Some("UTF-16LE, SHIFT-JIS"),
                &[src_filter::encoding::UTF_16LE, src_filter::encoding::SHIFT_JIS],
            ),
            (None, &[src_filter::encoding::SHIFT_JIS]),
        ] {
            assert_eq!(
...
mod extensions {
    use src::config::tree::{Extensions, Key};
...
mod checkout {
    use src::config::tree::{Checkout, Key};

    fn int(value: i64) -> Result<i64, src_config::value::Error> {
        Ok(value)
    }
...
mod protocol {
    use src::config::tree::{Key, Protocol};

    #[cfg(any(feature = "blocking-network-client", feature = "async-network-client"))]
    #[test]
    fn allow() -> crate::Result {
        use src::{config::tree::protocol, remote::url::scheme_permission::Allow};
    ...
    fn version() {
        ...
        #[cfg(any(feature = "blocking-network-client", feature = "async-network-client"))]
        {
            for (valid, expected) in [
                (None, src_protocol::transport::Protocol::V2),
                (Some(0), src_protocol::transport::Protocol::V0),
                (Some(1), src_protocol::transport::Protocol::V1),
                (Some(2), src_protocol::transport::Protocol::V2),
            ] {
                assert_eq!(
                    Protocol::VERSION
...
mod gitoxide {
    mod http {
        ...
        use src::config::tree::{gitoxide, Key};
...
mod http {
    ...
    fn follow_redirects() -> crate::Result {
        use src_transport::client::blocking_io::http::options::FollowRedirects;
        assert_eq!(
        ...
        assert_eq!(
            Http::FOLLOW_REDIRECTS
                .try_into_follow_redirects(bcow("something"), || Err(src_config::value::Error::new(
                    "invalid", "value"
                )))
                .unwrap_err()
                .to_string(),
    ...
    fn http_version() -> crate::Result {
        use src_transport::client::blocking_io::http::options::HttpVersion;
    ...
    fn ssl_version() -> crate::Result {
        use src_transport::client::blocking_io::http::options::SslVersion::*;
    ...
    fn proxy_auth_method() -> crate::Result {
        use src_transport::client::blocking_io::http::options::ProxyAuthMethod::*;
        for (actual, expected) in [
            ("anyauth", AnyAuth),
            ("basic", Basic),
            ("digest", Digest),
...
mod remote {
    ...
    fn refspecs() {
        ...
        assert!(Remote::FETCH
            .try_into_refspec(bcow(fetch_spec), src_refspec::parse::Operation::Fetch)
            .is_ok());
        assert!(Remote::FETCH.validate(fetch_spec.into()).is_ok());

        let push_spec = "HEAD:refs/heads/name";
        assert!(Remote::PUSH
            .try_into_refspec(bcow(push_spec), src_refspec::parse::Operation::Push)
            .is_ok());
        assert!(Remote::PUSH.validate(push_spec.into()).is_ok());

        assert_eq!(
            Remote::FETCH
                .try_into_refspec(bcow("*/*/*:refs/heads/*"), src_refspec::parse::Operation::Fetch)
                .unwrap_err()
                .to_string(),
        ...
        assert_eq!(
            Remote::PUSH
                .try_into_refspec(bcow("*/*/*:refs/heads/*"), src_refspec::parse::Operation::Push)
                .unwrap_err()
                .to_string(),
