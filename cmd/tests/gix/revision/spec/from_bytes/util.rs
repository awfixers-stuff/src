...
use src_object::{bstr, bstr::BStr};
use src_ref::bstr::{BString, ByteSlice};
use src_revision::spec::Kind;
use std::sync::LazyLock;

const FIXTURE_NAME: &str = "make_rev_spec_parse_repos.sh";
static BASELINE: LazyLock<HashMap<PathBuf, HashMap<BString, Option<src_revision::Spec>>>> = LazyLock::new(|| {
    fn kind_of(spec: &BStr) -> src_revision::spec::Kind {
        if spec.starts_with(b"^") {
            src_revision::spec::Kind::IncludeReachable
        } else if spec.contains_str(b"...") {
            src_revision::spec::Kind::ReachableToMergeBase
        } else if spec.contains_str(b"..") {
            src_revision::spec::Kind::RangeBetween
        } else if spec.ends_with(b"^!") {
            src_revision::spec::Kind::ExcludeReachableFromParents
        } else if spec.ends_with(b"^@") {
    ...
    fn lines_of(kind: src_revision::spec::Kind) -> Option<usize> {
        Some(match kind {
    ...
    fn object_id_of_next(lines: &mut std::iter::Peekable<bstr::Lines<'_>>) -> src_hash::ObjectId {
        let hex_hash = lines.next().expect("valid respect yields enough lines");
        object_id_of(hex_hash).expect("git yields full objects ids")
    }
    fn object_id_of(input: &[u8]) -> Option<src_hash::ObjectId> {
        let hex_hash = input.strip_prefix(b"^").unwrap_or(input);
        src_hash::ObjectId::from_str(hex_hash.to_str().expect("hex is ascii")).ok()
    }
    let mut baseline_map = HashMap::new();
    let base = src_testtools::scripted_fixture_read_only(FIXTURE_NAME).unwrap();
        ...
        .filter_map(|e| e.ok().and_then(|e| (e.file_name() == "baseline.git").then_some(e)))
    {
        ...
        while let Some(spec) = lines.next() {
            ...
            let first_hash = match u8::from_str(exit_code_or_hash) {
                ...
                Err(_) => match src::ObjectId::from_str(exit_code_or_hash) {
                    Ok(hash) => hash,
            ...
            let rev_spec = match num_lines {
                Some(line_count) => match line_count {
                    1 if kind == src_revision::spec::Kind::IncludeReachable => src_revision::Spec::Include(first_hash),
                    1 if kind == src_revision::spec::Kind::ExcludeReachable => src_revision::Spec::Exclude(first_hash),
                    2 | 3 => {
                        let second_hash = object_id_of_next(&mut lines);
                        if line_count == 2 {
                            src_revision::Spec::Range {
                                from: second_hash,
                                to: first_hash,
                            }
                        } else {
                            lines.next().expect("merge-base to consume");
                            src_revision::Spec::Merge {
                                theirs: first_hash,
                                ours: second_hash,
                            }
                        }
                ...
                None => {
                    let rev_spec = match kind {
                        src_revision::spec::Kind::ExcludeReachableFromParents => {
                            src_revision::Spec::ExcludeParents(first_hash)
                        }
                        _ => unreachable!(),
                    };
...
pub fn parse_spec_no_baseline<'a>(
    spec: &str,
    repo: &'a src::Repository,
) -> Result<src::revision::Spec<'a>, src_error::Error> {
...
pub fn parse_spec_better_than_baseline<'a>(
    ...
    let res = src::revision::Spec::from_bstr(spec, repo, Default::default());
...
pub fn parse_spec_no_baseline_opts<'a>(
    spec: &str,
    repo: &'a src::Repository,
    opts: src::revision::spec::parse::Options,
) -> Result<src::revision::Spec<'a>, src_error::Error> {
    src::revision::Spec::from_bstr(spec, repo, opts)
}
...
pub fn parse_spec_opts<'a>(
    ...
    let res = src::revision::Spec::from_bstr(spec, repo, opts);
    compare_with_baseline(&res, repo, spec, BaselineExpectation::Same);
    res
}

pub fn rev_parse<'a>(spec: &str, repo: &'a src::Repository) -> Result<src::revision::Spec<'a>, src_error::Error> {
...
fn compare_with_baseline(
    res: &Result<src::revision::Spec<'_>, src_error::Error>,
    repo: &src::Repository,
    spec: &str,
...
pub fn parse_spec(spec: impl AsRef<str>, repo: &src::Repository) -> Result<src::revision::Spec<'_>, src_error::Error> {
    parse_spec_opts(spec.as_ref(), repo, Default::default())
}

pub fn repo(name: &str) -> crate::Result<src::Repository> {
    let base = src_testtools::scripted_fixture_read_only(FIXTURE_NAME)?;
    Ok(src::open(base.join(name))?)
}
