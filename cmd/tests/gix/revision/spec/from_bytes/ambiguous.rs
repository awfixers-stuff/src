...
fn ranges_are_auto_disambiguated_by_committish() {
    ...
    let expected = src_revision::Spec::Range { from: id, to: id }.attach(&repo);
    ...
    let expected = src_revision::Spec::Merge { theirs: id, ours: id }.attach(&repo);
    for spec in ["000000000...000000000", "...000000000", "000000000..."] {
...
fn trees_can_be_disambiguated_by_blob_access() {
    ...
    assert_eq!(
        ...
        ("a0blgqsjc".into(), src_object::tree::EntryKind::Blob.into())
    );
}
...
fn repository_local_disambiguation_hints_disambiguate() {
...

    {
        ...
        let expected = src_revision::Spec::Range { from: id, to: id }.attach(&r);
        assert_eq!(
