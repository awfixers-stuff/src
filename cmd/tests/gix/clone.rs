...
mod blocking_io {
    ...
    use src_ref::TargetRef;
    use src_refspec::parse::Operation;

    fn shallow_ids(repo: &src::Repository, expected: &'static str) -> crate::Result<Vec<src::ObjectId>> {
    ...
    fn fetch_shallow_no_checkout_then_unshallow() -> crate::Result {
        let tmp = src_testtools::tempfile::TempDir::new()?;
        let called_configure_remote = std::sync::Arc::new(std::sync::atomic::AtomicBool::default());
        let remote_name = "special";
        let desired_fetch_tags = src::remote::fetch::Tags::Included;
        let mut prepare = src::prepare_clone_bare(remote::repo("base").path(), tmp.path())?
        ...
        let (repo, _out) = prepare.fetch_only(src::progress::Discard, &std::sync::atomic::AtomicBool::default())?;
        drop(prepare);
        ...
        remote
            ...
            .prepare_fetch(src::progress::Discard, Default::default())?
            .with_shallow(Shallow::undo())
            .receive(src::progress::Discard, &AtomicBool::default())?;
    ...
    fn shallow_clone_uses_single_branch_refspec() -> crate::Result {
        let tmp = src_testtools::tempfile::TempDir::new()?;
        let (repo, _out) = src::prepare_clone_bare(remote::repo("base").path(), tmp.path())?
            .with_shallow(Shallow::DepthAtRemote(1.try_into()?))
            .fetch_only(src::progress::Discard, &std::sync::atomic::AtomicBool::default())?;
    ...
    fn from_shallow_prohibited_with_option() -> crate::Result {
        let tmp = src_testtools::tempfile::TempDir::new()?;
        let err = src::clone::PrepareFetch::new(
            remote::repo("base.shallow").path(),
            tmp.path(),
            src::create::Kind::Bare,
            Default::default(),
            src::open::Options::isolated().config_overrides([Clone::REJECT_SHALLOW.validated_assignment_fmt(&true)?]),
        )?
        .fetch_only(src::progress::Discard, &std::sync::atomic::AtomicBool::default())
        .unwrap_err();
        assert!(
            matches!(
                err,
                src::clone::fetch::Error::Fetch(src::remote::fetch::Error::Fetch(
                    src_protocol::fetch::Error::RejectShallowRemote
                ))
            ),
    ...
    fn from_shallow_allowed_by_default() -> crate::Result {
        let tmp = src_testtools::tempfile::TempDir::new()?;
        let (repo, _change) = src::prepare_clone_bare(remote::repo("base.shallow").path(), tmp.path())?
        ...
        assert_eq!(
            src::open_opts(repo.git_dir(), src::open::Options::isolated())?
                .config_snapshot()
                .boolean("my.marker"),
            None,
    ...
    fn from_non_shallow_then_deepen_then_deepen_since_to_unshallow() -> crate::Result {
        let tmp = src_testtools::tempfile::TempDir::new()?;
        let (repo, _change) = src::prepare_clone_bare(remote::repo("base").path(), tmp.path())?
        ...
        remote
            ...
            .with_shallow(Shallow::Since {
                cutoff: src::date::Time::new(1112354053, 0),
            })
            .receive(src::progress::Discard, &AtomicBool::default())?;
    ...
    fn from_non_shallow_by_deepen_exclude_then_deepen_to_unshallow() -> crate::Result {
        ...
        let (repo, _change) = src::prepare_clone_bare(remote::repo("base").path(), tmp.path())?
            .with_fetch_options(src::remote::ref_map::Options {
                extra_refspecs: vec![src::refspec::parse(
    ...
    fn fetch_only_with_configuration() -> crate::Result {
        ...
        let remote_name = "special";
        let desired_fetch_tags = src::remote::fetch::Tags::Included;
        let mut prepare = src::clone::PrepareFetch::new(
            remote::repo("base").path(),
            tmp.path(),
            src::create::Kind::Bare,
            Default::default(),
            src::open::Options::isolated().config_overrides([
        ...
        .configure_remote({
            ...
            move |r| {
                called_configure_remote.store(true, std::sync::atomic::Ordering::Relaxed);
                let r = r
                    .with_refspecs(Some("+refs/tags/b-tag:refs/tags/b-tag"), src::remote::Direction::Fetch)?
                    .with_fetch_tags(desired_fetch_tags);
                Ok(r)
            }
        });
        let (repo, out) = prepare.fetch_only(src::progress::Discard, &std::sync::atomic::AtomicBool::default())?;
        drop(prepare);
        ...
        let num_refspecs = remote.refspecs(src::remote::Direction::Fetch).len();
        assert_eq!(
            num_refspecs, 2,
        ...
        assert!(
            src::path::from_bstr(Cow::Borrowed(
                remote
                    .url(src::remote::Direction::Fetch)
                    .expect("present")
                    .path
                    .as_ref()
            ))
        ...
        match out.status {
            src::remote::fetch::Status::Change { update_refs, .. } => {
                for edit in &update_refs.edits {
                    ...
                    if r.name().category().expect("known") != src_ref::Category::Tag {
                        assert!(r
                            .name()
                ...
                for mapping in update_refs
                    ...
                    .filter(|(_, update)| {
                        matches!(
                            update.mode,
                            src::remote::fetch::refs::update::Mode::ImplicitTagNotSentByRemote
                        )
                    })
        ...
        assert_eq!(
            referent
                .remote_name(src::remote::Direction::Fetch)
        ...
        assert_eq!(
            repo.branch_remote_ref_name(ref_name, src::remote::Direction::Fetch)
                .expect("present")?
                .as_bstr(),
            "refs/heads/main"
        );
    ...
    fn assert_reflog(log: std::io::Result<Option<src_ref::file::log::iter::Forward<'_>>>) {
        let lines = log
            .unwrap()
            .expect("log present")
        ...
        let path = src_path::from_bstr(line.message.rsplit(|b| *b == b' ').next().expect("path").as_bstr());
    ...
    fn fetch_and_checkout() -> crate::Result {
        ...
        let mut prepare = src::clone::PrepareFetch::new(
            remote::repo("base").path(),
            tmp.path(),
            src::create::Kind::WithWorktree,
            Default::default(),
            restricted(),
        )?;
        let (mut checkout, _out) =
            prepare.fetch_then_checkout(src::progress::Discard, &std::sync::atomic::AtomicBool::default())?;
        let (repo, _) = checkout.main_worktree(src::progress::Discard, &std::sync::atomic::AtomicBool::default())?;
    ...
    fn fetch_and_checkout_specific_ref() -> crate::Result {
        ...
        assert_eq!(
            checked_out_ref
                .remote_ref_name(src::remote::Direction::Fetch)
                .transpose()?
                .unwrap()
                .as_bstr(),
            remote_ref_name,
    ...
    fn fetch_and_checkout_specific_non_existing() -> crate::Result {
        ...
        let err = prepare
            .fetch_then_checkout(src::progress::Discard, &std::sync::atomic::AtomicBool::default())
            .unwrap_err();
        assert_eq!(
            err.to_string(),
    ...
    fn fetch_succeeds_despite_remote_head_ref() -> crate::Result {
        ...
        let (mut checkout, _out) = prepare.fetch_then_checkout(src::progress::Discard, &AtomicBool::default())?;
        let (repo, _) = checkout.main_worktree(src::progress::Discard, &AtomicBool::default())?;
    ...
    fn assure_index_entries_on_disk(index: &src::worktree::Index, work_dir: &Path) {
        for entry in index.entries() {
            let entry_path = work_dir.join(src_path::from_bstr(entry.path(index)));
    ...
    fn fetch_and_checkout_empty_remote_repo() -> crate::Result {
        for version in [
            src::protocol::transport::Protocol::V0,
            src::protocol::transport::Protocol::V2,
        ] {
            let tmp = src_testtools::tempfile::TempDir::new()?;
            let mut prepare = src::clone::PrepareFetch::new(
                src_testtools::scripted_fixture_read_only("make_empty_repo.sh")?,
                tmp.path(),
                src::create::Kind::WithWorktree,
                Default::default(),
            ...
            let (mut checkout, out) =
                prepare.fetch_then_checkout(src::progress::Discard, &std::sync::atomic::AtomicBool::default())?;
            let (repo, _) =
                checkout.main_worktree(src::progress::Discard, &std::sync::atomic::AtomicBool::default())?;
    ...
    fn fetch_only_without_configuration() -> crate::Result {
        let tmp = src_testtools::tempfile::TempDir::new()?;
        let (repo, out) = src::clone::PrepareFetch::new(
            remote::repo("base").path(),
            tmp.path(),
            src::create::Kind::Bare,
            Default::default(),
        ...
        match out.status {
            src::remote::fetch::Status::Change { write_pack_bundle, .. } => {
                assert!(
...
fn clone_and_early_persist_without_receive() -> crate::Result {
    let tmp = src_testtools::tempfile::TempDir::new()?;
    let repo = src::clone::PrepareFetch::new(
        remote::repo("base").path(),
        tmp.path(),
        src::create::Kind::Bare,
        Default::default(),
    ...
    assert_eq!(repo.kind(), src::repository::Kind::Common);
    Ok(())
}
...
fn clone_and_destination_must_be_empty() -> crate::Result {
    ...
    match src::clone::PrepareFetch::new(
        remote::repo("base").path(),
        tmp.path(),
        src::create::Kind::Bare,
        Default::default(),
...
fn clone_bare_into_empty_directory_and_early_drop() -> crate::Result {
    ...
    let prep = src::clone::PrepareFetch::new(
        remote::repo("base").path(),
        tmp.path(),
        src::create::Kind::Bare,
        Default::default(),
