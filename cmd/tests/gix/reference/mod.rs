...
mod log {
    ...
    fn message() {
        assert_eq!(
            src::reference::log::message("commit", "the subject\n\nthe body".into(), 0),
            "commit (initial): the subject"
        );
        assert_eq!(
            src::reference::log::message("other", "the subject".into(), 1),
            "other: the subject"
        );

        assert_eq!(
            src::reference::log::message("rebase", "the subject".into(), 2),
...
mod find {
    use src_ref::{FullName, FullNameRef, Target, TargetRef};

    use crate::util::hex_to_id;

    fn repo() -> crate::Result<src::Repository> {
    ...
    fn and_peel() -> crate::Result {
        ...
        let obj = tag_ref.peel_to_kind(src::object::Kind::Tag)?;
        assert_eq!(tag_ref.peel_to_tag()?.id, obj.id);
        assert_eq!(obj.kind, src::object::Kind::Tag);
        assert_eq!(
        ...
        let err = tag_ref.peel_to_kind(src::object::Kind::Blob).unwrap_err();
        ...
        let obj = tag_ref.peel_to_kind(src::object::Kind::Tree)?;
        ...
        let obj = tag_ref.peel_to_kind(src::object::Kind::Commit)?;
