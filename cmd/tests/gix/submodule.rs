...
mod open {
        ...
        for (name, expected) in [
                ...
                repo.submodules()?.expect("modules present").zip(expected)
            {
                ...
                for check_dirty in [false, true] {
                    let status = match sm.status(src::submodule::config::Ignore::None, check_dirty) {
                        Ok(status) => status,
                        Err(err) => {
                            assert_eq!(
    ...
    mod status {
        ...
        fn changed_head_compared_to_superproject_index() -> crate::Result {
            ...
            let mut status = sm.status(src::submodule::config::Ignore::None, false)?;
            assert_eq!(
                status.is_dirty(),
                Some(true),
            ...
            let status_with_ignore = sm.status(src::submodule::config::Ignore::Dirty, false)?;
            assert_eq!(
                status_with_ignore, status,
                "The lowest status that makes these changes observable"
            );

            let status_with_ignore_check_only = sm.status(src::submodule::config::Ignore::Dirty, true)?;
            assert_eq!(
                status_with_ignore_check_only, status,
                "dirty-check has no observable influence here yet as there no 'more expensive' changes"
            );

            let status_with_ignore = sm.status(src::submodule::config::Ignore::All, false)?;
            assert_eq!(
        ...
        fn modified_in_index_only() -> crate::Result {
            let mut repo: src::Repository = repo("submodule-index-changed")?;
            let sm = repo.submodules()?.into_iter().flatten().next().expect("one submodule");

            for mode in [
                src::submodule::config::Ignore::Untracked,
                src::submodule::config::Ignore::None,
            ] {
                for check_dirty in [false, true] {
            ...
            repo.config_snapshot_mut()
                .set_value(&src::config::tree::Diff::IGNORE_SUBMODULES, "all")?;
            ...
            let sm = repo.submodules()?.into_iter().flatten().next().expect("one submodule");
            let status = sm.status_opts(src::submodule::config::Ignore::None, true, &mut |platform| platform)?;
            assert_eq!(
                status.is_dirty(),
                Some(true),
        ...
        fn modified_and_untracked() -> crate::Result {
            ...
            let status = sm.status(src::submodule::config::Ignore::Dirty, false)?;
            assert_eq!(status.is_dirty(), Some(false), "Dirty skips worktree changes entirely");

            let mut status = sm.status_opts(
                src::submodule::config::Ignore::None,
                false,
                &mut |status: src::status::Platform<'_, src::progress::Discard>| {
                    status.index_worktree_options_mut(|opts| {
                        opts.sorting = Some(src_status::index_as_worktree_with_renames::Sorting::ByPathCaseSensitive);
                    })
                },
            )?;
            assert_eq!(
            ...
            let status = sm.status(src::submodule::config::Ignore::Untracked, false)?;
            assert_eq!(
                status.is_dirty(),
                Some(true),
        ...
        fn changed_head_empty_worktree() -> crate::Result {
            ...
            let status = sm.status(src::submodule::config::Ignore::None, false)?;
            assert_eq!(
                status.state,
                src::submodule::State {
                    repository_exists: true,
        ...
        fn is_dirty_skips_expensive_checks() -> crate::Result {
            ...
            let status = sm.status(src::submodule::config::Ignore::None, true)?;
            assert_eq!(
                status.changes, None,
