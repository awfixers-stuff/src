pub fn submodule_repo(name: &str) -> crate::Result<src::Repository> {
...
mod into_iter {
    use src::status::{tree_index::TrackRenames, Item, Submodule};
    use src_diff::Rewrites;
    use src_testtools::size_ok;
    ...
    fn submodule_tree_index_modification() -> crate::Result {
        ...
        let mut status = repo
            .status(src::progress::Discard)?
            .index_worktree_options_mut(|opts| {
                opts.sorting =
                    Some(src::status::plumbing::index_as_worktree_with_renames::Sorting::ByPathCaseSensitive);
            })
    ...
    fn submodule_fully_ignored_by_override() -> crate::Result {
        ...
        let mut status = repo
            ...
            .index_worktree_submodules(Some(Submodule::Given {
                ignore: src::submodule::config::Ignore::All,
                check_dirty: false,
            }))
            .into_iter(None)?;
        let mut items: Vec<_> = status.by_ref().filter_map(Result::ok).collect();
    ...
    fn tree_index_modification_worktree_modification_racy_git() -> crate::Result {
        ...
        let mut status = repo.status(src::progress::Discard)?.into_iter(None)?;
        let mut items: Vec<_> = status.by_ref().filter_map(Result::ok).collect();
        items.sort_by(|a, b| a.location().cmp(b.location()));
    ...
    fn error_during_tree_traversal_causes_failure() -> crate::Result {
        ...
        let platform = repo.status(src::progress::Discard)?.head_tree(hex_to_id(
...
mod index_worktree {
    mod iter {
        use src::status::index_worktree::Item;
        use src_testtools::size_ok;
            ...
            assert_eq!(
                ...
                [
                    Item::DirectoryContents {
                        entry: src_dir::Entry {
                            rela_path: "new".into(),
                            status: src_dir::entry::Status::Untracked,
                            property: None,
                            disk_kind: Some(src_dir::entry::Kind::Directory),
                            index_kind: None,
                            pathspec_match: Some(src_dir::entry::PathspecMatch::Always),
                        },
                        collapsed_directory_status: None
                    },
                    Item::DirectoryContents {
                        entry: src_dir::Entry {
                            ...
                            status: src_dir::entry::Status::Untracked,
                            property: None,
                            disk_kind: Some(src_dir::entry::Kind::File),
                            index_kind: None,
                            pathspec_match: Some(src_dir::entry::PathspecMatch::Always),
                        },
                        collapsed_directory_status: None
        ...
        fn untracked_files_settings_none() -> crate::Result {
            ...
            repo.config_snapshot_mut()
                .set_value(&src::config::tree::Status::SHOW_UNTRACKED_FILES, "no")?;
        ...
        fn early_drop_for_is_dirty_emulation() -> crate::Result {
            ...
            let is_dirty = repo
                .status(src::progress::Discard)?
                .index_worktree_submodules(src::status::Submodule::AsConfigured { check_dirty: true })
                .index_worktree_options_mut(|opts| {
                    opts.sorting =
                        Some(src::status::plumbing::index_as_worktree_with_renames::Sorting::ByPathCaseSensitive);
                })
        ...
        fn tracked_entries_under_a_symlinked_directory_are_reported_as_removed() -> crate::Result {
            ...
            assert_eq!(
                items[idx].summary(),
                Some(src::status::index_worktree::iter::Summary::Removed)
            );
...
mod is_dirty {
    ...
    fn untracked_files_are_excluded() -> crate::Result {
        ...
        assert_eq!(
            repo.status(src::progress::Discard)?
                .into_index_worktree_iter(None)?
                .count(),
            1,
