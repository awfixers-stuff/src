...
static GIT_CONFIG: &[Record] = &[
    ...
    Record {
        config: "fetch.output",
        usage: NotPlanned("'src' might support it, but there is no intention on copying the 'git' CLI")
    },
    Record {
...
pub fn show_progress() -> anyhow::Result<()> {
    let sorted = {
        ...
        v.extend(src::config::Tree.sections().iter().flat_map(|section| {
            fn to_record(key: &dyn src::config::tree::Key) -> Record {
                let config = key.logical_name();
                let note = key.note().map(|note| match note {
                    src::config::tree::Note::Deviation(n) | src::config::tree::Note::Informative(n) => n.to_string(),
                });
                let link = key.link().map(|link| match link {
                    src::config::tree::Link::FallbackKey(key) => {
                        format!("fallback is '{fallback}'", fallback = key.logical_name())
                    }
                    src::config::tree::Link::EnvironmentOverride(name) => format!("overridden by '{name}'"),
                });
