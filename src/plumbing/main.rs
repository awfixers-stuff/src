...
use src::bstr::{io::BufReadExt, BString};
...
pub mod async_util {
    ...
    pub fn prepare(
        ...
        src_features::progress::DoOrDiscard<prodash::tree::Item>,
    ) {
...
pub fn main() -> Result<()> {
    let args: Args = Args::parse_from(src::env::args_os());
    ...
    let repository = {
        ...
        move |mut mode: Mode| -> Result<src::Repository> {
            let mut mapping: src::sec::trust::Mapping<src::open::Options> = Default::default();
            if !config.is_empty() {
                mode = match mode {
            ...
            let to_match_settings = |mut opts: src::open::Options| {
            ...
            let mut repo = src::ThreadSafeRepository::discover_with_environment_overrides_opts(
                repository,
                Default::default(),
                mapping,
            )
            .map(src::Repository::from)?;
            if !config.is_empty() {
                repo.config_snapshot_mut()
                    .append_config(config.iter(), src::config::Source::Cli)
            ...
            }
            {
                ...
                if config_mut
                    .boolean(src::config::tree::Gitoxide::PARSE_PRECIOUS)
                    .is_none()
                {
                    config_mut.set_raw_value(&src::config::tree::Gitoxide::PARSE_PRECIOUS, "true")?;
                }
            }
            Ok(repo)
        }
    ...
    match cmd {
            ...
            move |progress, out, err| {
                ...
                core::repository::status::show(
                    ...
                    core::repository::status::Options {
                        ...
                        statistics,
                        thread_limit: thread_limit.or(cfg!(target_os = "macos").then_some(3)), // TODO: make this a configurable when in `src`, this seems to be optimal on MacOS, linux scales though! MacOS also scales if reading a lot of files for refresh index
                    ...
                    core::repository::archive::Options {
                        ...
                        format: format.map(|f| match f {
                            crate::plumbing::options::archive::Format::Internal => {
                                src::worktree::archive::Format::InternalTransientNonPersistable
                            }
                            crate::plumbing::options::archive::Format::Tar => src::worktree::archive::Format::Tar,
                            crate::plumbing::options::archive::Format::TarGz => {
                                src::worktree::archive::Format::TarGz { compression_level }
                            }
                            crate::plumbing::options::archive::Format::Zip => {
                                src::worktree::archive::Format::Zip { compression_level }
                            }
                        }),
                    },
                )
                ...
                move |progress, _out, _err| {
                    let mut engine = core::corpus::Engine::open_or_create(
                        ...
                        core::corpus::engine::State {
                            gitoxide_version: option_env!("src_VERSION")
                                .ok_or_else(|| anyhow::anyhow!("src_VERSION must be set in build-script"))?
                                .into(),
                            progress,
        ...
        Subcommands::Credential(cmd) => core::repository::credential(
            ...
            match cmd {
                credential::Subcommands::Fill => src::credentials::program::main::Action::Get,
                credential::Subcommands::Approve => src::credentials::program::main::Action::Store,
                credential::Subcommands::Reject => src::credentials::program::main::Action::Erase,
            },
        ),
                        ...
                        move |progress, out, _err| {
                            ...
                            core::pack::index::from_pack(
                                ...
                                core::pack::index::Context {
                                    ...
                                    out,
                                    object_hash,
                                    should_interrupt: &src::interrupt::IS_INTERRUPTED,
                                },
                            )
                        },
                    ),
            ...
            attributes::Subcommands::ValidateBaseline { statistics, no_ignore } => prepare_and_run(
                ...
                move |progress, out, err| {
                    core::repository::attributes::validate_baseline(
                        ...
                        stdin_or_bail()
                            .ok()
                            .map(|stdin| stdin.byte_lines().filter_map(Result::ok).map(src::bstr::BString::from)),
                        progress,
                        out,
                        err,
        ...
        Subcommands::Blame {
            ...
            move |_progress, out, err| {
                ...
                core::repository::blame::blame_file(
                    ...
                    src::blame::Options {
                        diff_algorithm,
                        ranges: src::blame::BlameRanges::from_one_based_inclusive_ranges(ranges)?,
                        since,
                        rewrites: Some(src::diff::Rewrites::default()),
