...
pub mod pretty {
    ...
    fn init_tracing(
        enable: bool,
        reverse_lines: bool,
        progress: &src::progress::prodash::tree::Root,
    ) -> anyhow::Result<()> {
        if enable {
            ...
            (_, true) => {
                ...
                let thread = std::thread::spawn({
                    ...
                    move || {
                        ...
                        let mut out = Vec::new();
                        let res = src::trace::coarse!("run", name = name).into_scope(|| {
...
mod clap {
    ...
    impl builder::TypedValueParser for AsBString {
        ...
        fn parse_ref(&self, _cmd: &Command, _arg: Option<&Arg>, value: &OsStr) -> Result<Self::Value, Error> {
            src::env::os_str_to_bstring(value).ok_or_else(|| Error::new(ErrorKind::InvalidUtf8))
        }
    }
    ...
    impl builder::TypedValueParser for AsHashKind {
        type Value = src::hash::Kind;

        fn parse_ref(&self, cmd: &Command, arg: Option<&Arg>, value: &OsStr) -> Result<Self::Value, Error> {
            builder::StringValueParser::new()
                .try_map(|arg| src::hash::Kind::from_str(&arg))
                .parse_ref(cmd, arg, value)
        }
    ...
    static PATHSPEC_DEFAULTS: std::sync::LazyLock<src::pathspec::Defaults> = std::sync::LazyLock::new(|| {
        src::pathspec::Defaults::from_environment(&mut |n| std::env::var_os(n)).unwrap_or_default()
    });

    impl TypedValueParser for AsPathSpec {
        type Value = src::pathspec::Pattern;

        fn parse_ref(&self, cmd: &Command, arg: Option<&Arg>, value: &OsStr) -> Result<Self::Value, Error> {
            OsStringValueParser::new()
                .try_map(|arg| {
                    let arg: &std::path::Path = arg.as_os_str().as_ref();
                    src::pathspec::parse(src::path::into_bstr(arg).as_ref(), *PATHSPEC_DEFAULTS)
                })
                .parse_ref(cmd, arg, value)
        }
    }
    ...
    impl TypedValueParser for CheckPathSpec {
        ...
        fn parse_ref(&self, cmd: &Command, arg: Option<&Arg>, value: &OsStr) -> Result<Self::Value, Error> {
            OsStringValueParser::new()
                .try_map(|arg| -> Result<_, src::pathspec::parse::Error> {
                    let arg = src::path::into_bstr(std::path::PathBuf::from(arg));
                    src::pathspec::parse(arg.as_ref(), Default::default())?;
                    Ok(arg.into_owned())
                })
                .parse_ref(cmd, arg, value)
        }
    ...
    impl TypedValueParser for AsTime {
        type Value = src::date::Time;

        fn parse_ref(&self, cmd: &Command, arg: Option<&Arg>, value: &OsStr) -> Result<Self::Value, Error> {
            StringValueParser::new()
                .try_map(|arg| src::date::parse(&arg, Some(std::time::SystemTime::now())).map_err(src::Exn::into_inner))
                .parse_ref(cmd, arg, value)
        }
    }
    ...
    impl TypedValueParser for AsPartialRefName {
        type Value = src::refs::PartialName;

        fn parse_ref(&self, cmd: &Command, arg: Option<&Arg>, value: &OsStr) -> Result<Self::Value, Error> {
            AsBString
                .try_map(src::refs::PartialName::try_from)
                .parse_ref(cmd, arg, value)
        }
    }
...
mod value_parser_tests {
    ...
    fn since() {
        ...
        pub struct Cmd {
            ...
            pub arg: Option<src::date::Time>,
        }

        let c = Cmd::parse_from(["cmd", "--since", "2 weeks ago"]);
        assert!(matches!(c.arg, Some(src::date::Time { .. })));
    }
}
