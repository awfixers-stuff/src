...
//! For CI this would mean longer compile times though as it rebuilds `src`
...
fn main() -> Result<()> {
    match std::env::current_exe()?
        ...
        .unwrap_or("src")
    {
        "src" => crate::plumbing::main(),
        "ein" => crate::porcelain::main(),
        unknown => bail!("Executable named '{unknown}' cannot be launched. Exe must be named either `src` or `ein`."),
    }
}
