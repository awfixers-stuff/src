...
src := $(shell cargo metadata --format-version 1 | jq -r .target_directory)/release/src
$(src): always
	cargo build --release --no-default-features --features small
...
stress: ## Run various algorithms on big repositories
	...
	time $(src) --verbose no-repo pack verify --re-encode $(linux_repo)/objects/pack/*.idx
	time $(src) --verbose no-repo pack multi-index -i $(linux_repo)/objects/pack/multi-pack-index create $(linux_repo)/objects/pack/*.idx
	time $(src) --verbose no-repo pack verify $(linux_repo)/objects/pack/multi-pack-index
	rm -Rf out; mkdir out && time $(src) --verbose no-repo pack index create -p $(linux_repo)/objects/pack/*.pack out/
	time $(src) --verbose no-repo pack verify out/*.idx

	time $(src) --verbose no-repo pack verify --statistics $(rust_repo)/objects/pack/*.idx
	time $(src) --verbose no-repo pack verify --algorithm less-memory $(rust_repo)/objects/pack/*.idx
	time $(src) --verbose no-repo pack verify --re-encode $(rust_repo)/objects/pack/*.idx
	# We must ensure there is exactly one pack file for the pack-explode *.idx globs to work.
	git repack -Ad
	time $(src) --verbose no-repo pack explode .git/objects/pack/*.idx

	rm -Rf delme; mkdir delme && time $(src) --verbose no-repo pack explode .git/objects/pack/*.idx delme/

	$(MAKE) stress-commitgraph
	$(MAKE) bench-src-config

.PHONY: stress-commitgraph
stress-commitgraph: release-lean $(commit_graphs)
	set -x; for path in $(wordlist 2, 999, $^); do \
		time $(src) --verbose no-repo commit-graph verify $$path; \
	done

.PHONY: bench-src-config
bench-src-config:
	cd src-config && cargo bench

##@ Maintenance

baseline_asset_dir = src/src/assets/baseline-init
...
transport_fixtures = src-transport/tests/fixtures
