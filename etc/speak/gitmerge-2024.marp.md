...
**the binaries `src` and `ein`**

<!-- `src` dev tool, `ein` client + tooling -->
...
<!-- `src` will never be `git`, never stable, won't mimmick it -->
...
- new project with Git integration, `gitoxide` features suffice
    ...
    * and you can figure out how to use it (`src::Repository`)
...
* [exhaustive docs](https://docs.rs/src/latest/src/)
* **Security**
...
pub fn git_clone_repository(repository_url: &str, target_dir: &Path) -> Result<(), Error> {
    let url =
        src::url::parse(repository_url.into()).context("Failed to parse repository URL")?;
    let should_interrupt = AtomicBool::new(false);
    let mut prepared_clone =
        src::prepare_clone(url, target_dir).context("Failed to prepare clone")?;
...
#[tauri::command(async)]
pub fn git_clone_repository(repository_url: &str, target_dir: &Path) -> Result<(), Error> {
    let url = src::url::parse(repository_url.into()).map_err(anyhow::Error::from)?;
    let should_interrupt = AtomicBool::new(false);

    let (mut checkout, _outcome) = src::prepare_clone(url, target_dir)
...
pub fn git_clone_repository(repository_url: &str, target_dir: &Path) -> Result<(), UnmarkedError> {
    ...
    src::prepare_clone(repository_url, target_dir)?
        .fetch_then_checkout(src::progress::Discard, &should_interrupt)
        .map(|(checkout, _outcome)| checkout)?
        .main_worktree(src::progress::Discard, &should_interrupt)?;
    Ok(())
}
````
