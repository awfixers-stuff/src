...
A small but meaningful contribution by [Lorenz Leutgeb](https://github.com/lorenzleutgeb) helped pave the way for SHA256 support, by making the current SHA1 implementation optional, and the respective `src-hash::Kind` enum non-exhaustive.
This also allows to one day create builds that only support SHA256 as safer (and much faster) alternative.

### `src commit sign`

Last but not least, and kindly contributed by [Christoph Rüßler](https://github.com/cruessler), we now have the `src commit sign` subcommand, which does exactly like it says: it signs a commit using GPG, like Git would, without implementing all the configuration variables and special cases that Git would.

As such, it's a prototype, but also a first step towards implementing signing just like Git would, so it can eventually move into the `src` crate 🎉.
