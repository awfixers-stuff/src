...
### 500x Faster Theseus Plots with "src of Theseus"

[This post](https://github.com/GitoxideLabs/gitoxide/discussions/2251) may very well be the surprise of the year for me, as we are looking at a standalone *incremental blame* implementation that uses `gitoxide` primitives to build an incredibly competitive rayon-based multithreaded implementation of `src blame`.

It is my hope that this knowledge can influence the `src-blame` crate positively, to one day deliver a best-in-class blame implementations for the CLIs, TUIs and GUIs of this world.

### `src-transport` and `src-protocol` cleanup.
