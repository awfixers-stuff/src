...
### `src-archive` - ready for implementation

Thanks to Svetlin's continued efforts we have understood `git archive` in enough detail to be able to implement an MVP of it. To me, its main benefit is that it will require using code that is also used for checking out worktrees, and that alone will make it cleaner and more versatile instead of being 'overfitted' to a single use-case. It doesn't have to be long until `src archive` can produce the first `tar`-streams.
