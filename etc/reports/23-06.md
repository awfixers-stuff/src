...
## Integration of `src-negotiate` into `src`

Low-level negotiation algorithms are now driven by the `src` implementation of the fetch negotiation loop, right after implementing yet another surprise called `mark_complete_and_common_ref()`, a function that is the part of the negotiation algorithm that is shared among all implementations.
...
With negotiation improvements came graph primitives, and with these came commit-graph integration not only in the `src_revision::Graph`, but also in the venerable `src_traverse::commit::Ancestors()` iterator which now optionally makes use of  a commit-graph. This is facilitated by providing more information about commits during traversal, like their parents, their commit-time and possibly their generation, as this information was obtained during traversal anyway and can be leveraged by the caller.
...
##### `src revision list` and `src commit-graph list`

`src revision list` now shows all information that is easily available from an iteration, adding the commit-time as well as the amount of parents for brevity, while `src commit-graph list` sub-command was added to show all data that is present in the commit-graph if one is available.
...
What's quite revealing is `src fetch --open-negotiation-graph=path.svg` which will show visualise the negotiation graph as it was after the negotiation was finished along with all flags that were set per commit.
And for good measure, one can enable simple graph visualization for `src revision list --svg` as well.

## `src corpus` and `--trace`
...
`gitoxide` now integrates with `tracing-core` using the `src-trace` crate which by default compiles to nothing at all. Only when `tracing` feature is enabled by the application will it actually incur a cost, and then only the bare minimal one as `src-trace` is quite minimal compared to its `tracing` counterpart.

With that it's possible to see how fast certain parts of the code are running, and thanks to `tracing-forest` these visualize beautifully. All this can be activated with `src --trace`.

`src corpus` is a runner over a set of repositories which stores information about each run in a sqlite database for later analysis. What's neat about it is that it collects the tracing spans and serializes them into a database as well. With that, it should be possible to compare details of each computation with each other while only really taking care of tracing.

Now I just have to find a way to more regularly perform `src corpus` runs and to finally figure out an easy way to analyse the information contained in it.

## Community

### Rust Foundation sponsorship

The negotiation upgrade was promptly merged mostly by upgrading `src` to the latest version, so `cargo` should now work correctly with certain [crates.io](http://crates.io/) proxies which are particularly necessary in China. Next up is a filtering system to be able to perform correct worktree checkouts.
