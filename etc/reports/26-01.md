...
## A new dawn for error handling: `exn` via `src-error`
...
`src-error` completely forks `exn` to add what's needed to work in `gitoxide`, but work is underway to eventually reintegrated the `exn` crate.
Here is the new system:

1. `src-error` provides simple standard error types for common situations, like `Message` and `ParseError`. These implement `std::error::Error` and can be used like `Result<T, Message>`.
...
3. `src_error::Error` is returned by `src` and it's essentially erasing types from `Exn` or any other errors, while providing facilities to introspect the call chain. This makes it similar to `git2::Error` for instance, which has proven to be so convenient I wouldn't want it any other way anymore. It also serves as a way to turn error trees powered by `Exn` back into call chains that work well with `anyhow`.
...
### Bugfix in `src-pack` that surfaced with `zlib-rs`
...
It turned out that `src-pack` was relying on ZIP never writing more to the output than what it needed to consume the input stream, something that is an implementation defined behavior. 
