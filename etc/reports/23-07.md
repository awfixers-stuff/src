This month felt quite intense and I have been busy non-stop, and particularly in the last three days where `src archive` came to life. After all, when you have the capability to properly checkout and filter worktree files, one should also be able to stream these straight into an archive format.
But let's start in order.

## The `src filter` crate
...
Three days, and a last-minute feature that I wanted to be able to talk about. In short, `src archive` is `git archive`, but without legacy features and with up to 1.8 times more performance for `zip` archives, or about 18% performance for everything else.
...
The `src-worktree-stream` and `src-archive` crates have additionally been made available in `src` through high-level APIs that configure themselves like one would expect. Those could easily be consumed by the `src archive` CLI implementation, that is quite trivial if you look at it.

Finally, I also added a workspace-streaming task into `src corpus` and streamed the worktree in full of all ~68k repositories stored on the 4TB corpus disk. It took just an hour. It seems to work.

## Community

### Removal of GPL licensed files from the codebase

But how is that possible? Fortunately an eagle-eyed packager found these template files that were used when initialising a new repository. These where among the first files added to the repository, as `src init` was the first command I implemented. Back in the days it was just a hobby project and I didn't know what would become of it.
...
### `crates-index` used `src` (PR pending)

Inspired by the fruitful interaction with the `cargo-binstall` maintainers I decided to *quickly* port `crates-index` over to `src` instead. And overall, it was indeed quite a straightforward transformation which came with many API improvements to make the conversion to `src` appear just as natural as the `git2` API it replaces.
