...
To make blob content merging more approachable, there is now a [`src merge file`](https://github.com/Byron/gitoxide/pull/1611) not dissimilar to `git merge-file` available on the command-line.
...
While watching a fun video about Git where the UX-issues of `git cat-file` where pointed out once again I decided to quickly hack together the answer: a `src cat-file` with perfect UX, just called `src cat` for short.
...
#### `src_WORK_TREE` support

Brought to us by the same person that also pushed `src blame` to completion (review pending), is this innocuously looking improvement that turned out to hell of a journey to actually get working: `GIT_WORK_TREE` support for the `src` binary.
...
And finally, in order to make real-world debugging for `src blame` easier, there is now a [`src diff tree`](https://github.com/GitoxideLabs/gitoxide/pull/1626) command which exposes an algorithm that has been long available, but in API only.

This is also a perfect example of what `src` is meant to be for - a tool for  developers to facilitate to develop tools with `gitoxide` and `gitoxide` itself. Everything is allowed, and it can start (and stay) very simple, while it's useful.

#### 'Blame' is getting there

Thanks to the introduction of the new and recently contributed `Topo` traversal `src blame` is now matches the Git baseline even better, to the point where mismatches *just* seem to be related to the [slider problem](https://github.com/mhagger/diff-slider-tools/blob/master/README.md).
...
Still, there is no news here, yet I am excited for the dam-break that is definitely going to happen. `src status` just needs one component to be complete, and that can already be useful. Also, I will revisit it once `src merge tree` is usable, maybe it can benefit already.
