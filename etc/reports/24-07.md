...
`gitoxide` does that lazily, and requires the API-user to opt-in to it, but when using the `src` executable for instance, many sub-commands will execute `git.exe` in order to learn additional paths that are needed for proper functioning, for example when performing a clone.
...
It's worth noting that the fix was distributed as patch-release of the `src-path` crate, which is pleasant and easy to do, finally showing the benefits of the 'many-small-crates' model used here.
