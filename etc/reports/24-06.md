With the sickness of last month mostly overcome, I did return to ordinary working hours and found the month productive thus far. However, I still didn't find the peace to work continue on `src status` and the diffs of indices with trees, unfortunately, but at least there are some smaller improvements to share.
...
I do hope that this is finally it with `src-config`, which already has a massive API surface with a lot of flexibility. It seems I'd have to use it much more to know for sure.
