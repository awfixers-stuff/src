...
I will do my best to work it all off to even be in a state to think about new features, with candidates being a `git2`-style commit-walk and a proper `src reset`.
...
Thanks to Christoph we now have `src diff file` to use, producing unified diff of anything currently stored in Git. And I hope that future upgrades will enable it to also be able to read from the worktree, making it a driver of the `src::diff:blob::Platform`, effectively.

In other news…

#### `src-blame` will be used in `helix`
...
With `src status` now available, the plan still stands to bring it into `cargo`. Before that, there is a huge stack of unfinished PRs to wrap up though, so it's unlikely to happen anytime soon.
