...
Gitoxide is now also able to write and verify multi-pack indices, with such functionality exposed in `src pack multi-index`,
and it successfully creates indices for repositories that require large-offset support.

Furthermore, the foundation for a complete `fsck` like command were set to deeply validate all data known in a git repository. For now `src repository verify` will only verify object databases though, completely ignoring references at the moment.

### `src` embrace hierarchical sub-commands

Due to the now lifted requirement to support the now abandoned `argh` parser, `src` previously used a single level of
