...
The latter led me to rename `src` to `ein`, which is hinting at `Einstein` as the exact opposite of `git`, and shall remind me of that it must be no less than a UX revolution that on top of that, will handle the biggest of mono-repos better than git does.
