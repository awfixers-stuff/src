...
## `src repo revspec explain` and beginnings of `src repo revspec parse`

As always, no new functionality feels quite like it's useful unless one can run it against real-world repositories easily. Thus `src repo revspec explain` (actually repository-independent) allows to verbalize all calls the delegate receives from the parser with the net-effect of showing what a rev-spec really does.

In order to better grasp the quality of errors, I decided to also and immediately implement a minimal version of revspec parsing, the `src` equivalent of `git rev-parse`. And even though most rev-spec features aren't implemented yet, it's already usable for some few and it's just nice to run it from time to time.
