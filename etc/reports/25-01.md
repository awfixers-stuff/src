...
The year definitely starts off well with a complete implementation of no other than… `src status`.

## `src status`

For a while now there was `src status`, but it was unable to see changes between `HEAD^{tree}` and the current index.
This was now [rectified](https://github.com/GitoxideLabs/gitoxide/pull/1410) and `src status` can serve as early demonstration of the newfound powers.
...
### Experimental support for `src blame`!

Thanks to Christoph's fantastic work (*and incredible patience*), the long-standing [Blame-PR](https://github.com/GitoxideLabs/gitoxide/pull/1453) was finally merged so we can now try `src blame` in the wild.
