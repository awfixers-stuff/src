...
### `src::Repository::blame_file()`

Thanks to [Christoph Rüßler](https://github.com/cruessler) `gitoxide` had a very competitive `src blame <file>` implementation for a while already, and one that was available on the command-line as well.

Now, however, he also added it to the `src::Repository`, making it available other tools with ease - previously they would have to use the plumbing in the `src-blame` crate directly.
