...
Cargo once again [was suffering from a bug caused by `src`](https://github.com/rust-lang/cargo/issues/13923) - this time, `cargo doc` would fail (but `cargo build` and friends would work as normal. Once again, the `list-files` code that is now powered by `src` in stable even, ran into a condition where it would fail as it couldn't read the Git configuration if the `core.excludesFile` key was empty. Git would just ignore that, but `src` did not.
