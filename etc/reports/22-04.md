...
Lastly, the culmination of this attribute stack work will be `check-attr` and `check-ignore` like sub-commands of the`src` plumbing tool to put it through its paces.
...
This system is flexible enough to easily configure similar behaviour to what git does, and there are plans to add something like `--strict` or `--paranoid` to turn git-like behaviour on for `src` and `ein` as well.
...
As usual, a sub-command was added to allow running the code on real repositories, so `src mailmap verify` can check mailmaps for errors.
