...
use src::progress::DynNestedProgress;
...
impl Execute for WorktreeStream {
    fn execute(
        ...
        use src::Progress;
        let repo = src::open_opts(repo, src::open::Options::isolated())?;
        let (stream, _) = {
            let _span = src::trace::coarse!("read index and create worktree stream");
            repo.worktree_stream(repo.head_commit()?.tree_id()?)?
        };
        progress.init(None, src::progress::bytes());
        std::io::copy(
            &mut stream.into_read(),
            &mut src::features::interrupt::Write {
                inner: src::features::progress::Write {
                    inner: std::io::sink(),
                    progress,
                },
                should_interrupt,
...
impl Execute for OpenRepo {
    fn execute(
        ...
        src::open_opts(repo, src::open::Options::isolated())?;
        Ok(())
    }
}
