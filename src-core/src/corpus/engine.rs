...
use src::{Count, NestedProgress, Progress};
use rusqlite::params;
...
pub type ProgressItem = src::progress::DoOrDiscard<src::progress::prodash::tree::Item>;
...
impl Engine {
    ...
    fn perform_run(
        ...
        let threads = src::parallel::num_threads(threads);
        ...
        'tasks_loop: for (task_id, task) in tasks {
            ...
            repo_progress.init(Some(repos.len()), src::progress::count("repos"));
            if task.execute_exclusive || threads == 1 || dry_run {
                ...
                let repo_progress = src::threading::OwnShared::new(src::threading::Mutable::new(
                    repo_progress.add_child("in parallel"),
                ));
                src::parallel::in_parallel_with_slice(
                    &mut repos,
                    Some(threads),
                    {
                        ...
                        move |tid| {
                            let mut progress = src::threading::lock(&shared_repo_progress);
                            (
                    ...
                    || (!src::interrupt::is_triggered()).then(|| Duration::from_millis(100)),
                    std::convert::identity,
                )?;
                let repo_progress = src::threading::lock(&repo_progress);
                repo_progress.show_throughput(task_start);
    ...
    fn prepare_corpus_path(&self, corpus_path: PathBuf) -> anyhow::Result<(PathBuf, db::Id)> {
        let corpus_path = src::path::realpath(corpus_path)?;
    ...
    fn find_repos(
        ...
        self.state.progress.init(None, src::progress::count("repos"));
        ...
        let repos = std::thread::scope({
            ...
            |scope| -> anyhow::Result<_> {
                        ...
                        scope.spawn({
                            ...
                            move || -> anyhow::Result<_> {
                                for repo_path in path_rx {
                                    let res = (|| {
                                        let repo = src::open_opts(&repo_path, src::open::Options::isolated())?;
                ...
                let write_db = scope.spawn(move || -> anyhow::Result<Vec<db::Repo>> {
                    ...
                    progress.init(None, src::progress::count("repos"));
                ...
                let repos = src::interrupt::Iter::new(
