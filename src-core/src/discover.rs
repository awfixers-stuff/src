...
pub fn discover(repo: &Path, mut out: impl std::io::Write) -> anyhow::Result<()> {
    ...
    has_err |= print_result(
        &mut out,
        src::open_opts(repo, src::open::Options::default().strict_config(true)),
    )?;
    ...
    if has_err {
        ...
        has_err |= print_result(
            &mut out,
            src::open_opts(repo, src::open::Options::default().strict_config(false)),
        )?;
    }

    writeln!(out)?;
    writeln!(out, "discover from {}:", repo.display())?;
    has_err |= print_result(&mut out, src::discover(repo))?;

    writeln!(out)?;
    writeln!(out, "discover (plumbing) from {}:", repo.display())?;
    has_err |= print_result(&mut out, src::discover::upwards(repo))?;
