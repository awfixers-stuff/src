...
pub fn update(
    ...
    progress: &mut impl src::NestedProgress,
...
) -> anyhow::Result<Vec<src::ObjectId>> {
    let commit_id = repo.head_id()?.detach();
    let threads = src::features::parallel::num_threads(threads);
    ...
    let out = std::thread::scope(|scope| -> anyhow::Result<_> {
        ...
        for item in con
            ...
            .query_map([], |r| {
                Ok(src::ObjectId::try_from(r.get_ref(0)?.as_bytes()?)
                    .unwrap_or_else(|_| src::ObjectId::null(src::hash::Kind::Sha1)))
            })?
        {
        ...
        let rewrites = {
            ...
            let (r, was_configured) = src::diff::new_rewrites(&repo.config_snapshot(), true)?;
            if was_configured && r.is_none() {
                src::trace::warn!("Rename tracking is disabled by configuration, but we enable it using the default");
            }
            let mut r = r.unwrap_or_default();
            r.copies = Some(src::diff::rewrites::Copies {
        ...
        #[derive(Clone)]
        struct Task {
            commit: src::hash::ObjectId,
            parent_commit: Option<src::hash::ObjectId>,
            compute_stats: bool,
        }
                    ...
                    scope.spawn({
                        ...
                        move || -> anyhow::Result<()> {
                            let mut rewrite_cache =
                                repo.diff_resource_cache(src::diff::blob::pipeline::Mode::ToGit, Default::default())?;
                                    ...
                                    if compute_stats {
                                        ...
                                        from.changes()?
                                            ...
                                            .for_each_to_obtain_tree_with_cache(&to, &mut rewrite_cache, |change| {
                                                use src::object::tree::diff::Change::*;
                                                                ...
                                                                if let Ok(prep) = cache.prepare_diff() {
                                                                    ...
                                                                    match prep.operation {
                                                                        Operation::InternalDiff { algorithm } => {
                                                                            let counts = src::diff::blob::diff(
                                                                                algorithm,
                                                                                &tokens,
                                                                                src::diff::blob::sink::Counter::default(
                                                                                ),
                                                                            );
        ...
        impl<Find> src::prelude::Find for Db<'_, Find>
        where
            Find: src::prelude::Find + Clone,
        {
            fn try_find<'b>(&self, id: &src::oid, buf: &'b mut Vec<u8>) -> Result<Option<src::objs::Data<'b>>, Error> {
                ...
                if self.known_commits.binary_search(&id.to_owned()).is_err() {
                    let res = {
                        let mut parents = src::objs::CommitRefIter::from_bytes(obj.data).parent_ids();
        ...
        let commit_iter = src::interrupt::Iter::new(commit_id.ancestors(&db), || anyhow!("Cancelled by user"));
        ...
        for c in commit_iter {
            match c?.map(|c| c.id) {
                ...
                Err(src::traverse::commit::simple::Error::Find { .. }) => {
...
fn add_lines(out: &mut Vec<FileChange>, path: &BStr, lines_counter: &AtomicUsize, id: src::Id<'_>) {
...
fn remove_lines(out: &mut Vec<FileChange>, path: &BStr, lines_counter: &AtomicUsize, id: src::Id<'_>) {
    if let Ok(blob) = id.object() {
        let mut lines = LineStats::default();
        let nl = blob.data.lines_with_terminator().count();
        lines.removed += nl;
