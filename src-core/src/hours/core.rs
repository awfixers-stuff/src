...
type SpawnResultWithReturnChannelAndWorkers<'scope> = (
    crossbeam_channel::Sender<Vec<(CommitIdx, Option<src::hash::ObjectId>, src::hash::ObjectId)>>,
...
pub fn spawn_tree_delta_threads<'scope>(
    ...
    let (tx, rx) = crossbeam_channel::unbounded::<Vec<(CommitIdx, Option<src::hash::ObjectId>, src::hash::ObjectId)>>();
                ...
                move || -> Result<_, anyhow::Error> {
                    ...
                    let mut cache = line_stats
                        .then(|| -> anyhow::Result<_> {
                            Ok(repo.diff_resource_cache(src::diff::blob::pipeline::Mode::ToGit, Default::default())?)
                        })
                        .transpose()?;
                    for chunk in rx {
