...
impl SignatureRef<'_> {
    fn seconds(&self) -> src::date::SecondsSinceUnixEpoch {
        self.time.seconds
    }
}
...
fn commit_author_identities(
    commit_data: &[u8],
) -> Result<(src::actor::SignatureRef<'_>, SmallVec<[src::actor::IdentityRef<'_>; 2]>), src::objs::decode::Error> {
    let commit = src::objs::CommitRef::from_bytes(commit_data)?;
    let author = commit.author()?.trim();
    let mut authors = smallvec![src::actor::IdentityRef::from(author)];
    authors.extend(commit.co_authored_by_trailers().filter_map(|trailer| {
        src::actor::IdentityRef::from_bytes::<src::objs::decode::ParseError>(trailer.value.as_ref())
            .ok()
...
where
    ...
    P: NestedProgress,
{
    let repo = src::discover(working_dir)?;
                ...
                for (commit_idx, commit_data) in rx {
                    if let Ok((commit_author, authors)) = commit_author_identities(&commit_data) {
                        ...
                        for identity in authors {
                            let author = mailmap.resolve_cow(src::actor::SignatureRef {
                                name: identity.name,
