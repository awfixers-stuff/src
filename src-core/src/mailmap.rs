...
pub fn verify(path: impl AsRef<Path>, format: OutputFormat, mut out: impl Write) -> anyhow::Result<()> {
    ...
    let mut err_count = 0;
    for err in src::mailmap::parse(&buf).filter_map(Result::err) {
        err_count += 1;
        writeln!(out, "{err}")?;
    }

    let mut seen = HashSet::<(_, _)>::default();
    for entry in src::mailmap::parse(&buf).filter_map(Result::ok) {
    ...
    if err_count == 0 {
        writeln!(out, "{} lines OK", src::mailmap::parse(&buf).count())?;
        Ok(())
    } else {
