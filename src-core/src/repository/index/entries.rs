...
pub(crate) mod function {
    ...
    fn print_entries(
        ...
        format: OutputFormat,
        mut all_attrs: Option<&mut BTreeSet<src::attrs::Assignment>>,
        simple: bool,
        prefix: &BStr,
        recurse_submodules: bool,
        out: &mut impl std::io::Write,
    ) -> anyhow::Result<Statistics> {
        let _span = src::trace::coarse!("print_entries()", git_dir = ?repo.git_dir());
        ...
        if let Some(entries) = index.prefixed_entries(pathspec.common_prefix()) {
            ...
            while let Some(entry) = entries.next() {
                ...
                {
                    let sm_path = src::path::to_unix_separators_on_windows(sm.path()?);
    ...
    fn init_cache(
        ...
        Option<(src::attrs::search::Outcome, src::AttributeStack<'_>)>,
    )> {
            ...
            .map(|attrs| {
                repo.attributes(
                    ...
                    match attrs {
                        ...
                        Attributes::Index => src::worktree::stack::state::attributes::Source::IdMapping,
                    },
                    match attrs {
                        Attributes::WorktreeAndIndex => {
                            src::worktree::stack::state::ignore::Source::WorktreeThenIdMappingIfNotSkipped
                                .adjust_for_bare(repo.is_bare())
                        }
                        Attributes::Index => src::worktree::stack::state::ignore::Source::IdMapping,
                    },
                    None,
                )
    ...
    struct Attrs {
        is_excluded: bool,
        attributes: Vec<src::attrs::Assignment>,
    }
    ...
    struct Statistics {
        ...
        pub cache: Option<src::worktree::stack::Statistics>,
    ...
    fn to_json(
        out: &mut impl std::io::Write,
        index: &src::index::File,
        entry: &src::index::Entry,
        attrs: Option<Attrs>,
        is_last: bool,
        prefix: &BStr,
    ) -> anyhow::Result<()> {
        use src::bstr::ByteSlice;
        #[derive(serde::Serialize)]
        struct Entry<'a> {
            stat: &'a src::index::entry::Stat,
            hex_id: String,
            flags: u32,
            mode: u32,
    ...
    fn print_attrs(attrs: Option<Attrs>, mode: src::index::entry::Mode) -> Cow<'static, str> {
