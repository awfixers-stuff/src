use anyhow::Context;
use src::diff::blob::unified_diff::ConsumeBinaryHunk;
use src::{
...
fn write_changes(
    ...
    changes: Vec<src::diff::tree_with_rewrites::Change>,
) -> Result<(), std::io::Error> {
    for change in changes {
        match change {
            src::diff::tree_with_rewrites::Change::Addition {
                location,
                id,
                entry_mode,
                ..
            ...
            src::diff::tree_with_rewrites::Change::Deletion {
                location,
                id,
                entry_mode,
                ..
            ...
            src::diff::tree_with_rewrites::Change::Modification {
                location,
                previous_id,
                id,
                previous_entry_mode,
            ...
            src::diff::tree_with_rewrites::Change::Rewrite {
                source_location,
                source_id,
                id,
                location,
...
fn resolve_revspec(
    ...
    match result {
        Err(err) => {
            ...
            let not_found = err
                .sources()
                .find_map(|err| err.downcast_ref::<src::refs::file::find::existing::Error>());
            if let Some(src::refs::file::find::existing::Error::NotFound { name }) = not_found {
                let root = repo.workdir().map(ToOwned::to_owned);
                let name = src::path::os_string_into_bstring(name.into())?;

                Ok((ObjectId::null(src::hash::Kind::Sha1), root, name))
            } else {
                Err(err.into())
            }
        }
...
pub fn file(
    ...
    let worktree_roots = src::diff::blob::pipeline::WorktreeRoots { old_root, new_root };

    let mut resource_cache = repo.diff_resource_cache(
        src::diff::blob::pipeline::Mode::ToGitUnlessBinaryToTextIsPresent,
        worktree_roots,
    )?;

    resource_cache.set_resource(
        old_blob_id,
        src::object::tree::EntryKind::Blob,
        old_path.as_ref(),
        src::diff::blob::ResourceKind::OldOrSource,
        &repo.objects,
    )?;
    resource_cache.set_resource(
        new_blob_id,
        src::object::tree::EntryKind::Blob,
        new_path.as_ref(),
        src::diff::blob::ResourceKind::NewOrDestination,
        &repo.objects,
    )?;

    let outcome = resource_cache.prepare_diff()?;

    use src::diff::blob::platform::prepare_diff::Operation;
    ...
    let interner = src::diff::blob::intern::InternedInput::new(
    ...
    let unified_diff = src::diff::blob::diff(algorithm, &interner, unified_diff)?;
...
pub(crate) fn tokens_for_diffing(data: &[u8]) -> impl TokenSource<Token = &[u8]> {
    src::diff::blob::sources::byte_lines(data)
}
