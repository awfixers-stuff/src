...
pub(crate) mod function {
    ...
    use src::{bstr::BString, remote::fetch::Status, NestedProgress};
        ...
        P::SubProgress: 'static,
    {
        ...
        let url: src::Url = url.as_ref().try_into()?;
        let directory = directory.map_or_else(
            || {
                let path = src::path::from_bstr(Cow::Borrowed(url.path.as_ref()));
        ...
        let mut prepare = src::clone::PrepareFetch::new(
            ...
            {
                let mut opts = src::open::Options::default().config_overrides(overrides);
                opts.permissions.config.git_binary = true;
                opts
            },
        )?;
        if no_tags {
            prepare = prepare.configure_remote(|r| Ok(r.with_fetch_tags(src::remote::fetch::Tags::None)));
        }
        let (mut checkout, fetch_outcome) = prepare
            .with_shallow(shallow)
            .with_ref_name(ref_name.as_ref())?
            .fetch_then_checkout(&mut progress, &src::interrupt::IS_INTERRUPTED)?;

        let (repo, outcome) = if bare {
            (checkout.persist(), None)
        } else {
            let (repo, outcome) = checkout.main_worktree(progress, &src::interrupt::IS_INTERRUPTED)?;
            (repo, Some(outcome))
        };
        ...
        match fetch_outcome.status {
            ...
            Status::Change {
                ...
                let remote = repo
                    .find_default_remote(src::remote::Direction::Fetch)
        ...
        if let Some(src::worktree::state::checkout::Outcome { collisions, errors, .. }) = outcome {
