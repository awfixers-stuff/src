...
mod refs_impl {
    ...
    #[src::protocol::maybe_async::maybe_async]
    pub async fn refs_fn(
        ...
        match kind {
            refs::Kind::Tracking { .. } => print_refmap(
                &repo,
                remote.refspecs(src::remote::Direction::Fetch),
                map,
                show_unmapped,
                out,
                err,
    ...
    pub(crate) fn print_refmap(
        ...
        for mapping in &map.mappings {
            ...
            let target_id = match &mapping.remote {
                ...
                src::remote::fetch::refmap::Source::Ref(r) => print_ref(&mut out, r)?,
            };
            match &mapping.local {
                Some(local) => {
                    write!(out, " -> {local} ")?;
    ...
    pub(crate) fn print_ref(mut out: impl std::io::Write, r: &handshake::Ref) -> std::io::Result<&src::hash::oid> {
        match r {
            handshake::Ref::Direct {
                full_ref_name: path,
                object,
            ...
            handshake::Ref::Unborn { full_ref_name, target } => {
                static NULL: src::hash::ObjectId = src::hash::ObjectId::null(src::hash::Kind::Sha1);
