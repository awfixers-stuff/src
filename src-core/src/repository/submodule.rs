use anyhow::bail;
use src::{commit::describe::SelectRef, prelude::ObjectIdExt, Repository, Submodule};
...
fn print_sm(sm: Submodule<'_>, dirty_suffix: Option<&str>, out: &mut impl std::io::Write) -> anyhow::Result<()> {
    let _span = src::trace::coarse!("print_sm", path = ?sm.path());
    let state = sm.state()?;
...
fn submodule_short_hash(id: Option<src::ObjectId>, repo: Option<&Repository>) -> String {
    id.map_or_else(
        || "none".to_string(),
