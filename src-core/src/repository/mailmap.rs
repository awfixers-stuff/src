...
pub fn entries(
    ...
    let mut mailmap = src::mailmap::Snapshot::default();
...
pub fn check(
    ...
    for contact in contacts {
        let actor = match src::actor::IdentityRef::from_bytes::<()>(&contact) {
            Ok(a) => a,
            Err(_) => {
                ...
                src::actor::IdentityRef {
                    name: "".into(),
                    email: email.into(),
                }
            }
        };
        let resolved = mailmap.resolve_cow(src::actor::SignatureRef {
            name: actor.name,
            email: actor.email,
            time: Default::default(),
        });
        let resolved = src::actor::IdentityRef {
