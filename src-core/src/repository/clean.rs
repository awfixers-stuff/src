...
pub(crate) mod function {
    ...
    pub fn clean(
        ...
        let mut pathspec = pathspec_matches_result
            .then(|| {
                repo.pathspec(
                    ...
                    true,
                    &index,
                    src::worktree::stack::state::attributes::Source::WorktreeThenIdMapping,
                )
            })
            .transpose()?;
        ...
        for (mut entry, dir_status) in entries.into_iter() {
            ...
            let pathspec_includes_entry = match pathspec.as_mut() {
                None => entry
                    .pathspec_match
                    .is_some_and(|m| m != src::dir::entry::PathspecMatch::Excluded),
            ...
            let keep = match entry.status {
                ...
                Status::Ignored(src::ignore::Kind::Expendable) => {
                    skipped_ignored += usize::from(!ignored);
                    ignored
                }
                Status::Ignored(src::ignore::Kind::Precious) => {
            ...
            if entry.disk_kind.is_none() {
                entry.disk_kind = workdir
                    .join(src::path::from_bstr(entry.rela_path.as_bstr()))
                    .symlink_metadata()
                    .ok()
            ...
            if disk_kind == src::dir::entry::Kind::Directory
                && src::discover::is_git(&workdir.join(src::path::from_bstr(entry.rela_path.as_bstr()))).is_ok()
            {
                if debug {
                    writeln!(err, "DBG: upgraded directory '{}' to bare repository", entry.rela_path).ok();
                }
                disk_kind = src::dir::entry::Kind::Repository;
            }
            ...
            let is_ignored = matches!(entry.status, src::dir::entry::Status::Ignored(_));
            let entry_path = src::path::from_bstr(entry.rela_path);
            let display_path = src::path::relativize_with_prefix(&entry_path, prefix);
            if disk_kind == src::dir::entry::Kind::Directory {
                saw_ignored_directory |= is_ignored;
                saw_untracked_directory |= entry.status == src::dir::entry::Status::Untracked;
            }
            ...
            writeln!(
                ...
                status = match entry.status {
                    Status::Ignored(kind) => {
                        Cow::Owned(format!(
                            "({})",
                            match kind {
                                src::ignore::Kind::Precious => "💲",
                                src::ignore::Kind::Expendable => "🗑️",
                            }
                        ))
                    }
                    Status::Untracked => {
                ...
                maybe = if entry.property == Some(src::dir::entry::Property::EmptyDirectoryAndCWD) {
                ...
                suffix = match disk_kind {
                    ...
                    Kind::Directory if entry.property == Some(src::dir::entry::Property::EmptyDirectory) => {
                        " empty"
                    }
                    Kind::Repository => {
                        " repository"
    ...
    struct InterruptibleCollect {
        inner: src::dir::walk::delegate::Collect,
    }

    impl src::dir::walk::Delegate for InterruptibleCollect {
