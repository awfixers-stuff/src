...
use src::{diff::blob::ResourceKind, filter::plumbing::driver::apply::Delay, revision::Spec};
...
pub fn display_object(
    ...
    spec: Spec<'_>,
    tree_mode: TreeMode,
    cache: Option<(BlobFormat, &mut src::diff::blob::Platform)>,
    mut out: impl std::io::Write,
) -> anyhow::Result<()> {
    let id = spec.single().context("rev-spec must resolve to a single object")?;
    let header = id.header()?;
    match header.kind() {
        src::object::Kind::Tree if matches!(tree_mode, TreeMode::Pretty) => {
            for entry in id.object()?.into_tree().iter() {
                writeln!(out, "{}", entry?)?;
            }
        }
        src::object::Kind::Blob if cache.is_some() && spec.path_and_mode().is_some() => {
...
pub(super) mod function {
    ...
    pub fn cat(repo: src::Repository, revspec: &str, out: impl std::io::Write) -> anyhow::Result<()> {
