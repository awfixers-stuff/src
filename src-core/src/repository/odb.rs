...
pub fn info(
    ...
    pub struct Statistics {
        ...
        #[cfg_attr(not(feature = "serde"), allow(dead_code))]
        pub structure: Vec<src::odb::store::structure::Record>,
        #[cfg_attr(not(feature = "serde"), allow(dead_code))]
        pub metrics: src::odb::store::Metrics,
    }
...
pub fn statistics(
    ...
    use src::odb::{find, HeaderExt};

    if format == OutputFormat::Human {
        writeln!(err, "Only JSON is implemented - using that instead")?;
    }

    progress.init(None, src::progress::count("objects"));
    ...
    impl Statistics {
        fn count(&mut self, kind: src::object::Kind, size: u64) {
            use src::object::Kind::*;
            match kind {
                Commit => {
                    self.commits += 1;
        ...
        fn consume(&mut self, item: src::odb::find::Header) {
            match item {
    ...
    impl src::parallel::Reduce for Reduce {
        type Input = Result<Vec<(src::ObjectId, src::odb::find::Header)>, anyhow::Error>;
        type FeedProduce = ();
    ...
    let mut stats = if src::parallel::num_threads(thread_limit) > 1 {
        src::parallel::in_parallel(
            src::interrupt::Iter::new(
                src::features::iter::Chunks {
                    inner: object_ids,
                    size: chunk_size,
                },
                cancelled,
        ...
        for (count, id) in object_ids.enumerate() {
            if count % chunk_size == 0 && src::interrupt::is_triggered() {
    ...
    if let Some(mut ids) = stats.ids.take() {
        ...
        let start = std::time::Instant::now();
        let repo = src::open_opts(repo.git_dir(), repo.open_options().to_owned())?;
        progress.set_name("re-counting".into());
        progress.init(Some(ids.len()), src::progress::count("objects"));
        let counter = progress.counter();
        counter.store(0, Ordering::Relaxed);
        let errors = src::parallel::in_parallel_with_slice(
            &mut ids,
            thread_limit,
            {
                let objects = repo.objects.clone();
            ...
            |id, (odb, counter, has_error), _threads_left, _stop_everything| -> anyhow::Result<()> {
                ...
                if let Err(_err) = odb.header(id) {
                    *has_error = true;
                    src::trace::error!(err = ?_err, "Object that is known to be present wasn't found");
                }
                Ok(())
            },
...
pub fn entries(repo: src::Repository, format: OutputFormat, mut out: impl io::Write) -> anyhow::Result<()> {
    if format != OutputFormat::Human {
