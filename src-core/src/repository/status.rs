...
pub fn show(
    ...
    mut progress: impl src::NestedProgress + 'static,
    Options {
        ignored,
        format,
        output_format,
    ...
    let mut iter = repo
        ...
        .should_interrupt_shared(&src::interrupt::IS_INTERRUPTED)
        .index_worktree_options_mut(|opts| {
            if let Some((opts, ignored)) = opts.dirwalk_options.as_mut().zip(ignored) {
                opts.set_emit_ignored(Some(match ignored {
                    Ignored::Collapsed => src::dir::walk::EmissionMode::CollapseDirectory,
                    Ignored::Matching => src::dir::walk::EmissionMode::Matching,
                }));
            }
            opts.rewrites = index_worktree_renames.map(|percentage| src::diff::Rewrites {
                copies: None,
            ...
            if opts.rewrites.is_some() {
                if let Some(opts) = opts.dirwalk_options.as_mut() {
                    opts.set_emit_untracked(src::dir::walk::EmissionMode::Matching);
                    if ignored.is_some() {
                        opts.set_emit_ignored(Some(src::dir::walk::EmissionMode::Matching));
                    }
                }
            }
            opts.thread_limit = thread_limit;
            opts.sorting = Some(src::status::plumbing::index_as_worktree_with_renames::Sorting::ByPathCaseSensitive);
        })
        .index_worktree_submodules(match submodules {
            Some(mode) => {
                let ignore = match mode {
                    Submodules::All => src::submodule::config::Ignore::None,
                    Submodules::RefChange => src::submodule::config::Ignore::Dirty,
                    Submodules::Modifications => src::submodule::config::Ignore::Untracked,
                    Submodules::None => src::submodule::config::Ignore::All,
                };
                src::status::Submodule::Given {
                    ignore,
                    check_dirty: false,
                }
            }
            None => src::status::Submodule::AsConfigured { check_dirty: false },
        })
        .into_iter(pathspecs)?;
        ...
        match item {
            status::Item::TreeIndex(change) => {
                ...
                let status = match change {
                    src::diff::index::Change::Addition { .. } => "A",
                    src::diff::index::Change::Deletion { .. } => "D",
                    src::diff::index::Change::Modification { .. } => "M",
                    src::diff::index::Change::Rewrite {
                        ref source_location, ..
                    } => {
                        let source_location = src::path::from_bstr(source_location.as_ref());
                        let source_location = src::path::relativize_with_prefix(&source_location, prefix);
                        writeln!(
                            out,
                            ...
                            dest_rela_path =
                                src::path::relativize_with_prefix(&src::path::from_bstr(location), prefix).display(),
                        )?;
                        continue;
                    }
                };
                writeln!(
                    out,
                    "{status: >2}  {rela_path}",
                    rela_path = src::path::relativize_with_prefix(&src::path::from_bstr(location), prefix).display(),
                )?;
            }
            ...
            status::Item::IndexWorktree(index_worktree::Item::DirectoryContents {
                ...
                if collapsed_directory_status.is_none() {
                    writeln!(
                        ...
                        status = "?",
                        rela_path =
                            src::path::relativize_with_prefix(&src::path::from_bstr(entry.rela_path), prefix).display(),
                        slash = if entry.disk_kind.unwrap_or(src::dir::entry::Kind::File).is_dir() {
                            "/"
                        } else {
                            ""
                        }
            ...
            status::Item::IndexWorktree(index_worktree::Item::Rewrite {
                ...
                writeln!(
                    ...
                    status = "R",
                    source_rela_path =
                        src::path::relativize_with_prefix(&src::path::from_bstr(source.rela_path()), prefix).display(),
                    dest_rela_path = src::path::relativize_with_prefix(
                        &src::path::from_bstr(dirwalk_entry.rela_path.as_bstr()),
                        prefix
                    )
                    .display(),
                )?;
    ...
    progress.init(Some(out.worktree_index.entries().len()), src::progress::count("files"));
...
fn print_index_entry_status(
    ...
    prefix: &Path,
    rela_path: &BStr,
    status: EntryStatus<(), src::submodule::Status>,
) -> std::io::Result<()> {
    let char_storage;
    ...
    let rela_path = src::path::from_bstr(rela_path);
    let display_path = src::path::relativize_with_prefix(&rela_path, prefix);
...
fn change_to_char(change: &Change<(), src::submodule::Status>) -> u8 {
