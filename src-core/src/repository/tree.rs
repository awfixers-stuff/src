...
mod entries {
    ...
    pub struct Traverse<'repo, 'a> {
        ...
        repo: Option<&'repo src::Repository>,
    ...
    impl<'repo, 'a> Traverse<'repo, 'a> {
        pub fn new(repo: Option<&'repo src::Repository>, out: Option<&'a mut dyn std::io::Write>) -> Self {
            Traverse {
    ...
    impl src::traverse::tree::Visit for Traverse<'_, '_> {
...
fn treeish_to_tree<'repo>(treeish: Option<&str>, repo: &'repo src::Repository) -> anyhow::Result<Tree<'repo>> {
...
fn format_entry(
    mut out: impl io::Write,
    entry: &src::objs::tree::EntryRef<'_>,
    filename: &src::bstr::BStr,
    size: Option<u64>,
) -> std::io::Result<()> {
    use src::objs::tree::EntryKind::*;
    write!(
        out,
        "{} {}{} ",
        match entry.mode.kind() {
