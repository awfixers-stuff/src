...
pub struct Options {
    ...
    pub shallow: src::remote::fetch::Shallow,
...
pub(crate) mod function {
        ...
        P::SubProgress: 'static,
    {
        ...
        if !ref_specs.is_empty() {
            remote.replace_refspecs(ref_specs.iter(), src::remote::Direction::Fetch)?;
            remote = remote.with_fetch_tags(src::remote::fetch::Tags::None);
        }
        let res: src::remote::fetch::Outcome = remote
            .connect(src::remote::Direction::Fetch)?
            .prepare_fetch(&mut progress, Default::default())?
            .with_dry_run(dry_run)
            .with_shallow(shallow)
            .receive(&mut progress, &src::interrupt::IS_INTERRUPTED)?;
        ...
        let ref_specs = remote.refspecs(src::remote::Direction::Fetch);
        match res.status {
    ...
    fn render_graph(
        repo: &src::Repository,
        graph: &src::negotiate::IdMap,
        path: &std::path::Path,
        mut progress: impl src::Progress,
    ) -> anyhow::Result<()> {
        progress.init(Some(graph.len()), src::progress::count("commits"));
        progress.set_name("building graph".into());

        let mut map = src::hashtable::HashMap::default();
        ...
        fn new_node(id: src::Id<'_>, flags: src::negotiate::Flags) -> Element {
    ...
    fn print_negotiate_info(
        mut out: impl std::io::Write,
        negotiate: Option<&src::remote::fetch::outcome::Negotiate>,
    ) -> std::io::Result<()> {
    ...
    pub(crate) fn print_updates(
        repo: &src::Repository,
        negotiate: &src::remote::fetch::outcome::Negotiate,
        update_refs: src::remote::fetch::refs::update::Outcome,
        refspecs: &[src::refspec::RefSpec],
        mut map: src::remote::fetch::RefMap,
        mut out: impl std::io::Write,
        mut err: impl std::io::Write,
    ) -> anyhow::Result<()> {
        let mut last_spec_index = src::remote::fetch::refmap::SpecIndex::ExplicitInRemote(usize::MAX);
        ...
        fn consume_skipped_tags(skipped: &mut Option<usize>, out: &mut impl std::io::Write) -> std::io::Result<()> {
            if let Some(skipped) = skipped.take() {
                if skipped != 0 {
                    writeln!(
                        out,
                        "\tskipped {skipped} tags known to the remote without bearing on this commit-graph. Use `src remote ref-map` to list them."
                    )?;
                }
            }
            Ok(())
        ...
        for (update, mapping, spec, edit) in updates {
            ...
            if let Some(num_skipped) = skipped_due_to_implicit_tag.as_mut() {
                if matches!(update.mode, src::remote::fetch::refs::update::Mode::NoChangeNeeded) {
                    *num_skipped += 1;
                    continue;
                }
            }
            ...
            match &mapping.remote {
                src::remote::fetch::refmap::Source::ObjectId(id) => {
                    write!(out, "{}", id.attach(repo).shorten_or_id())?;
                }
                src::remote::fetch::refmap::Source::Ref(r) => {
