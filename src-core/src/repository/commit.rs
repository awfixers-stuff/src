...
/// For this to go into `src`, one will have to implement many more options and various validation programs.
pub fn verify(repo: src::Repository, rev_spec: Option<&str>) -> Result<()> {
    ...
    let mut cmd: std::process::Command = src::command::prepare("gpg").into();
    cmd.args(["--keyid-format=long", "--status-fd=1", "--verify"])
        .arg(&signed_storage)
        .arg("-")
        .stdin(Stdio::piped());
    src::trace::debug!("About to execute {cmd:?}");
...
pub fn sign(repo: src::Repository, rev_spec: Option<&str>, mut out: impl std::io::Write) -> Result<()> {
    let rev_spec = rev_spec.unwrap_or("HEAD");
    let object = repo
        .rev_parse_single(format!("{rev_spec}^{{commit}}").as_str())?
        .object()?;
    ...
    if commit_ref.extra_headers().pgp_signature().is_some() {
        src::trace::info!("The commit {id} is already signed, did nothing", id = object.id);
...
pub fn describe(
    ...
    use src::commit::describe::SelectRef::*;
