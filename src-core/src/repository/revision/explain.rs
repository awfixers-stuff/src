...
pub fn explain(spec: std::ffi::OsString, mut out: impl std::io::Write) -> anyhow::Result<()> {
    ...
    src::revision::plumbing::spec::parse(spec, &mut explain).map_err(src::Error::from)?;
...
struct Explain<'a> {
    ...
    oid_prefix: Option<src::hash::Prefix>,
    ...
    fn reflog(&mut self, query: ReflogLookup) -> Result<(), Exn> {
        ...
        match query {
            ...
            ReflogLookup::Date(time) => writeln!(
                ...
                time.format_or_unix(src::date::time::format::ISO8601),
                ref_name
            )
            .ok(),
        };
