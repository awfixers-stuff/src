use src::worktree::IndexPersistedOrInMemory;
...
pub(crate) mod function {
    ...
    pub fn query(
        ...
        match input {
            PathsOrPatterns::Paths(paths) => {
                for path in paths {
                    let mode = src::path::from_bstr(Cow::Borrowed(path.as_ref()))
                        .metadata()
                        .ok()
                ...
                if !pathspec_matched_entry {
                    ...
                    for pattern in pathspec.search().patterns() {
                        ...
                        let entry = cache.at_entry(
                            path,
                            Some(is_dir_to_mode(
                                workdir.is_some_and(|wd| wd.join(src::path::from_bstr(path)).is_dir())
                                    || pattern.signature.contains(src::pathspec::MagicSignature::MUST_BE_DIR),
                            )),
                        )?;
                        if !entry.matching_attributes(&mut matches) {
                            continue;
    ...
    fn print_match(
        matches: &src::attrs::search::Outcome,
        path: &BStr,
...
pub(crate) fn attributes_cache(
    repo: &src::Repository,
) -> anyhow::Result<(src::AttributeStack<'_>, IndexPersistedOrInMemory)> {
    let index = repo.index_or_load_from_head()?;
    let cache = repo.attributes(
        &index,
        if repo.is_bare() {
            src::worktree::stack::state::attributes::Source::IdMapping
        } else {
            src::worktree::stack::state::attributes::Source::WorktreeThenIdMapping
        },
        src::worktree::stack::state::ignore::Source::IdMapping,
        None,
    )?;
    Ok((cache, index))
}
