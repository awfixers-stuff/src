...
pub(crate) mod function {
    ...
    use src::{attrs::Assignment, bstr::BString, Count, Progress};
    ...
    pub fn validate_baseline(
        ...
        let feed_attrs = {
            ...
            std::thread::spawn({
                ...
                move || -> anyhow::Result<()> {
                    let mut child =
                        std::process::Command::from(src::command::prepare(src::path::env::exe_invocation()))
                    ...
                    std::thread::spawn({
                        ...
                        move || -> anyhow::Result<()> {
                            progress.init(num_entries, src::progress::count("paths"));
        ...
        if mismatches.is_empty() {
            ...
            bail!(
                ...
                src::path::realpath(repo.workdir().unwrap_or(repo.git_dir()))?.display(),
                mismatches.len(),
    ...
    enum Baseline {
        Attribute { assignments: Vec<src::attrs::Assignment> },
    ...
    pub enum Mismatch {
        Attributes {
            actual: Vec<src::attrs::Assignment>,
            expected: Vec<src::attrs::Assignment>,
        },
        Exclude {
    ...
    pub struct ExcludeMatch {
        pub pattern: src::glob::Pattern,
        pub source: Option<PathBuf>,
        pub sequence_number: usize,
    }

    impl From<src::ignore::search::Match<'_>> for ExcludeMatch {
        fn from(value: src::ignore::search::Match<'_>) -> Self {
            ExcludeMatch {
    ...
    fn parse_attribute_line(line: &str) -> Option<(&str, src::attrs::AssignmentRef<'_>)> {
        use src::{attrs::StateRef, bstr::ByteSlice};
        ...
        if let Some(((mut path, attr), info)) = tokens.next().zip(tokens.next()).zip(tokens.next()) {
            ...
            let assignment = src::attrs::AssignmentRef {
                name: src::attrs::NameRef::try_from(attr.as_bytes().as_bstr()).ok()?,
                state,
            };
