...
use src::{bstr::BString, config::AsKey};
...
pub fn list(
    ...
    let repo = src::open_opts(repo.git_dir(), repo.open_options().clone().cli_overrides(overrides))?;
...
impl Filter {
    ...
    fn matches_section(&self, section: &src::config::file::Section<'_>) -> bool {
        let ignore_case = src::glob::wildmatch::Mode::IGNORE_CASE;

        if !src::glob::wildmatch(self.name.as_bytes().into(), section.header().name(), ignore_case) {
            return false;
        }
        match (self.subsection.as_deref(), section.header().subsection_name()) {
            (Some(filter), Some(name)) => {
                if !src::glob::wildmatch(filter.as_slice().into(), name, ignore_case) {
                    return false;
                }
            }
            (None, _) => {}
...
fn write_meta(meta: &src::config::file::Metadata, out: &mut impl std::io::Write) -> std::io::Result<()> {
    writeln!(
        out,
        "# From '{}' ({:?}{}{})",
        meta.path
        ...
        if meta.trust != src::sec::Trust::Full {
            ", untrusted"
        } else {
            Default::default()
        }
