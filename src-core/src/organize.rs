...
use src::{objs::bstr::ByteSlice, progress, NestedProgress, Progress};
...
) -> impl Iterator<Item = (PathBuf, src::repository::Kind)> {
    progress.init(None, progress::count("filesystem items"));

    #[derive(Debug, Clone, Copy)]
    struct RepoInfo {
        kind: src::repository::Kind,
        is_bare: bool,
    }
    ...
    fn is_repository(path: &Path) -> Option<RepoInfo> {
        ...
        if path.is_dir() {
            if path.join("HEAD").is_file() && path.join("config").is_file() {
                src::discover::is_git(path).ok().map(|discovered_kind| {
                    let is_bare = discovered_kind.is_bare();
                    let kind = match discovered_kind {
                        src::discover::repository::Kind::PossiblyBare => src::repository::Kind::Common,
                        src::discover::repository::Kind::WorkTree { linked_git_dir: None } => {
                            src::repository::Kind::Common
                        }
                        src::discover::repository::Kind::WorkTree {
                            linked_git_dir: Some(_),
                        } => src::repository::Kind::LinkedWorkTree,
                        src::discover::repository::Kind::WorkTreeGitDir { .. } => src::repository::Kind::LinkedWorkTree,
                        src::discover::repository::Kind::Submodule { .. } => src::repository::Kind::Submodule,
                        src::discover::repository::Kind::SubmoduleGitDir => src::repository::Kind::Submodule,
                    };
            ...
            Some(RepoInfo {
                kind: src::repository::Kind::LinkedWorkTree,
                is_bare: false,
            })
        }
    }
...
fn find_origin_remote(repo: &Path) -> anyhow::Result<Option<gix_url::Url>> {
    ...
    let local = src::config::Source::Local;
    let config = src::config::File::from_path_no_includes(non_bare.as_path().into(), local)
        .or_else(|_| src::config::File::from_path_no_includes(repo.join("config"), local))?;
    Ok(config
...
fn handle(
    ...
    if matches!(kind, src::repository::Kind::LinkedWorkTree) {
        return Ok(());
    }
        ...
        .join(to_relative({
            ...
            match kind {
                ...
                src::repository::Kind::Common => {
                    ...
                    if !src::discover::is_bare(&git_dir) {
