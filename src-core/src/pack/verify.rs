    ...
    let res = match ext {
        ...
        "idx" => {
            ...
            idx.verify_integrity(
                pack.as_ref().map(|p| src::odb::pack::index::verify::PackContext {
                    data: p,
                    options: src::odb::pack::index::verify::integrity::Options {
                        verify_mode: mode,
        ...
        "" => {
            match path.file_name() {
                Some(file_name) if file_name == "multi-pack-index" => {
                    let multi_index = src::odb::pack::multi_index::File::at(path)?;
                    let res = multi_index.verify_integrity(&mut progress, should_interrupt, src::odb::pack::index::verify::integrity::Options{
                        verify_mode: mode,
                        traversal: algorithm.into(),
                        thread_limit,
                        make_pack_lookup_cache: cache
