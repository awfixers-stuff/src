...
pub fn verify(
    ...
    src::odb::pack::multi_index::File::at(multi_index_path)?.verify_integrity_fast(&mut progress, should_interrupt)?;
    Ok(())
}
...
pub fn create(
    ...
    let mut out = BufWriter::new(src::lock::File::acquire_to_update_resource(
        output_path,
        src::lock::acquire::Fail::Immediately,
        None,
    )?);
    src::odb::pack::multi_index::File::write_from_index_paths(
        index_paths,
        &mut out,
        &mut progress,
        should_interrupt,
        src::odb::pack::multi_index::write::Options { object_hash },
    )?;
...
pub fn info(
    ...
    #[cfg(feature = "serde")]
    {
        let file = src::odb::pack::multi_index::File::at(&multi_index_path)?;
