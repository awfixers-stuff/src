    ...
    P::SubProgress: 'static,
{
    ...
    let repo = src::discover(repository_path)?.into_sync();
                    ...
                    move |tip| {
                        ObjectId::from_hex(&Vec::from_os_str_lossy(tip.as_ref())).or_else(|_| {
                            easy.find_reference(tip.as_ref())
                                ...
                                .and_then(|r| r.into_fully_peeled_id().map(src::Id::detach).map_err(Into::into))
                        })
                    }
                })
    ...
    let counts = {
        ...
        let (_, _, thread_count) = src::parallel::optimize_chunk_size_and_thread_limit(50, None, thread_limit, None);
