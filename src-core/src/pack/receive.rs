...
use src::protocol::transport::client::async_io::connect;
#[cfg(feature = "blocking-client")]
use src::protocol::transport::client::blocking_io::connect;
use src::{config::tree::Key, protocol::maybe_async, remote::fetch::Error, DynNestedProgress};
pub use src::{
    hash::ObjectId,
    ...
    P::SubProgress: 'static,
{
    ...
    let trace_packetlines = std::env::var_os(
        src::config::tree::Gitoxide::TRACE_PACKET
            .environment_override()
            .expect("set"),
    )
    .is_some();

    let agent = src::protocol::agent(src::env::agent());
    let mut handshake = src::protocol::handshake(
        &mut transport.inner,
        transport::Service::UploadPack,
        src::protocol::credentials::builtin,
    ...
    let fetch_refspecs: Vec<_> = wanted_refs
        .into_iter()
        .map(|ref_name| {
            src::refspec::parse(ref_name.as_bstr(), src::refspec::parse::Operation::Fetch).map(|r| r.to_owned())
        })
        .collect::<Result<_, _>>()?;
    let user_agent = ("agent", Some(agent.clone().into()));

    let context = src::protocol::fetch::refmap::init::Context {
    ...
    src::protocol::fetch(
        ...
        src::protocol::fetch::Context {
            handshake: &mut handshake,
            transport: &mut transport.inner,
            user_agent,
            trace_packetlines,
        },
        src::protocol::fetch::Options {
...
struct Negotiate<'a> {
    refmap: &'a src::protocol::fetch::RefMap,
}

impl src::protocol::fetch::Negotiate for Negotiate<'_> {
...
fn write_raw_refs(refs: &[Ref], directory: PathBuf) -> std::io::Result<()> {
    let assure_dir_exists = |path: &BString| {
        ...
        let path = directory.join(src::path::from_byte_slice(path));
