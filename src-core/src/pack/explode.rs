...
impl src::objs::Write for OutputWriter {
    fn write_buf(&self, kind: object::Kind, from: &[u8]) -> Result<ObjectId, src::objs::write::Error> {
        match self {
    ...
    fn write_stream(
        ...
        size: u64,
        from: &mut dyn Read,
    ) -> Result<ObjectId, src::objs::write::Error> {
        match self {
...
impl OutputWriter {
    fn new(path: Option<impl AsRef<Path>>, compress: bool, object_hash: src::hash::Kind) -> Self {
        match path {
