...
fn parse_file(index_path: impl AsRef<Path>, object_hash: src::hash::Kind) -> anyhow::Result<src::index::File> {
    src::index::File::at(index_path.as_ref(), object_hash, false, Default::default()).map_err(Into::into)
}
...
pub fn verify(
    ...
    file.verify_entries()?;
    file.verify_extensions(false, src::objs::find::Never)?;
