...
mod serde_only {
    ...
    mod ext {
        ...
        mod tree {
            ...
            impl<'a> From<&'a src::index::extension::Tree> for super::Tree {
                fn from(t: &'a src::index::extension::Tree) -> Self {
                    super::Tree {
    ...
    impl Collection {
        pub fn try_from_file(f: src::index::File, extension_details: bool) -> anyhow::Result<Self> {
            Ok(Collection {
                ...
                entries: {
                    ...
                    for entry in f.entries() {
                        ...
                        match entry.mode {
                            src::index::entry::Mode::DIR => dir += 1,
                            src::index::entry::Mode::FILE => file += 1,
                            src::index::entry::Mode::FILE_EXECUTABLE => executable += 1,
                            src::index::entry::Mode::SYMLINK => symlink += 1,
                            src::index::entry::Mode::COMMIT => submodule += 1,
                            _ => other += 1,
                        }
                        if entry.flags.contains(src::index::entry::Flags::INTENT_TO_ADD) {
                            intent_to_add += 1;
                        }
                        if entry.flags.contains(src::index::entry::Flags::SKIP_WORKTREE) {
                            skip_worktree += 1;
                        }
                    }
                    Entries {
