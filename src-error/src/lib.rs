...
//! * When there **is callee error to track** *in a `src-plumbing`*, use e.g. `Result<_, Exn<Simple>>`.
//!      - Remember that `Exn<T>` does not implement `std::error::Error` so it's not easy to use outside `src-` crates.
//!      - Use the type-erased version in callbacks like [`Exn`] (without type arguments), i.e. `Result<T, Exn>`.
//! * When there **is callee error to track** *in the `src` crate*, convert both `std::error::Error` and `Exn<E>` into [`Error`]
//!
...
//!
//! This section describes the mechanical translation from `thiserror` error enums to `src-error`.
//! In `Cargo.toml`, replace `thiserror = "<version>"` with `src-error = { version = "^0.1.0", path = "../src-error" }`.
//!
...
//!
//! Porcelain crates (like `src`) should not expose [`Exn<Message>`](Exn) in their public API
