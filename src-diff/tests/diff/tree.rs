mod changes {
    mod to_obtain_tree {
        ...
        fn directory_rename() -> crate::Result {
            ...
            assert_eq!(
                diff_with_previous_commit_from(&db, &all_commits["rename git-sec to src-sec"])?,
                vec![
                    Deletion {
                        entry_mode: EntryKind::Tree.into(),
        ...
        fn reverse_directory_rename() -> crate::Result {
            ...
            assert_eq!(
                diff_with_previous_commit_from(&db, &all_commits["rename src-sec to git-sec"])?,
                vec![
                    Addition {
                        entry_mode: EntryKind::Tree.into(),
                        oid: hex_to_id("d8c30fb72173778ed57fac5813c5e37038a8746c"),
