...
rm -Rf ./* && mkdir git-sec src && touch a git-sec/2 git-sequencer h src/5 && git add .
git commit -am "clear slate"

mkdir git-sec/subdir && touch git-sec/subdir/6 git-sec/7
git add . && git commit -m "add files to git-sec"

git mv git-sec src-sec && git commit -m "rename git-sec to src-sec"

git mv src-sec git-sec && git commit -m "rename src-sec to git-sec"
