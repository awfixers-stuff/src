...
For example, `src-odb` and `src-ref` both have breaking changes, where `src-odb`'s change is on August 1st and `src-ref`'s dependent change
is on September 10th. `src-odb`'s breaking change can be released earliest on September 1st, whereas `src-ref` can be released earliest at October 10th.
...
The MSRV is automatically assumed to be the latest stable version for all crates with the following exception: `src` and all it's dependencies must
