mod _ref {
    ...
    impl<'a> SignatureRef<'a> {
        ...
        pub fn to_owned(&self) -> Result<Signature, src_date::Error> {
            Ok(Signature {
    ...
    impl<'a> SignatureRef<'a> {
        ...
        pub fn seconds(&self) -> src_date::SecondsSinceUnixEpoch {
            self.time
                .trim()
                .split(' ')
                .next()
        ...
        /// The format is expected to be [raw](src_date::parse_header()).
        pub fn time(&self) -> Result<src_date::Time, src_date::Error> {
            self.time.parse()
        }
    }
}
...
pub(crate) mod write {
    ...
    pub(crate) fn validated_token(name: &BStr) -> Result<&BStr, src_error::ValidationError> {
        if name.find_byteset(b"<>\n").is_some() {
            return Err(src_error::ValidationError::new_with_input(
