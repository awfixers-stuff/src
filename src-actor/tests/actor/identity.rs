use bstr::ByteSlice;
use src_actor::Identity;

#[test]
fn round_trip() -> src_testtools::Result {
    static DEFAULTS: &[&[u8]] =     &[
    ...
    for input in DEFAULTS {
        let signature: Identity = src_actor::IdentityRef::from_bytes::<()>(input).unwrap().into();
...
#[test]
fn lenient_parsing() -> src_testtools::Result {
    for (input, expected_email) in [
        ...
        let identity = src_actor::IdentityRef::from_bytes::<()>(input.as_bytes()).unwrap();
