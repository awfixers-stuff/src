...
use bstr::ByteSlice;
use src_actor::{Signature, SignatureRef};

#[test]
fn trim() {
    let sig = src_actor::SignatureRef::from_bytes::<()>(b" \t hello there \t < \t email \t > 1 -0030").unwrap();
    let sig = sig.trim();
...
fn round_trip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for input in DEFAULTS {
        let signature: Signature = src_actor::SignatureRef::from_bytes::<()>(input).unwrap().into();
...
fn signature_ref_round_trips_with_seconds_in_offset() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let signature: SignatureRef = src_actor::SignatureRef::from_bytes::<()>(input).unwrap();
    let mut output = Vec::new();
    signature.write_to(&mut output)?;
    assert_eq!(output.as_bstr(), input.as_bstr());
    Ok(())
...
fn parse_timestamp_with_trailing_digits() {
    let signature = src_actor::SignatureRef::from_bytes::<()>(b"first last <name@example.com> 1312735823 +051800")
    ...
    let signature = src_actor::SignatureRef::from_bytes::<()>(b"first last <name@example.com> 1312735823 +0518")
...
fn parse_missing_timestamp() {
    let signature = src_actor::SignatureRef::from_bytes::<()>(b"first last <name@example.com>")
