...
* **async**
  ...
  * **User Interfaces**
    ...
    * `blocking` can be used to make `Read` and `Iterator` async, or move any operation onto a thread which blends it into the
       ...
       * Long-running operations can be roughly interacted with using `src_features::interrupt::trigger()` function, and after a moment
...
* **interruption of long-running operations**
  * Use `src-features::interrupt::*` for building support for interruptions of long-running operations only.
...
* **prepare for SHA256 support by using `src_hash::ObjectId` and `src_hash::oid`**
...
* **when to use interior mutability**
  ...
  - When using primitives to support interior mutability, use the provided ones and utility functions in `src_features::threading::*` exclusively to allow switching between
  ...
  - Types that are not thread-local must be `Sync`, but only if the `src-features/parallel` is enabled due to the usage of `src_features::threading::…` primitives which won't
    be thread-safe without the feature.
* **when to use shared ownership**
  - Use `src_features::threading::OwnShared` particularly when shared resources supposed to be used by thread-local handles. Going through a wrapper for shared ownership is fast
...
- Use `Context` when data is required to perform an operation at all. See `src_config::path::Context` as reference. It can't be defaulted and the fields could also be parameters.

## Lifetimes

In _plumbing_ crates, prefer to default to keeping references if this is feasible to avoid typically expensive clones.

In _porcelain_ crates, like `src`, we have `Platforms` which are typically cheap enough to create on demand as they configure one or more method calls. These
...
- Be sure to clone locally and run tests with `src_TEST_IGNORE_ARCHIVES=1` to assure new fixture scripts (if there are any) are validated
